...
void temple(void)
{
	...
	for(t=0;t<9;t++)
	{
	 ...
	 cost[t] = tempcost;
			...
			if (c[charselectnew].stamina < c[charselectnew].staminamax) ForeColor(whiteColor);
			...
			if (c[charselectnew].stamina < 1)
			{
			 ...
			 InsetRect(&itemRect,9,9);
			 if (c[charselectnew].stamina < -9) ploticon3(2015,itemRect);
			  else ploticon3(177,itemRect);
			}
				
			DialogNum(55,c[charselectnew].money[0]);
			DialogNumLong(56,moneypool[0]);
			
		    for(t=0;t<5;t++) MyrCDiStr(59+t,(StringPtr)"");
		    for(t=0;t<40;t++)
		    {
