...
SetCCursor(sword);
  while (Button())
  {
   ...
   if ((temp <= maxloop) && (temp != tempold) && (temp > -1))
   {
    ...
    if (temp < 9)
    {
     if (c[temp].guarding) SetCCursor(shield);
     else SetCCursor(sword);
    }
    else
    {
     if (monster[temp-10].guarding) SetCCursor(shield);
     else SetCCursor(sword);
    }
    combatupdate2(temp);
    tempold = temp;
  ...
  if (inspell) combatupdate2(charup);  /*** snaps back to char ****/
  infocombat = FALSE;
...
void combatchoice(void)
{
 ...
 if (theControl == melee)
 {
  ...
  if (c[charup].toggle)
  {
	if (!c[charup].armor[2]) c[charup].weaponsound = (30 + c[charup].gender*8);
	 else
	 {
  ...
  {
	if (c[charup].armor[15]) c[charup].toggle = 1;
  }
  if (temp != c[charup].toggle) combatupdate2(charup);
   else sound(6000);
 }
 ...
 if (theControl == monsterbut)
 {
  ...
  in();
  downbutton(TRUE);
  if (lastshown > 9) beast(lastshown,2,0);
   else viewcharacter(lastshown,0);
  out();
  centerstage(0);
 ...
 if (theControl == showitems) 
 {
  ...
  downbutton(TRUE);
  SetPort(GetWindowPort(look));
  if (lastshown < 9) showcondition(lastshown,lastshown,1,0,lastshown);
   else showcondition2(lastshown,1);
  upbutton(TRUE);
  FlushEvents(everyEvent,0);
 }
 
 if (theControl == turn) goto forceturn;
 ...
 if (theControl == condition)
 {
  ...
  downbutton(TRUE);
  if (lastshown < 9) showcondition(lastshown,lastshown,0,0,lastshown);
  else showcondition2(lastshown,0);
  upbutton(TRUE);
  FlushEvents(everyEvent,0);
 }
 ...
 if (theControl == attacks)
 {
  if (lastshown > 9)
  {
   ...
   downbutton(TRUE);
   beast(lastshown,0,0);
   if (inwindow(charup)) combatupdate2(lastshown);
 }
  else sound(6000);
  FlushEvents(everyEvent,0);
}
  ...
  if (theControl == campbut) /****** auto move *******/
  {
   ...
   sound(141);
    ...
    if (!inwindow(charup))
    {
	  if (inspell) goto centercharupspell;
	    else  centerfield(pos[charup][0],pos[charup][1]);
    }
    if (!c[charup].condition[25]) c[charup].condition[25] = TRUE;
    FlushEvents(everyEvent,0);
  }
 ...
 if ((theControl == viewspellsbut) && (c[charup].armor[13]))
 {
  ...
  ploticon3(129,r);
  if (incombat) charselectnew = q[up];
  if (getscroll()) 
    {
     ...
     else
     {
      ...
      FlushEvents(everyEvent,0);
				  ...
				  if (c[charup].condition[25])
				  {
				   ...
				   {
				    temp = c[charup].toggle;
				    if (c[charup].handtohand < 8) toweap(charup);
				    if (c[charup].toggle != temp) combatupdate2(charup);
				    theControl = movelook;
				   }
				   else
				   {
    ...
    if ((theControl == combatitem) && (lastshown == q[up]))
    {
	 ...
	 downbutton(TRUE);
       ...
       if (!c[charup].toggle) loaditem(c[charup].armor[2]);
        else
        {
         if (c[charup].armor[10])
         {
          loaditem(c[charup].armor[10]);
          if (item.sp2 > 1100) loadspell2(item.sp2);
           else loaditem(c[charup].armor[15]);
         }
          else loaditem(c[charup].armor[15]);
        }
       ...
       if (item.sp2 > 1100)
          {
           ...
           if ((item.sp2 > 1100) && (c[charup].toggle) && (item.charge != -1) && (item.type == 10)) arrowused = TRUE;
            else if (c[charup].toggle) arrowused = FALSE;
           ...
           if (!c[charup].toggle) itemused = c[charup].armor[2];
            else if (arrowused) itemused = c[charup].armor[10];
             else itemused = c[charup].armor[15];
           ...
           if (powerlevel == 8) powerlevel = Rand(7);
           
           if (!arrowused)
           {
	           for(itemnum=0;itemnum<c[charup].numitems;itemnum++)
	           {
	            if ((c[charup].items[itemnum].id == c[charup].armor[2+(13*c[charup].toggle)]) && (c[charup].items[itemnum].equip)) goto bustout;
			   }
		   }
		   else
		   {
	           for(itemnum=0;itemnum<c[charup].numitems;itemnum++)
	           {
	            if ((c[charup].items[itemnum].id == c[charup].armor[10]) && (c[charup].items[itemnum].equip)) goto bustout;
			   }
		   }
...
shottry = 0;
			  ...
			  if (c[charup].condition[25]) /****** possesed char using spell/missle weap ***/
			  {
			   ...
			   if (!getrange(charup,c[charup].traiter,FALSE)) goto notargets;
			   
			tryover:
			   
			   temp = c[charup].target;
			   if ((twixt(temp,6,9)) || (temp < 0)) goto getnew;
			   if (temp < 10)
				{
				 if ((c[temp].traiter != c[charup].traiter) && (c[temp].inbattle)) goto keepmoving;
					else goto getnew;
				}
...
keepmoving:
			   ...
			   target[0][0] = temp;
			     ...
			     if (range[temp] > spellrange) goto getnew;
		         ...
		         if ((!c[charup].items[itemnum].charge) && (item.drop)) dropitem(charup,itemused,itemnum,1,FALSE);
		           ...
		           if (c[charup].attacks > 1) combatupdate(charup);
		           if (c[charup].stamina < 1) c[charup].attacks = 0;
		           return;
		          }
		          else 
		          {
		           getnew:
		           if (!cycle) c[charup].target = randrange(0,maxloopminus);
		            else
		            {
		             c[charup].target++;
		             if (c[charup].target > maxloopminus) c[charup].target = 0;
		            }
    ...
    if (theControl == castspellsbut)
    {
     if (!cancast(charup,0))
     {
      if ((fastspell) && (memoryspell == TRUE)) c[charup].spellpoints += abs(spellinfo.cost*powerlevel);
      inspell = fastspell = 0;
      sound(143);
      return;
     }
...
wand:
     ...
     if (spellinfo.special == 58)//**** summon monster, lets pick what they get right away
      {      
	   ...
	   if (sumsize > 27) sumsize = 200;/*** max size cratures ***/
...
onemoretime:
    ...
    tryagain:
	 ...
	 if ((pickfile = MyrFopen(filename,"rb")) != NULL)
	 {
        ...
        if (itemused)
        {
	     ...
	     else templong = randrange(0,200); /********** setmonster for summons ********/
        }
        else templong = randrange(0,200); /********** setmonster for summons ********/
        
	    if (spellinfo.spellclass) templong = spellinfo.spellclass;
	...
	good:
     ...
     if (spellinfo.targettype < 1) numoftar = powerlevel;
      else numoftar = 1;
      
     showspellinfo();
     
    if ((spellrange == 0) && (spellinfo.targettype < 8)) goto castonself;
	...
	if (spellinfo.targettype > 8)
     	 {
     	  for(t=0;t<maxloop;t++) track[t] = 0;
     	  
	      if (spellinfo.targettype == 12)/*** Target Everybody ***/
	      {
     	   for(t=0;t<=charnum;t++) if (c[t].inbattle) track[t] = TRUE;
     	   for(t=0;t<nummon;t++) if (monster[t].stamina > 0) track[t+10] = TRUE;
	      }
	        
     	  if (spellinfo.targettype == 9)/**** all friendly ****/
     	  {
     	   for(t=0;t<=charnum;t++) if ((!c[t].traiter) && (c[t].inbattle)) track[t] = TRUE;
     	   for(t=0;t<nummon;t++) if ((!monster[t].traiter) && (monster[t].stamina > 0)) track[t+10] = TRUE;
     	  }
     	  else /***** all enemy ****/
     	  {
     	   for(t=0;t<=charnum;t++) if ((c[t].traiter) && (c[t].inbattle)) track[t] = TRUE;
     ...
     {  
      ...
      if (spellx > 9 + (screensize*5)) spellx = (9 + (screensize*5));
      if (spelly > 9 + (screensize*3)) spelly = (9 + (screensize*3));
      
      if (spellx < 0) spellx = 0;
      if (spelly < 0) spelly = 0;
      ...
      if (TickCount() - oldtick > 7)
      {
       ...
       if (spellinfo.canrotate)
       {
        ...
        if (rotateindex > 7) rotateindex = 0;
      ...
      testforinfo: 
		     if (gTheEvent.modifiers & cmdKey)
		      {
		       if (inspell) centerfield(5+(2*screensize),5+screensize);//   Fantasoft 7.1
		       combatinfo(charup);
		     ...
		     if (PtInRect(point,&buttonrect)) key = '-';
		     ...
		     if (PtInRect(point,&buttonrect)) key = 'a';
		     ...
		     buttonrect.right = buttonrect.left + 49;
		     if (PtInRect(point,&buttonrect)) key = 'n';

		     /************ p ** center on previous ******************/
		     oldspellx = oldspelly = spellx = spelly = 5;
		     buttonrect.left = 417+leftshift;
		     buttonrect.right = buttonrect.left + 49;
		     if (PtInRect(point,&buttonrect)) key = 'p';

		     /********** c ** center on charup *******************/
			 buttonrect.left = 471+leftshift;
			 buttonrect.right = buttonrect.left + 49;
		     if (PtInRect(point,&buttonrect)) key = 'c';
     		  ...
     		  if (spellinfo.special == 57)
     		  {
			      ...
			      if (c[temptemp].stamina > 0)/***** bring back ****/
			      {
					...
					for(loopud=start;loopud<stop;loopud++)
					{
						...
						good2:
						sound(631);
						if (c[temptemp].condition[6] > -1) c[temptemp].condition[6] = 3;
						c[temptemp].traiter = c[temptemp].bleeding = inspell = 0;
						c[temptemp].target = -1;
						c[temptemp].beenattacked = c[temptemp].inbattle = TRUE;
						
						if (c[temptemp].condition[5] > -1) c[temptemp].condition[5] = 2;
			...
			break;
		     ...
		     dpl:
			     ...
			     if ((!usescroll) && (powerlevel > 1) && (numoftar > targetnum))
			     {
			      ...
			      if (!spellinfo.targettype) numoftar--;
			      ...
			      if (memoryspell == TRUE) c[charup].spellpoints += spellinfo.cost;
			      updatecharshort(charup,FALSE);
			      delay(5);
			      combatupdate2(charup);
			      showspellinfo();
		     ...
		     case 'a':/************ a ** abort spell ******************/
			     ...
			     SetCCursor(sword);
			     c[charup].movement -= 3;
			     if ((memoryspell) && (charup < 6)) c[charup].spellsofar--;
			     cleartarget();
			     if (c[charup].movement < 0) c[charup].movement = 0;
			     if ((!usescroll) && (memoryspell == TRUE)) c[charup].spellpoints += (.66*(spellinfo.cost * powerlevel));
...
launch:
		     ...
		     if (spellinfo.targettype >1)
		     {
     		 ...
     		 else if (((temptarget <= maxloop) && (!spellinfo.size)) || ((temptarget > maxloop) && (spellinfo.size)))
     		     {
     		      for(t=0;t<7;t++)
     		      {
     		       ...
     		       {
     		        if (spellinfo.spellclass == 9) missileset(dx,dy);
		            if (!spellinfo.size)
		            {
		             ...
		             target[targetnum][1] = 1;
		             ...
		             else
		                {
						 ...
						 {
						  if (mapstats[hit0-1000].solid) bad = TRUE;
						 }
						  else bad = TRUE;

		                 if ((size == 1) || (size == 3))
						 {
						  if (hit2 < 999) bad = TRUE;
							else if (mapstats[hit2-1000].solid > 1) bad = TRUE;
						 }
		                
		                 if (size > 1)
						 {
						  if (hit3 < 999) bad = TRUE;
							else if (mapstats[hit3-1000].solid > 1) bad = TRUE;
						 }
		                
		                 if (size == 3)
						 {
						  if (hit1 < 999) bad = TRUE;
							else if (mapstats[hit1-1000].solid > 1) bad = TRUE;
						 }
		             ...
		             targetrect.left = 32*spellx;
		              ...
		              if (spellinfo.size)/**** for summon monster ***/
		               {
			            if ((monpick.size == 1) || (monpick.size == 3)) targetrect.top -=16;
			            if (monpick.size >1) targetrect.left -=16;
			           }
		                targetrect.bottom = targetrect.top+32;
		                targetrect.right = targetrect.left+32;
		           } 
		            else if (targetnum < numoftar)
		              {
		               ...
		               updatespell(1);
		               size = 0;
		               if (temptarget > 9) size=monster[temptarget-10].size;
	                   targetrect.top = 32*monpos[temptarget-10][1];
	                   targetrect.left = 32*monpos[temptarget-10][0];
		               
			            if ((size==1) || (size==3)) targetrect.top -=16;
			            if (size >1) targetrect.left -=16;
		                targetrect.bottom = targetrect.top+32;
		                targetrect.right = targetrect.left+32;
		              }
		              else 
		      ...
		      else combaterrors(2);/*********** out of range *********/
		      }
		    break;
		}
      pass:
    ...
    if ((theControl == charmainbut) && (abs(point.v/50) <= charnum))
	     ...
	     if (!inwindow(charup)) 
	     {
	      if (inspell) goto centercharupspell;
	       else 
	       {
	        ...
	        if ((pos[charup][0] + deltax + fieldx < 3) || (pos[charup][1] + deltay + fieldy < 3) || (pos[charup][0] + deltax + fieldx > 86) || (pos[charup][1] + deltay + fieldy > 86)) goto canflee;
     ...
     if (lastshown != charup) combatupdate2(charup);
     ...
     if ((poss) || (c[charup].condition[25]))
       {
        if (checkforenemy(1)) c[charup].target = enemy[0];
        else
        {
         if (c[charup].target < 0)
         {
		  ...
		  if (twixt(temp,6,9)) goto getnewt;

          if (temp < 10)
		  {
		   if ((c[temp].traiter != c[charup].traiter) && (c[temp].inbattle)) c[charup].target = temp;
			else goto getnewt;
		  }
			else if ((c[charup].traiter != monster[temp-10].traiter) && (monster[temp-10].stamina > 0)) c[charup].target = temp;
			 else goto getnewt;
         }
        }
         temptarget = c[charup].target;
          ...
          if (temptarget < 9)
          {
		   if ((!c[temptarget].inbattle) || (c[temptarget].stamina < 1) || (c[charup].traiter == c[temptarget].traiter)) goto getnewt;
           targetx = pos[temptarget][0];
           targety = pos[temptarget][1];
          }
          else
          {
		   if ((monster[temptarget-10].stamina < 1) || (monster[temptarget-10].traiter == c[charup].traiter)) goto getnewt;
           targetx = monpos[temptarget-10][0];
           targety = monpos[temptarget-10][1];
          }
          
          deltax = deltay = 0;
          if (targetx < pos[charup][0]) deltax = -1;
          if (targetx > pos[charup][0]) deltax = 1;
          if (targety < pos[charup][1]) deltay = -1;
          if (targety > pos[charup][1]) deltay = 1;
      ...
      if (point.v < pos[charup][1] *32) deltay --;
      if (point.v > pos[charup][1] *32 + 32) deltay ++;
      if (point.h < pos[charup][0] *32) deltax --;
      if (point.h > pos[charup][0] *32 + 32) deltax ++;
     }
      key=0;
   ...
   moveforward:
	  ...
	  if (deltax < 0) c[charup].face = 1;
	   else if (deltax > 0) c[charup].face = 0;
      ...
      if (hit <= maxloop) 
      {
       if (hit < 9)
		{
		 if ((c[charup].traiter != c[hit].traiter)) goto attackfriendhere;
		}
		else if (monster[hit-10].traiter != c[charup].traiter)
        {
         attackfriendhere:
	       ...
	       if (!c[charup].toggle)
	       {
	         if (c[charup].armor[2]) loaditem(c[charup].armor[2]);
	       }
	        else if (c[charup].armor[15]) loaditem(c[charup].armor[15]);
	         else goto noweapon;
      ...
      noweapon:
         ...
         return;
              ...
              if (hit > 9)
				{
				 if ((!monster[hit-10].size) && (c[charup].movement > 4)) goto goodswap;
					else if ((!poss) && (!c[charup].condition[25])) goto askem;
					 else if ((poss) || (c[charup].condition[25])) goto loopposs;
				}
			else if (c[charup].movement > 4)
              {
	goodswap:
                  if ((poss) || (c[charup].condition[25])) goto autoswap;
	              ...
	              else if (itemHit == 1) return;/**** hit esc key ****/
	               else goto wantstoattack;
	          }
...
if (hit > 999)
	{
      ...
      {
       if ((!poss) && (!c[charup].condition[25])) sound(-129);
        else goto loopposs; /******* send poss char back to try again **************/
      }
		else goto keepchecking;
	}
      else
		{
keepchecking:
		 ...
		 if (c[charup].movement - cost > -1)
		  {
	        ...
	        if (c[charup].beenattacked != beenattacked) updatecontrols();
	        bodyground(charup,0);
	        /******** tag field ********/
      ...
      {
       deltax = deltay = 0;
       if ((poss) || (c[charup].condition[25])) goto loopposs;
       sound(-130);
      }
      
        if (checkforenemy(0)) getup(FALSE);
     }
}