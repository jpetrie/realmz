    ...
    PtoCstr((StringPtr)c[charselectnew].name);
		...
		for(t=0;t<5;t++)
		{
		 ...
		 for(t=0;t<c[charselectnew].numitems;t++) 
		  {
		   if (c[charselectnew].items[t].id == 806) numparch += c[charselectnew].items[t].charge;
		  }
		  	iconhand = NIL;
		  	
            if (numparch > 7) iconhand = GetCIcon(179);
            else if (numparch) iconhand = GetCIcon(167+numparch);
		     else  iconhand = GetCIcon(176);
    ...
    for (;;)
     {
        ...
        if (itemHit == 15)
         {
          free = FALSE;
          for(t=0;t<5;t++) if (!c[charselectnew].scrollcase[t].powerlevel) free = TRUE;
                   ...
                   if (c[charselectnew].items[tt].id == 806)
                    {
                     ...
                     if (!c[charselectnew].items[tt].charge) dropitem(charselectnew,806,tt,0,FALSE);
                     goto bustout;
                    }
                  }
                  goto bustout;
