...
over:
	...
	for(;;)
	{
		...
		{
			if (DialogSelect(&gTheEvent,&dummy,&itemHit)) goto out;
		}
...
{	
	...
	if (prompt1)
	{
	 ...
	 if ((fp = MyrFopen(filename,"rb")) == NULL)
	 {
	  ...
	  if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(134);
 ...
 out:
        ...
        DisposeDialog(question);
        SetPort(oldPort);
        if (!incombat) updatefat(TRUE,0,FALSE);
         else updatefat(FALSE,0,TRUE);
        return(itemHit -1);
}
/***************************** question3 ********************************/
