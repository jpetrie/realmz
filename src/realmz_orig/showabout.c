...
ModalDialog(0L,&itemHit);
	...
	if (!StringWidth(Name_String)) exit(0);
...
changed:
	...
	#ifdef PC
		if (MyrBitTstLong(&serial,6+divine)) ParamText(myString,(Ptr)str,theString,(StringPtr)"\pP");
		 else ParamText(myString,Name_String,theString,(StringPtr)"\pP");
	#else
		if (MyrBitTstLong(&serial,6+divine)) ParamText(myString,(StringPtr)str,theString,(StringPtr)"\pX");
		 else ParamText(myString,Name_String,theString,(StringPtr)"\pX");
	#endif
	...
	if (iconhand) DisposeCIcon(iconhand);
	  ...
	  if (itemHit == 11)
 		 {
 		  ...
 		  if (!StringWidth(myString)) goto donothing;
