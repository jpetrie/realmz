...
void Spellselect(void)
{
 ...
 lock = characterl;
          ...
          for(t=0;t<7;t++) DialogNum(24+t,costs[t]);
          ...
          if ((itemHit < 9) && (itemHit > 1))
            {
              ...
              for(t=0;t<12;t++)
              {
				 ...
				 buttonrect.left -=2;
				 buttonrect.top --;
               if (characterl.cspells[spelllevel][t]) downbutton(FALSE);
                else
                {
                 upbutton(FALSE);
                 ForeColor(yellowColor);
               ...
               if (characterl.cspells[spelllevel][t]) DrawPicture(on,&light);
	 ...
	 for(t=0;t<7;t++)/************ update canheal,canidentify,candetect status ********/
	 {
	  for(tt=0;tt<12;tt++)
	  {
	   if (characterl.cspells[t][tt])
	   {
	    ...
	    if (spellinfo.special == 57) characterl.canheal = TRUE;
	    if (spellinfo.special == 48) characterl.canidentify = TRUE;
	    if (spellinfo.special == 63) characterl.candetect = TRUE;
	   }
	  }
	 }
