...
short thiefenc(void)
{
 ...
 if ((fp = MyrFopen(":Data Files:CT","rb")) == NULL) scratch(51);
 ...
 if (Rand(100) < 10)
 {
  if ((!doreg4()) && (currentscenario > 10)) scratch(452);
 }
		 
	bigupdate:
	
		 if (thief.sound[0]) sound(thief.sound[0]);	
		...
		update2:
			...
			PtoCstr((StringPtr)c[charselectnew].name);
				...
				for(t=24;t<32;t++)
				{
				 ...
				 {
				  upbutton(FALSE);
				  if (!first) ForeColor(yellowColor);
				  
				 }
				  else downbutton(FALSE);

				 MyrPascalDiStr(t,myString);
				 if (c[charselectnew].spec[t-19]) DialogNum(t+8,c[charselectnew].spec[t-19]+thief.modifer[t-24]);
				  else DialogNum(t+8,0);
			...
			for (;;)
			{
				...
				if ((itemHit > 7) && (itemHit < 16) && (c[charselectnew].spec[itemHit - 3] > 0) && (thief.type[itemHit-8]))
				{
				 ...
				 if ((type == 4) && (thief.type[9])) thief.type[4] = TRUE;/*** reset bendbars on trap triggger ***/
				 ...
				 if ((type != 2) && (type != 6) && (type != 7) && (type != 4))
				 {
				  if (Rand(100) <= (c[charselectnew].spec[itemHit - 3] + thief.modifer[itemHit - 8]))
				  {
				   ...
				   else if (type == 1) textbox(-1,thief.textf[type],0,0,textrect);/**** failed to detect trap ****/
				     else goto dotrap;
				 ...
				 {
				  if ((thief.type[9]) && (type != 2)) goto sprungtrap;
				  
				  if (picklock(charselectnew,type))/**** success ****/
				  {
					sound(thief.sounds[type]);	
					textbox(-1,thief.texts[type],0,0,textrect);
				    if (type == 2) thief.type[9] = 0;/**** disarm trap *****/
					   if (thief.codes[type]) 
					   {
					    reply = thief.codes[type];
					    if (reply != 4) c[charselectnew].exp += (300 * thief.tumblers);
					    goto out;
					   }
				  }
				  else  /***** failed *****/
				 ...
				 dotrap:
				   ...
				   if (thief.type[9])/***** trap is set and triggered ****/
				   {
				    ...
				    if (thief.spell)
				    {
				     if (thief.type[8]) track[charselectnew] = 1;
				      else for(t=0;t<=charnum;t++) track[t] = 1;
				...
				{
				 if ((fp = MyrFopen(":Data Files:CT","r+b")) == NULL) scratch(52);
				 fseek(fp,enc2.thiefsuccess * sizeof thief,SEEK_SET);
				 CvtThiefToPc(&thief);
				...
				GetDialogItem(templewindow,itemHit,&itemType,&itemHandle,&buttonrect);
				charselectnew++;
				if (itemHit == 3) charselectnew -=2;
				if (charselectnew > charnum) charselectnew = 0;
				if (charselectnew < 0) charselectnew = charnum;
				if ((c[charselectnew].stamina < 1) || (c[charselectnew].condition[25])) goto tryover;
				ploticon3(136,buttonrect);
				if (beenthere) goto update2;
				goto update;
				}
...
short encounter(short id,short mode)
{
	...
	SetCCursor(sword);
	 ...
	 {
	  sound(20005);
	  if ((fp = MyrFopen(":Data Files Data:CE","rb")) == NULL) scratch(53);
	...
	for(t=0;t<4;t++) if (!enc.choiceresult[t]) GetIndString(buffer[t],3,1);
	...
	TextFont(defaultfont);
	ForeColor(yellowColor);
	if (!enc.canbackout) SizeWindow(GetDialogWindow(win),259,267,0);
		else SizeWindow(GetDialogWindow(win),259,320,0);
...
over:
	...
	if (itemHit == 9) reply = 0;
	else if (enc.choiceresult[itemHit-1]) reply = itemHit;
	else
	{
		sound(6000);
		goto over;
	...
	centerpict();
	EraseRect(&textrect);
	if (reply) return (enc.choiceresult[reply-1]);
	 else return(0);
}
/**************** encounter2 *** Complex ****************************/
short encounter2(void)
{
	...
	if (!enc2.canbackout) SizeWindow(GetDialogWindow(win),300,50,0);
		else SizeWindow(GetDialogWindow(win),350,50,0);
...
tryagain:
	...
	if (itemHit == 7) /****** word *****/
	{
	 if (!enc2.wordresult) goto nochoice;
	 getword();
	 sound(1101);
	...
	if (!strcmp(gotword,(StringPtr)"home"))/**** check for home word ****/
	{
                if (indung) saveland(dunglevel);
				 ...
				 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(300);
	...
	if (!strcmp(gotword,(StringPtr)"give item"))/**** check for give item word ****/
	{
           ...
           for(t=0;t<20;t++) treasure.itemid[t] = 0;
	...
	if (!strcmp(gotword,(StringPtr)"change land"))/**** check for change land word ****/
	{
           ...
           if ((field[temp][temp1] < 999) && (field[temp][temp1] > 0)) field[temp][temp1] = temp2;
	...
	if (!strcmp(gotword,(StringPtr)"reset door"))/**** check for Reset Door word ****/
	{
           ...
           loadland(abs(templong)-1,1);
            ...
            temp = longvalue;
				 ...
				 if (templong < 0) getfilename("Data DDD");
				  else getfilename("Data DD");
           ...
           if (temp > 0) saveland(templong-1);
            else saveland(templong-1);
	...
	if (!strcmp(gotword,(StringPtr)"activate door"))/**** check for activate door word ****/
	{
           ...
           if (templong > 0) loadland(templong-1,1);
            else loadland(templong-1,1);
	...
	if (!strcmp(gotword,(StringPtr)"give creature"))/**** check for give creature word ****/
	{
		...
		if (goodword) encsuccess = enc2.wordresult;
		 else encsuccess = 4;
	...
	{
	 if (!enc2.spellid[0]) goto nochoice;
...
resolve:
		...
		if ((spellinfo.special == 65) || (spellinfo.special == 70))
		{
		 ...
		 if (enc2.thief)
		 {
		  if ((fp = MyrFopen(":Data Files:CT","rb")) == NULL) scratch(54);
		  fseek(fp,enc2.thiefsuccess * sizeof thief,SEEK_SET);
		  fread(&thief,sizeof thief,1,fp);
		  CvtThiefToPc(&thief);
		  fclose (fp);
	...
	{
	 if (!enc2.itemid[0]) goto nochoice;
	 	
	 	in();
		itemused = items();
		
		updatemain(FALSE,-1);
		if (!itemused) goto backup;
	...
	{
		if (!enc2.thief) goto nochoice;

		 reply = thiefenc();
  
		 if (!reply) goto backup;
		  else encsuccess = reply;
		goto out;
	}

	if (itemHit == 5)/****** Action *****/
	{
	 if (!enc2.choiceresult) goto nochoice;
		reply = actionpicker();
		updatemain(TRUE,-1);
		if (!reply) goto backup;
		temp = 0;
		...
		for(t=0;t<8;t++) if (enc2.group[t]) temp++;
		if (!temp)
		{
		 encsuccess = 4;
		 goto out;
		...
		for(t=0;t<8;t++)
		{
			if ((actiontaken[t]) && (!enc2.group[t])) encsuccess = 0;
			if ((!actiontaken[t]) && (enc2.group[t])) encsuccess = 0;
		}
		
		if (encsuccess) encsuccess = enc2.choiceresult;
		 else encsuccess = 4; /***** default fail *****/
		goto out;
	}
out:
	SetCCursor(sword);
