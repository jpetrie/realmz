...
keepgoing:
		...
		if (itemHit == 39) goto out;
		
		if (itemHit == 2) reply = gStop = TRUE;
	...
	update:
		 total = disk = temp = 0;
		  ...
		  if (GetControlValue((ControlHandle)itemHandle)) exchange = 1.0;/*** Credit elims X-Rate ****/		 
		 ...
		 if (GetControlValue((ControlHandle)itemHandle)) total += 0;
		 
		 GetDialogItem(regscreen,14,&itemType,&itemHandle,&itemRect);/**** Assault ****/
		 if (GetControlValue((ControlHandle)itemHandle)) total += 13;
		 
		 GetDialogItem(regscreen,56,&itemType,&itemHandle,&itemRect);/**** Divinity ****/
		 if (GetControlValue((ControlHandle)itemHandle)) total += 30;
