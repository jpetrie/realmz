...
void combat(short suprise,short mode)
{
 ...
 if (battlenum < 0) suprise = -1;/****** force suprise battle on party *****/
 
 getfilename("Data BD");
 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(7);
 fseek(fp,abs(battlenum) * sizeof battle,SEEK_SET); 
 fread(&battle,sizeof battle,1,fp); 
 CvtBattleToPc(&battle);
 fclose(fp);
 
 if (battle.messagebefore) textbox(-1,battle.messagebefore,0,0,textrect);
 
 textbox(3,53,0,0,textrect);/**** building map message ***/
 incombat = combatround = 1;
 for(t=0;t<=charnum;t++) if (c[t].stamina < 1) updatepictbox(t,TRUE,0);
 ...
 if (!indung) saveland(landlevel);
  else saveland(dunglevel);
 ...
 combatsetup(0,suprise,mode);
  ...
  while (incombat)
  {
   ...
   if (monster[monsterup].condition[0]) goto startretreat;/**** start retreat ***/
	  ...
	  if (!checkforenemy(1))
	   {
 	   ...
 	   else monster[monsterup].weapon = monster[monsterup].items[0];
 	}
 	 ...
 	 if ((!monster[monsterup].condition[5]) && (!monster[monsterup].condition[29]) && (!monster[monsterup].condition[39]) && (!monster[monsterup].condition[1]) && (monstercasting == 0))
 	  {
        ...
        {
         for(t=0;t<=charnum;t++) if ((c[t].traiter == monster[monsterup].traiter) && (c[t].inbattle)) track[t] = TRUE;
         for(t=0;t<nummon;t++) if ((monster[t].traiter == monster[monsterup].traiter) && (monster[t].stamina > 0)) track[t+10] = TRUE;
         inspell = didcast = TRUE;
         combatupdate2(monsterup+10);
         showspellinfo();
        ...
        if (!targetsleft) goto alloutofrange;
        
        if (!spellinfo.targettype) powerlevel = Rand(targetsleft);
        if (powerlevel > 7) powerlevel = 7;
        ...
        lowerlevel:
         ...
         if (spellinfo.cost * powerlevel > monster[monsterup].spellpoints) 
          {
           powerlevel --;
           if (!powerlevel) goto alloutofrange;
           goto lowerlevel;
          }
         targetnum = powerlevel;
         targetsleft = targetnum;
         
        if (spellinfo.targettype > 0) targetsleft = targetnum = 1;
        
        if (monster[monsterup].size) spellrange ++;
        if (monster[monsterup].size == 3) spellrange ++;
       ...
       if ((spellinfo.targettype < 2) && (range[maxloop+1] <= spellrange))
         {
          targetnum = 0;
             while (targetsleft) 
             {
              ...
              {
               pass = 0;
               if (targetnum) goto gocast;
               goto alloutofrange;
              }
              targetnew = Rand(10+nummon)-1;
              
              if (range[targetnew] > spellrange) goto passagain;//**** is target out of range?
			  ...
			  if (!cansee(fromx,fromy,seex,seey)) goto passagain;//**** is target in LOS?
			  }

              
              for(t=0;t<7;t++) if (target[t][0] == targetnew) goto passagain;

                 target[targetindex][0] = targetnew;
                 if (targetnew < 9) target[targetindex++][1] = 1;
                 else target[targetindex++][1] = 2;
                 targetsleft --;
                 targetnum ++;
              }
            }
        ...
        showtargets(0);
        showspellinfo();
        if (spellinfo.canrotate) rotate = (Rand(4)-1);
 	   ...
 	   if (!monster[monsterup].condition[1]) checkforenemy(0);
 	   
 startmove:
 
 cantseeindex = 0;
 
 if ((numenemy - killmon) <= 0) goto monsterdone;
	   ...
	   if ( monster[monsterup].target < -1) monster[monsterup].target = -1;
 	   temp = monster[monsterup].target;

	   if (temp == -1) goto newtarget;

	   if (temp < 9)
		{
		 if ((!c[temp].inbattle) || (c[temp].traiter == monster[monsterup].traiter)) goto newtarget;
		}
        else if ((monster[temp-10].stamina <1) || (monster[monsterup].traiter == monster[temp-10].traiter))
 	    {
 	     newtarget:
 	    
 	     if (sliding == TRUE) goto newtargetslide;
 	   ...
 	   newtargetslide:
 	     ...
 	     if ((temp > charnum) && (temp < 10))
 	           {
 	            if (sliding) monster[monsterup].target = temp = cantseeindex++;
 	            goto newtarget;
 	           }
 	    ...
 	    if (monster[monsterup].target > 100) monster[monsterup].target = -1;
 	    if (monster[monsterup].target < -1) monster[monsterup].target = -1;
...
startretreat:
 	         ...
 	         if ((nospell < 2) && (monster[monsterup].castpercent) && (!didattack))
 	          {
 	           ...
 	           if (!monster[monsterup].beenattacked)
 	           {
 	            if ((monster[monsterup].condition[5]) || (monster[monsterup].condition[29]) || (monster[monsterup].condition[39]) || (monster[monsterup].condition[1])) goto monsterdone;
 	            goto tryspell;
 	           }
 	          }
 	          goto monsterdone;
 	    ...
 	    if (monster[monsterup].condition[1]) goto monsterdone;
 	    checkforenemy(0);
     	combatupdate(q[up]);
       }
       
        if (monster[monsterup].condition[0]) checkforenemy(0);/***** done retreating ***/
 	         if ((nospell < 2) && (monster[monsterup].castpercent) && (!didattack))
 	          {
 	           nospell = 2;
 	           monster[monsterup].movement = 0;
 	           if (!monster[monsterup].beenattacked) goto tryspell;
 	          }
        monsterdone:
        sliding = cantseeindex = FALSE;/**** reset sliding monster stats ****/
 	    ...
 	    if (((poss) || (c[charup].condition[25])) && (q[up] < 9)) goto jumpposs;
...
// Fantasoft v7.0 End
		     ...
		     case 'e':/********** e ** escape *******************/
		           ...
		           {
		            if ((c[charup].condition[1]) || (c[charup].traiter) || (c[charup].condition[29]) || (c[charup].condition[2]) || (c[charup].condition[6]))
		            {/***** bad condition ****/
		             warn(83);
		             upbutton(TRUE);
		            }
		     ...
		     if (c[charup].movement == c[charup].movementmax)
		     {
		           ...
		           {
		            q[ttt] = q[ttt+1];
		            if ((q[ttt] <= charnum) && (q[ttt] > -1)) c[q[ttt]].position --;
		           }
		           q[maxloopminus] = temp;
		    ...
		    if (whichset)/********* fast spells *********/
		    {
		    	if (charup < 6)
		    	{
				 ...
				 {
					 if (abs(spellinfo.cost*powerlevel) >= c[charup].spellpoints) theControl = castspellsbut;
					 else
					 {
		
						if ((incombat) && (!spellinfo.incombat)) sound(143);
							else if ((!incombat) && (!spellinfo.incamp)) sound(143);
								else if (!powerlevel) sound(143);
								  else
									{
		    ...
		    {
		     if (killparty > charnum) partyloss(0);
		     combatchoice();
		     if (c[charup].attacks  < 2) getup(FALSE);
		    }
		    break;
			  ...
			  case  inContent:
			      if ((q[up] < 9) && (q[up] > -1))
			      {
		     	  ...
		     	  if (PtInRect(point,&buttons))
				     ...
				     if (PtInRect(point,&buttonrect)) theControl = turn;
				     ...
				     /************ g ** guard ******************/
		     	     ...
		     	     buttonrect.left = 364+leftshift;
		     	     buttonrect.right = buttonrect.left + 49;
				     if (PtInRect(point,&buttonrect)) key = 'g';
				     
				     /************* f *** finish ****************/
		     		 buttonrect.left = 470+leftshift;
		     		 buttonrect.right = buttonrect.left + 50 + downshift;
				     if (PtInRect(point,&buttonrect)) key = 'f';
				     				     
				     /********** d ** delay *********************/
		     	     buttonrect.left = 417+leftshift;
		     	   	 buttonrect.right = buttonrect.left + 49;
				     if (PtInRect(point,&buttonrect)) key = 'd';
				     ...
				     /************ r ** reveal friendly ***********/
		     		 ...
		     		 buttonrect.left = 364+leftshift;
		     		 buttonrect.right = buttonrect.left + 102;
				     if (PtInRect(point,&buttonrect)) key = 'r';
				     ...
				     /************ b ** bandage ******************/
		     	     ...
		     	     buttonrect.left = 364+leftshift;
		     	     buttonrect.right = buttonrect.left + 60;
				     if (PtInRect(point,&buttonrect)) key = 'b';
				     ...
				     /************ e ** escape ******************/
		     	     buttonrect.left = 364+leftshift;
		     	     buttonrect.right = buttonrect.left + 54;
				     if (PtInRect(point,&buttonrect)) key = 'e';
				     
				     /************ u ** undo ******************/
		     	     buttonrect.left = 422+leftshift;
		     	     buttonrect.right = buttonrect.left + 44;
				     if (PtInRect(point,&buttonrect)) key = 'u';
				     ...
				     /************ t ** target ******************/
			     	 ...
			     	 buttonrect.left = 522+leftshift;
			     	 buttonrect.right = buttonrect.left + 44;
				     if (PtInRect(point,&buttonrect)) key = 't';
				     ...
				     /************ w ** weapon ******************/
			     	 ...
			     	 buttonrect.left = 522+leftshift;
			     	 buttonrect.right = buttonrect.left + 44;
				     if (PtInRect(point,&buttonrect)) key = 'w';
	                ...
	                if (c[charup].attacks  < 2) getup(FALSE);
	                 else if (!inwindow(charup)) centerstage(0);
	               }
			        break;
		     }
		      break;
