...
void combatupdate(short body) 
{
 ...
 EraseRect(&box);
 if (temp < tempmax) ForeColor(whiteColor);
 ...
 box.bottom = box.top + 15;
 EraseRect(&box);
 if (tempstamina < tempstaminamax) ForeColor(whiteColor);
  else ForeColor(yellowColor);
...
void combatupdate2(short body)
{
	...
	else if (!inspell)
	{
	 ...
	 if (body < 9)
	 {
	  if (!cansee(pos[charup][0],pos[charup][1],pos[body][0],pos[body][1])) MyrDrawCString(" Blocked");
	 }
	 else
	 {
	  if (!cansee(pos[charup][0],pos[charup][1],monpos[body-10][0],monpos[body-10][1])) MyrDrawCString(" Blocked");
	 }
	}
	...
	if (screensize)
	{
		...
		EraseRect(&iconrect);
	     ...
	     itemRect.bottom = itemRect.top +9;
			...
			{
				if (monster[body-10].spellimmune[t]) pict(150,itemRect);
					else pict(146,itemRect);
	     ...
	     itemRect.bottom = itemRect.top +9;
			...
			for(t=0;t<5;t++)//**** vuls
			{
				if (monster[body-10].save[t] < 0) pict(150,itemRect);
				  else pict(146,itemRect);
	...
	if (body < 10)
	{
	 ...
	 if (body == q[up])
	 {
      ...
      if (!c[body].toggle) loaditem(c[body].armor[2]);
       else loaditem(c[body].armor[15]);
       
	    if (item.sp2 > 1100)
	    {
		 for(t=0;t<c[body].numitems;t++)
		 {
		  if (c[body].items[t].id == item.itemid) break;
	     }
	...
	else
	   {
		...
		if (monster[body-10].lr == 1) temp += 308;
	...
	if (body <10)
	{
		string(c[body].ac);
		MoveTo (169,341+downshift);
		if (c[body].stamina < c[body].staminamax) ForeColor(whiteColor);
		string(c[body].stamina);
		MoveTo (173,390+downshift);
		if (c[body].spellpoints < c[body].spellpointsmax) ForeColor(whiteColor);
		 else ForeColor(yellowColor);
	...
	else
	{
		...
		MoveTo (169,341+downshift);
		if (monster[body-10].stamina < monster[body-10].staminamax) ForeColor(whiteColor);
		string(monster[body-10].stamina);
		MoveTo (173,390+downshift);
		if (monster[body-10].spellpoints < monster[body-10].maxspellpoints) ForeColor(whiteColor);
		 else ForeColor(yellowColor);
	...
	if (body < 10)
	{
	 ...
	 {
	  if (c[body].toggle) temp = c[body].armor[15];
	   else temp = c[body].armor[2];
	   
			for(t=0;t<c[body].numitems;t++)
			{
			 if (c[body].items[t].id == temp) break;
			}

				if (c[body].items[t].charge < 0) MyrDrawCString("--");
				 else string(c[body].items[t].charge);
	...
	{
		if (monster[body-10].weapon) MyrDrawCString("??");
		 else MyrDrawCString("--");
	}
	...
	for(t=0;t<40;t++)
	{
		if (body < 10)
		{
		 if (c[body].condition[t]) goto updateit;
		}
