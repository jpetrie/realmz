...
void change(short play,short mode)
{
	...
	if (c[cl].load < 0) c[cl].load = 0;
	characterl = c[cl];

	if (cr != -1)
	{
	 if (c[cr].load < 0) c[cr].load = 0;
	 characterr = c[cr];
    }
		
	RGBBackColor(&greycolor);
	
	if (mode) displaytag = TRUE;
	...
	if (theControl == rightarrow)
	{
		...
		if (cr > -1)
		{
			...
			if (characterr.stamina < 1) ploticon3(2019,shade);
			
			if (characterr.stamina < -9) ploticon3(2015,chr);
	...
	else
	{
		...
		if (characterl.stamina < 1) ploticon3(2019,shade);
		
		if (characterl.stamina < -9) ploticon3(2015,chr);
 ...
 if ((first == FALSE) || (theControl == leftarrow))
	{
		if (characterl.numitems > 9) SetControlMaximum(charitemsvert,characterl.numitems-9);
