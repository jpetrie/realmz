...
void itemcost(short mode) /****** 1 = sell, else buy ****/
{
 ...
 if (!mode) newcost *= theshop.inflation;
  else
  {
   if (theshop.inflation > 100) newcost *= 100;
    else newcost *= theshop.inflation;
  }
  
 newcost /= 100;
 if (newcost > 32000) newcost = 32000;
...
if (theControl != charitemsvert) theControl = shopitemsvert;
...
leftvalue = GetControlValue(charitemsvert);
	...
	if (point.h <= (320+(leftshift/2))) 
	{
	 ...
	 lg = characterl.items[gshopitem+GetControlValue(charitemsvert)-1].ident;
		...
		if (((acceptlowrange1) || (acceptlowrange2)) && (cr == -1))//** shop only deals with specific items.
		{
		 ...
		 if ((!twixt(tempid,acceptlowrange1,accepthighrange1) && (acceptlowrange1))) baditem++;
		 if ((!twixt(tempid,acceptlowrange2,accepthighrange2) && (acceptlowrange2))) baditem++;
	...
	{
	 if (cr >-1) item.charge = characterr.items[gshopitem+GetControlValue(shopitemsvert)-1].charge;
	}
	else item.charge = charge1;
	...
	if ((point.h > (320 + (leftshift/2))) && (cr == -1))
	{
	 ...
	 string(item.cost);
	 ...
	 else if (shopavail)
	    {
	        ...
	        if (item.charge > 0) charge1 = item.charge;
	          else charge1 = 0;
	        ...
	        if (cr == -1)
	        {
			 ...
			 itemcost(1);
			 if (!lg) item.cost = item.cost/50;/*** unidentified item ****/
			 string(item.cost);
			}
			 else item.cost = 0;
	    }
    ...
    TextSize(13);
 	...
 	if (item.iscurse)
 	{
 	 ...
 	 if ((cr == -1) && (point.h > (320 +(leftshift/2)))) GetIndString(myString,temp+lg,item.itemid-temp+1);
 	}
    ...
    TextSize(16);
 	...
 	if (point.h <= (320+(leftshift/2)))
 	{
 	 if (cr != -1)
 	 {
	  if (!canuse(-1,characterr.race,characterr.caste)) prob = TRUE;
	 }
	}
	else
	{
	 if (!canuse(-1,characterl.race,characterl.caste)) prob = TRUE;
	}
    ...
    if ((theshop.inflation == 0) && (item.charge > -1) && (cr == -1) && (point.h <= (320 + (leftshift/2))))/********* warhouse takes no charged weapons ****/
 	...
 	if (BitAnd(gTheEvent.modifiers,shiftKey))/***** shift click mode ****/
 	{
 	 ...
 	 if (point.h <= 320+(leftshift/2)) point.h = 500;
 	  else point.h = 35;
 	 goto fastmove;
 	}
 ...
 fastmove:
	...
	if (cr == -1) eraseshopstats(1);
	 else eraseshopstats(0);
...
if(((point.h <= (320 +(leftshift/2))) && (((characterl.numitems == 30) && (!ischar)) || ((characterl.load+item.wieght+(item.xcharge*item.charge)>characterl.loadmax) && (!ischar)))) || (((point.h > (320 +(leftshift/2))) && (cr!=-1)) && ((characterr.numitems == 30) || (characterr.load+item.wieght+(item.xcharge*item.charge)>characterr.loadmax))))
 {
 	...
 	return;
 	 ...
 	 if ((point.h > (320+(leftshift/2))) && (ischar) && (cr > -1)) /*******start move item stuff *********/
 	 {
 	  if (abs(item.type) == 13)
 	   {
		  for(t=0;t<characterr.numitems;t++) if (twixt(characterr.items[t].id,800,802))
		  {
		   twoscrolls:
		   ShowCursor();
		   warn(32);
 ...
 if (!truejoin)
 {
 	if (valu < max) 
 	{
 	  for (t=characterr.numitems; t>valu+9; t--) characterr.items[t] = characterr.items[t-1];
 	  temp = valu+9;
 	 }
 	...
 	characterr.items[temp].equip = FALSE;
 	  ...
 	  if (characterr.numitems < 9) 
 		...
 		characterr.numitems++;
 		updateshopwings(-2, -2);
 	  if (characterr.numitems > 9) SetControlMaximum(shopitemsvert,characterr.numitems-9);
 	  {
		theCode = kControlDownButtonPart;
		if (valu < max) skip = FALSE;
 	...
 	if ((point.h < (250+(leftshift/2))) && (!ischar))
 	 {
 	  if (abs(item.type) == 13)
 	   {
		for(t=0;t<characterl.numitems;t++) if (twixt(characterl.items[t].id,800,802)) goto twoscrolls;
		   for(t=0;t<5;t++) 
		   {
 ...
 if (!truejoin)
 {
 	if (valu < max)
 	  {
 	    for (t=characterl.numitems; t>valu+9; t--) characterl.items[t] = characterl.items[t-1];
 	    temp = valu+9;
 	  }
  }
 	 ...
 	 if (!truejoin) characterl.items[temp].equip = FALSE;
 	 if (!truejoin) characterl.numitems++;
 	 updateshopwings(-2, -2);
     if (cr == -1)
     {
 	 ...
 	 else characterl.items[temp] = characterr.items[GetControlValue(shopitemsvert)+gshopitem-1];
 ...
 if ((point.h < (250+(leftshift/2))) && (!ischar) && (cr != -1))
 	  { 
 	   ...
 	   movecalc(-1);
 	   movecalc(-2);
 	   for (t=GetControlValue(shopitemsvert)+gshopitem-1;t<characterr.numitems;t++) characterr.items[t] = characterr.items[t+1];

 	     theControl = shopitemsvert;
 	     max = GetControlMaximum(shopitemsvert);
 	     value = GetControlValue(shopitemsvert);
 	     if (max) SetControlMaximum(shopitemsvert,max-1);
 	     characterr.numitems--;
 	     updateshopwings(-2, -2);
 	     skiptest = TRUE;
 	     
 	     if (characterr.numitems - value > 9) displaytag = 2;
 	     Display(0);
 	     displaytag = FALSE;
 	     skiptest = FALSE;
 	    }
 	...
 	{ 
 	       ...
 	       if (theshop.id[t] == tempid)
 	        {
 	         ...
 	         {
 	          if (!theshop.num[t]) doupdateshop = TRUE;
 	         }
 	         theshop.num[t]++;
 	         goto bustout;
 	        }
 	 ...
 	 for (t=GetControlValue(charitemsvert)+gshopitem-1;t<characterl.numitems;t++) characterl.items[t] = characterl.items[t+1];

 	     movecalc(-1);
 	     theControl = charitemsvert;
 	     max = GetControlMaximum(charitemsvert);
 	     value = GetControlValue(charitemsvert);
 	     if (max) SetControlMaximum(charitemsvert,max-1);
 	     characterl.numitems--;
 	     updateshopwings(-2, -2);
 	     if (doupdateshop) skip = TRUE;
 	     if (characterl.numitems - value > 9) displaytag = 2;
 	     Display(0);
 	     displaytag = FALSE;
 	    }
