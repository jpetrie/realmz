...
{
	if (FrontWindow() == look)  updatemain(FALSE,0);
...
void DoFreeBeforeQuit(void)
{
	...
	if (charnum > -1)
	{
	for(t=0;t<=charnum;t++) savecharacter(t);
	}
	...
	if (!nofade && !nologo) fadeinout(25, fadeout);

	for(t=0;t<60;t++)
	{
	if (FrontWindow() != NIL) DisposeWindow(FrontWindow());
	}
	...
	if (!nofade && !nologo) fadeinout(25, fadein);
...
Rect mainrect;
					...
					switch(theItem)
					{
					 case 1:
					    if (FrontWindow() != GetDialogWindow(background))
					    {
						 	reply = indung;
						 	warp();
						 	if ((indung) && (!reply)) threed(dunglevel,floorx,floory,1);
						 	updatemain(FALSE,0);
						}
					 break;
					 ...
					 case 3:
						  ...
						  for(t=0;t<=charnum;t++)
						  {
						   c[t].money[0] += 200;
						   c[t].load += 200;
						   if (c[t].spellpointsmax) c[t].spellpointsmax = c[t].spellpoints = 400;
						   for(ttt=0;ttt<7;ttt++)
						   {
						    c[t].nspells[ttt] = 0;
						    for(tt=0;tt<12;tt++) c[t].cspells[ttt][tt] = TRUE;
						   }
						  }
					 ...
					 case 6:
					 	...
					 	for(t=0;t<maxmon;t++)
					 	{
					 	 monster[t].stamina = 1;
					 	 monster[t].traiter = 0;
					 	 for(tt=0;tt<40;tt++) monster[t].condition[tt] = 0;
					 	}
	
					 break;
				 
				 case 7:
				 		for(t=0;t<=charnum;t++) c[t].exp = 1;
				 break;
				}
			  }
             }
			...
			case  136:/***************** character menu ****************/
				switch(theItem)
				{
					...
					case 2:
						...
						if ((indung) && (viewtype == 1)) UpdateWindow(FALSE);
					break;
		...
		updatescreen:
				...
				if (charnum > -1) updatemain(FALSE,-1);
				else DrawDialog(background);
				if (incombat)
				{
					centerstage(0);
		...
		updateaftermovie:
						...
						else if (FrontWindow() == look)  updatemain(FALSE,0);
				 		 else if ((indung) && (viewtype == 1))
				 		{
				 		 updatemain(FALSE,0);
				 		 UpdateWindow(FALSE);
				 		}
				 		else if (FrontWindow() == GetDialogWindow(background)) DrawDialog(background);
			break;
				...
				{				
				 ...
				 case 5:/****** Edit Spell Names *********/
						...
						if (incombat)
						 ...
						 else if (FrontWindow() != GetDialogWindow(background)) updatemain(FALSE,-1);
				 break;
			...
			case 129: /************** file menu **************/
				switch(theItem)
				{
					...
					playsaved:
						...
						if (!revertgame) mainscreeninit(0,0);
						 else return(0);
					break;
				}
				break;
			...
			case 200:/**** maps *****/
				...
				else if (theItem == 1)/********* notes *************/
				{
				 ...
				 if (indung)
				 {
				 	 if ((fp = MyrFopen(":Data Files:DN","r+b")) == NULL) scratch(61);
				  noteid  = 10000 * dunglevel + floorx*90 + floory;
				 }
				 	else 
				 	{
				 	 if ((fp = MyrFopen(":Data Files:WN","r+b")) == NULL) scratch(62);
				 ...
				 do
				 {
			 	  ...
			 	  if (notenum > 500) goto newnote;
			 	   
			 	  if (!fread(&note,sizeof note,1,fp)) goto newnote;
			 	  CvtNoteToPc(&note);
			 	   ...
			 	   if (!MyrBitTstShort((void *)(&field[partyx + lookx][partyy +looky]),1))
			 	   {
				 	...
				 	if (indung)
					 {
					  ...
					  else
					  {
					   ...
					   if (randlevel.isdark) note.wasdark = partycondition[0]/30+1;
					    else note.wasdark = 0;
					   note.x = lookx + partyx;
					   note.y = looky + partyy;
					  }
		...
		newnote:
				 ...
				 if (indung)
				 {
				  if ((fp = MyrFopen(":Data Files:DN","a+b")) == NULL) scratch(63);
				 }
				 	else
				 	{
				 	 if ((fp = MyrFopen(":Data Files:WN","a+b")) == NULL) scratch(64);
				 	}
		...
		gotnote:
				...
				if (note.wasdark) loaddark(note.wasdark);
				 else loaddark(0);
				...
				while ((itemHit != 1) && (itemHit != 3))
				{
				 ...
				 if (indung)
				 {
				  if ((fp = MyrFopen(":Data Files:DN","r+b")) == NULL) scratch(63);
				 }
				 	else
				 	{
				 	 if ((fp = MyrFopen(":Data Files:WN","r+b")) == NULL) scratch(64);
				 	}
				   ...
				   CvtNoteToPc(&note);
				   if (!fwrite(&note,sizeof note,1,fp)) scratch(65);
				   CvtNoteToPc(&note);
				  }
				  
				  notenum--;
				  if (itemHit == 5) notenum +=2;
			...
			backupabit:
				 ...
				 if (randlevel.isdark) loaddark(partycondition[0]/30+1);
				...
				if (notenum == orignoteid)/**** save note only if on original ****/
				{
				 ...
				 GetDialogItemText(itemHandle,note.string);
				 	 ...
				 	 if (indung)
				 	 {
				 	  if (StringWidth(note.string)) MyrBitSetShort(&field[floory][floorx],10);
				 	   else MyrBitClrShort(&field[floory][floorx],10);
				 	 }
				 	  else
				 	  {
				 	  if (StringWidth(note.string)) MyrBitSetShort(&field[partyx+lookx][partyy+looky],1);
				 	   else MyrBitClrShort(&field[partyx+lookx][partyy+looky],1);
				 	  }
				 ...
				 if (!StringWidth((StringPtr)note.string)) note.id = 0;
	...
	blownotes2:
		...
		backup:
				...
				update:
					  ...
					  if (journalindex2 < 1) journalindex2 = 1;
					  if (journalindex2 > 5999) journalindex2 = 5999;//** fantasoft v7.1
					  ...
					  if ((journalindex2 == 1) || (journalindex2 == 5999))//** fantasoft v7.1
					  {
					   ...
					   else if ((itemHit == 4) || (itemHit == 5)) sound(141);
			...
			case 130:
				switch(theItem) /**************** game menu ***********/
				{
					case 1:/******************** start new game ************/
						...
						if (currentscenario == 10) reply = FALSE;
						...
						for(t=0;t<=charnum;t++)
						{
						 if (c[t].stamina <1) killparty++;
						 for(tt=0;tt<12;tt++)
							{
							 lastspell[t][0] = 0;
							 if (c[t].cspells[0][tt]) lastspell[t][1] = tt;
							}
						 }
					...
					case 3:/*********** end current game ***************/
					  if (!incombat)
					  {
					   if (question(1)) save(0);
					  }
					    else if(!question(10)) return(0);
