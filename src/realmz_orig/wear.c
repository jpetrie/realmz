...
short wear(short character,short itemnum,short play)
{
 ...
 if ((incombat) && (character != q[up])  && (play))
 {
  ...
  if ((incombat) && (c[character].condition[25])) play = FALSE;
			 ...
			 if (item.type > 1)
			 {
			  ...
			  for(t=0;t<= c[character].numitems;t++)
			  {
			   ...
			   if ((abs(item.type) == holdtype) && (c[character].items[t].equip)) correct = TRUE;
			  }
				   ...
				   else
				   {
				    ...
				    if (item.nohands) c[character].nohands = 0;
				   }
			    }
			 }
  ...
  switch(item.type)
  {
   case 2:/***************** melee weapon ****************/
		   if ((c[character].nohands <2 ) && (!c[character].armor[2]))
		   {
		      ...
		      if (c[character].items[itemnum].ident) c[character].weaponnum = 1;
		       else c[character].weaponnum = 0;
		      reply = TRUE;
		   ...
		   if (!c[character].armor[15])
		    {
		     if (MyrBitTstLong(&item.itemcat[0],12))/**** look for normal bow and require quiver ****/
		     {
		      ...
		      if (c[character].items[itemnum].ident) c[character].missilenum = 1;
		       else c[character].missilenum = 0;
   ...
   case 0: /*******   Ring  **************/
	   ...
	   {
	      reply = TRUE;
	      if (c[character].armor[0] == 0) c[character].armor[0] = item.itemid;/*** rings *****/
	       else c[character].armor[1] = item.itemid;
   ...
   default:
   		if (item.type < 20) c[character].armor[item.type] = item.itemid;
   		reply = TRUE;
   break;
 }
   ...
   if (reply)
   {
       ...
       if (c[character].ac + item.ac < 0) c[character].ac = 0;
        else c[character].ac += item.ac;
       ...
       c[character].damage += item.damage;
   	   if (c[character].damage > 110) c[character].damage = 110;
       c[character].movebonus += item.movement;
       
       movecalc(character);
       
       if ((item.sp1 >59) && (item.sp1 < 100)) c[character].condition[item.sp2] = 0;/**** neutralize condition ***/
       ...
       {
        if (c[character].condition[item.sp1-20] > -1) c[character].condition[item.sp1-20] = 0;
        c[character].condition[item.sp1-20] += item.sp2;/**** make condition[sp1-20] = sp2 ***/
       }
       
       if (item.sp3)/******* adds special ability *****/
       {
        if (item.sp3 < 0) c[character].special[abs(item.sp3) -1] += item.sp5;
         else if ((item.sp3 < 16) && (item.sp3 > 0)) c[character].spec[item.sp3-1] += item.sp5;
          else partycondition[item.sp3-30] -= abs(item.sp5);
       }
       
       if (item.sp4)
       {
        if (item.sp4 < 0) c[character].special[abs(item.sp4) -1] += item.sp5;
         else if ((item.sp4 < 16) && (item.sp4 > 0)) c[character].spec[item.sp4-1] += item.sp5;
          else partycondition[item.sp4-30] -= abs(item.sp5);
       }
    }
    
   		if (reply) if ((item.sound) && (play)) sound(600+item.sound);
 out:
