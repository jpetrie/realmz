...
short load(void)
{
 ...
 {
  if (!incombat) centerpict();
   else centerfield(5+(2*screensize),5+screensize);
  flashmessage((StringPtr)"Reverting to a previously saved game, please wait...",25,350+downshift,-1,0);
  showing = TRUE;
  partyloss(TRUE);
 }
  
 if (currentscenario < 7) currentscenario = 10;
...
#if !divine
 	 if (testlocation != (partyx + partyy + lookx + looky + floorx + floory + landlevel)) scratch2(6);
#endif
...
MyrCheckMemory(2);
	 ...
	 {
	  for(t=0;t<=charnum;t++) if (c[t].level > 7) scratch(456);
	 }
 #endif
 ...
 needtoregister:
  ...
  if (showing) flashmessage((StringPtr)"",10,10,-1,0);
  warn(65);
  return(0);
 }
...
if (currentscenario < topfantasoftsceanrio)// Fantasoft v7.1  changed it to use topfantasfotscenario varialbe
 ...
 if (picturehandle)
 {
  ...
  if (look == NIL) MoveWindow(splash ,GlobalLeft+160,GlobalTop+70,FALSE);
   else MoveWindow(splash ,GlobalLeft+(leftshift/2),GlobalTop+(downshift/2),FALSE);
 ...
 killparty = 0;
 for(t=0;t<= charnum;t++) if ((c[t].stamina < 1) || (c[t].condition[25])) killparty++;
 
 if (partycondition[0]) loaddark((partycondition[0]/30) +1);
  else loaddark(0);
  	
  	strcpy((StringPtr)filename,(StringPtr)hold);/******************** thief ******************/
	strcat((StringPtr)filename,"Data H1");
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(66);
	op = NIL;
	if ((op = MyrFopen(":Data Files:CT","w+b")) == NULL) scratch(67);
	
	while (fread(&thief,sizeof thief,1,fp) == 1)
	{
	 if (!fwrite(&thief,sizeof thief,1,op)) scratch(68);
	}
	fclose (op);
	fclose (fp);
	strcpy((StringPtr)filename,(StringPtr)hold);/******************** time encounters ******************/
	strcat((StringPtr)filename,"Data TD3");
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(69);
	if ((op = MyrFopen(":Data Files:CTD3","w+b")) == NULL) scratch(70);
	
	while (fread(&dotime,sizeof dotime,1,fp) == 1)
	{
	 if (!fwrite(&dotime,sizeof dotime,1,op)) scratch(71);
	}
	fclose (op);
	fclose (fp);
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(72);
	if ((op = MyrFopen(":Data Files:CE","w+b")) == NULL) scratch(73);
	
	while (fread(&enc,sizeof enc,1,fp) == 1)
	{
	 for(t=0;t<4;t++) fread(&buffer[t], 80,1,fp);
	 if (!fwrite(&enc,sizeof enc,1,op)) scratch(74);
	 for(t=0;t<4;t++) if (!fwrite(&buffer[t], 80,1,op)) scratch(75);
	}
	fclose (op);
	fclose (fp);
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(76);
	if ((op = MyrFopen(":Data Files:CE2","w+b")) == NULL) scratch(77);
	
	while (fread(&enc2,sizeof enc2,1,fp) == 1)
	{
	 for(t=0;t<9;t++) fread(&buffer[t], 40,1,fp);
	 if (!fwrite(&enc2,sizeof enc2,1,op)) scratch(78);
	 for(t=0;t<9;t++) if (!fwrite(&buffer[t], 40,1,op)) scratch(79);
	}
	fclose (op);
	fclose (fp);
 ...
 /**************************** Notes ******************/
	...
	if ((op = MyrFopen(":Data Files:DN","w+b")) == NULL) scratch(81);
	
	while (fread(&note,sizeof note,1,fp) == 1)
	{
	 if (!fwrite(&note,sizeof note,1,op)) scratch(82);
	}
	fclose (op);
	fclose (fp);
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(83);
	if ((op = MyrFopen(":Data Files:WN","w+b")) == NULL) scratch(84);
	
	while (fread(&note,sizeof note,1,fp) == 1)
	{
	 if (!fwrite(&note,sizeof note,1,op)) scratch(85);
	}
	fclose (op);
	fclose (fp);
...
skipnotes:
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(86);
	if ((op = MyrFopen(":Data Files:CD","w+b")) == NULL) scratch(87);
	while ((fread(&door,sizeof door,1,fp) == 1) && (count < 20))
	{
		...
		if (!fwrite(&door,sizeof door,1,op)) scratch(88);
		if (!fwrite(&field,sizeof field,1,op)) scratch(89);
		if (!fwrite(&randlevel,sizeof randlevel,1,op)) scratch(90);
	}
	fclose (fp);
	fclose (op);
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(91);
	if ((op =MyrFopen(":Data Files:CL","w+b")) == NULL) scratch(92);
	while ((fread(&door,sizeof door,1,fp) == 1) && (count < 20))
	{
		count++;
		...
		if (!fwrite(&door,sizeof door,1,op)) scratch(93);
		if (!fwrite(&field,sizeof field,1,op)) scratch(94);
		if (!fwrite(&randlevel,sizeof randlevel,1,op)) scratch(95);
		if (!fwrite(&site,sizeof site,1,op)) scratch(27);
	}
	fclose (fp);
	fclose (op);
...
/************ restore saved shop ***************/
 ...
 strcat((StringPtr)filename,"Data E1");
  ...
  if ((op = MyrFopen(":Data Files:CS","w+b")) == NULL) scratch(96);
  if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(97);
  
  while(fread(&theshop,sizeof theshop,1,fp))
   {
    if (!fwrite(&theshop,sizeof theshop,1,op)) scratch(98);
   }
  fclose (fp);
  fclose (op);
...
charselectold = charselectnew = 0;
  ...
  if (indung) loadland(dunglevel,1);
   else loadland(landlevel,1);
   
  if (shopavail) loadshop(0);
...
if (currentscenario > topfantasoftsceanrio)// Fantasoft v7.1  changed it to use topfantasfotscenario variable
{
 ...
 if (look == NIL)
 {
  ...
  if ((showing) && (background)) flashmessage((StringPtr)"",10,10,-1,0);
 }
 else if (showing) flashmessage((StringPtr)"",25,350,-1,0);
...
#if !development
 #if divine
 if (!usercheck2()) DisableItem(gOptions,0);
  else EnableItem(gOptions,0);
 #endif
#endif
 return(1);
}
...
void loadshop(short mode)
{
 ...
 if (mode) currentshop = mode;
 
 if ((fp = MyrFopen(":Data Files:CS","rb")) == NULL) scratch(99);
 fseek(fp,currentshop * sizeof theshop,SEEK_SET);
 fread(&theshop,sizeof theshop,1,fp);
 CvtShopToPc(&theshop);

 fclose (fp);
 if (!mode) shopavail = TRUE;
 if ((look) && (!mode))
 {
  updatecontrols();
  sound(30005);
...
void saveshop(short mode,char filename[256])
 ...
 {
  if ((fp = MyrFopen(":Data Files:CS","r+b")) == NULL) scratch(100);
  fseek(fp,currentshop * sizeof theshop,SEEK_SET);
  CvtShopToPc(&theshop);
 ...
 if (mode ==2) /*********** save game ******************/
 {
  if ((fp = MyrFopen(":Data Files:CS","rb")) == NULL) scratch(101);
  if ((op = MyrFopen(filename,"w+b")) == NULL) scratch(102);
  while(fread(&theshop,sizeof theshop,1,fp))
   {
    if (!fwrite(&theshop,sizeof theshop,1,op)) scratch(103);
   }
  fclose (fp);
  fclose (op);
