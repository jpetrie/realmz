...
void beast(short who,short mode,short specific)
{
	...
	if (mode == 2) pict(154,itemRect);
		else pict(142,itemRect);
	...
	if (mode == 1)
	{
		 ...
		 if (specific) fseek(fp,specific * sizeof monpick,SEEK_SET);
		  else fseek(fp,(menupos[who] -1)* sizeof monpick,SEEK_SET);
	...
	else monpick = monster[who-10];

	if (mode)
	{
	 for(t=29;t<42;t++) MyrCDiStr(t,(StringPtr)"");
	 for(t=44;t<49;t++) MyrCDiStr(t,(StringPtr)"");
	}
...
backup:
	...
	if (mode == 1)
	{
	 getfilename("Data DES");
	 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(5);
	 if (specific) fseek(fp,specific * 256,SEEK_SET);
	  else fseek(fp,(menupos[index] -1) * 256,SEEK_SET);
	 fread(&des,256,1,fp);
	 fclose(fp);
	...
	if (monpick.size < 2) InsetRect(&itemRect,16,0);
	if ((monpick.size == 0) || (monpick.size == 2)) InsetRect(&itemRect,0,16);
	...
	if (monpick.weapon > 0)
	{
	 ...
	 for(t=11;t<23;t++) MyrCDiStr(t,(StringPtr)"");
	...
	if (mode)
	{
		...
		for(t=0;t<6;t++)
		{
			if (monpick.type[t]) DrawPicture(on,&itemRect);
				else DrawPicture(non,&itemRect);
		...
		for(t=0;t<6;t++)
		{
			if (monpick.spellimmune[t]) DrawPicture(on,&itemRect);
				else DrawPicture(non,&itemRect);
		...
		for(t=0;t<5;t++)
		{
			if (monpick.save[t] < 0) DrawPicture(on,&itemRect);
				else DrawPicture(non,&itemRect);
	...
	for (;;)
	{
		...
		if (mode != 1) goto out;
		if ((itemHit == 1) || (itemHit == 4)) goto out;
		if ((itemHit == 2) || (itemHit == 3))
		{
		 sound(141);
			ploticon3(135,buttonrect);
			if (itemHit == 3) index+=2;
			index--;
			...
			if (specific) fseek(fp,specific * sizeof monpick,SEEK_SET);
			 else fseek(fp,(menupos[index] -1) * sizeof monpick,SEEK_SET);
