...
void showageupdate(short who,short agegroup, short backup)
{
 ...
 short tt;
  ...
  if (backup) for(tt=0;tt<15;tt++) DialogNumNZ(tt+17,-(races.agechange[agegroup][tt]));
   else for(tt=0;tt<15;tt++) DialogNumNZ(tt+17,races.agechange[agegroup-1][tt]);
...
void applyage(short raceid, short agegroup, short direction)
{
 ...
 if (characterl.movementmax < 2) characterl.movementmax = 2;

 for (t=0;t<7;t++) characterl.save[t] += direction * races.agechange[agegroup-1][8+t];
}
