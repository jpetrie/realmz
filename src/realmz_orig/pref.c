...
short createpref( FSSpec pref_FSSpec, short mode )
{
	...
	if (!mode) FSpCreateResFile( &pref_FSSpec, 'RLMZ', 'PREF', smSystemScript );
		else FSpCreateResFile( &pref_FSSpec, 'MACS', 'pref', smSystemScript );
...
short openpref(short mode)
{
  ...
  if (mode == 10)/**** Erase old Realmz Prefs. ****/
  {
   ...
   if (( file_ref_num == -1 ) && ((!mode) || (mode == 3))) file_ref_num = createpref( pref_FSSpec, mode );
   
   if (mode) return (dummyrefnum);
...
void preference(void)
{
 ...
 if ((background != NIL) && ((FrontWindow() == look) || (FrontWindow() == gWindow))  && (look != NIL)) in();
...
backup:
		...
		if (usedefaultfont) defaultfont = ID_FONT_DEFAULT;
		 else defaultfont = 3;
	...
	for(;;)
	{
		...
		GetDialogItem(preferwindow,itemHit,&itemType,&itemHandle,&itemRect);
		 ...
		 if (itemHit < 3)
		 {
		  ...
		  if (FrontWindow() == GetDialogWindow(background)) DrawDialog(background);
		   else updatemain(FALSE,-1);
		  goto out;
		 }
			 ...
			 {
			  if (GetControlValue((ControlHandle)itemHandle)) SetControlValue((ControlHandle)itemHandle,FALSE);
			    else SetControlValue((ControlHandle)itemHandle,TRUE);
