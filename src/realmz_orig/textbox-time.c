...
void textbox(short class,short index,short click,short different, Rect newrect)
{
 ...
 if (class == -1)
  {
	...
	if ((fp = MyrFopen(filename,"r")) == NULL) scratch(200);
 ...
 EraseRect(&userect);
 if (!different) userect.bottom = 333;
#ifndef PC // Myriad
 TEDelete(messagetext);
 #endif
 if (!different) userect.bottom = 448+downshift;
 TEUpdate(&userect,messagetext);
 TEDispose(messagetext); 
 if ((!inspell) && (!inbooty)) sound(5000);
 ...
 {
  backup:
  if (autonote) SetCCursor(mouse);
   else SetCCursor(notecursor);
  delay(10);
...
for(;;)
{
  ...
  if ((gTheEvent.what == mouseDown) || (gTheEvent.what == keyDown))
  {
   if ((class == -1) && (((gTheEvent.message&charCodeMask) == 'n') || (autonote)))/**** n key = take note ***/
   {
	...
	if (!autonote) flashmessage((StringPtr)"Note Taken.",30,355,20,145);
	}   
    EraseRect(&userect);
    SetCCursor(sword);
    goto out;
...
short timeclick(unsigned char number,short checkforrandom)
{
 ...
 Boolean tag = FALSE;
 if (basescale[lastpix]) scale = 1;/********* if indoor area, scale = 1 else = 5 ****/
 ...
 if (tyme.tm_min >= 60)
  {
	...
	if (timeclick) updatefat(FALSE,1,FALSE);
      ...
      if (holdover[t].spellpoints < holdover[t].maxspellpoints)
       {
        ...
        if (holdover[t].spellpoints > holdover[t].maxspellpoints) holdover[t].spellpoints = holdover[t].maxspellpoints;
       }
     }
    ...
    if ((tyme.tm_hour == 12) || (tyme.tm_hour == 24))
      {
       if (tyme.tm_hour == 24)
        ...
        /**************** check for time encounter ******/
		 ...
		 if ((fp = MyrFopen(":Data Files:CTD3","r+b")) == NULL) scratch(201);
		 ...
		 do
		 {
		  ...
		  if (timeindex > 150) goto nottimetoday;
		   ...
		   if (dotime.day == tyme.tm_yday)
		   {
			  /**** struct timeencounter
			 {
			  ...
			  short stuff[10];
				   ...
				   if (Rand(100) > dotime.percent) goto breakofday;
				   if ((dotime.recitem > 0) && (!checkforitem(dotime.recitem,FALSE,-1))) goto breakofday;
				   if ((dotime.recquest > -1) && (!quest[dotime.recquest])) goto breakofday;/**** need the quest ***/
					   ...
					   if (dotime.stuff[0] == 1)/*** land ****/
					    {
						 ...
						 test.v = looky + partyy + deltay;
					      ...
					      else
					      {
						   ...
						   {
						    if (!PtInRect(test,&randlevel.randrect[dotime.recrect])) doday = FALSE;
						   }
						   
						   if (dotime.recx > -1)/**** need x ***/
						   {
						    if (test.h != dotime.recx) doday = FALSE;
						   }
						   
						   if (dotime.recx > -1)/**** need y ***/
						   {
						    if (test.v != dotime.recy) doday = FALSE;
						   }
				 		if (doday) goto breakofday;
					}
				}
				else
				{
        ...
        if (!tag)
        {
	       for(t=0;t<=charnum;t++) 
	       {
	        if (c[t].stamina < c[t].staminamax)
	        {
	         ...
	         if (c[t].stamina > -10)
	         {
		       if (!checkforitem(877,TRUE,-1)) chunk /=2;/***** who = -1 is anybody ****/
		       if (chunk < 1) chunk = 1;
	      ...
	      for(t=0;t<heldover;t++)
	      {
	       if (holdover[t].stamina < holdover[t].staminamax)
	       {
	        ...
	        if (holdover[t].stamina > holdover[t].staminamax) holdover[t].stamina = holdover[t].staminamax;
	       }
	      }
 ...
 string(tyme.tm_yday);
      ...
      for(t=19;t>-1;t--)
      {
       if ((twixt(test.h,randlevel.randrect[t].left,randlevel.randrect[t].right)) && (twixt(test.v,randlevel.randrect[t].top,randlevel.randrect[t].bottom)))
       {
	       ...
	       if (randlevel.battlerange[t][0])
	        {
		         ...
		         if (Rand(100) < randlevel.option[t])
		         {
		          ...
		          reply = question2(0,0);
		          EraseRect(&textrect);
		          if (!reply) goto optnot;
		          /**** good suprise *****/
		          suprise = TRUE;
		         }
		         else if (Rand(100) < 10) suprise = -1;/**** bad suprise *****/
        ...
        optnot:
        if (randlevel.only[t]) return(0);/***** hit an only rect *****/
       }
      }
     return(0);
   }
