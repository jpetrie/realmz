...
void tickcheck(void)
{
 ...
 if ((defaultfont != ID_FONT_DEFAULT) && (defaultfont != 3)) defaultfont = 3;
 ...
 if (TickCount() > oldflametick + 10)
 {
  ...
  if (partycondition[0])
  {
   ...
   if (icon.top < (331+ downshift)) icon.top = 331 + downshift;
  ...
  for(t=1;t<9;t++)
  {
   ...
   if (t==5) OffsetRect(&icon,-156,40);
   if (partycondition[t]) ploticon2(t*8+13992+flamestage);
  }
 flamestage++;
 if (flamestage > 7) flamestage = 0;
 }
}
/********************************** updatetorch *********************************/
void updatetorch(void)
{
 ...
 if (checkforitem(805,FALSE,-1)) upbutton(TRUE);
