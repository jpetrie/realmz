...
void partyselect(short mode)
{
 ...
 ShowWindow(GetDialogWindow(party));
  ...
  {
   if ((fp = MyrFopen(":Character Files:Data CD","w+b")) == NULL) scratch2(10);
   fclose(fp);
 ...
 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(130);
 ...
 if (doreg()) maxlevel = 999;
 
 charselectnew = 0;
 
 for(t=0;t<=charnum;t++) c[t].inbattle = TRUE;
 ...
 for(t=54;t<57;t++) 
 {
   ...
   if ((mode) || (charnum > 1)) MyrCDiStr(89,(StringPtr)"");
   ...
   shortupdate:
	...
	DrawPicture(marker,&itemRect);
 	...
 	if (maxlevel == 999) MyrCDiStr(77,(StringPtr)"None");
 	 else DialogNumLong(77,maxlevel);
 	...
 	else MyrCDiStr(72,(StringPtr)"");
 	
 	percent /=100;
 	 	
 	if ((fp = MyrFopen(":Character Files:Data CD","rb")) == NULL) scratch(131);
 	fseek(fp,index * 32,SEEK_SET);
    for(t=2;t<14;t++)
    {
     if (t == filepick) ForeColor(yellowColor); 
      else ForeColor(cyanColor);
   	 fread(&name,30,1,fp);
   	 ...
   	 if(!feof(fp))
   	 {
   	  ...
   	  if (StringWidth((StringPtr)name)) DialogNum(t+40,level);
   	   else MyrCDiStr(t+40,(StringPtr)"");
   	 }
	  else
	  {
    ...
    for (;;)
     ...
     over:
		...
		if ((itemHit == 37) && (filepick))/****** erase char from disk ******/
		{
		  ...
		  ploticon3(130,buttonrect);
		  if (!StringWidth(myString)) goto update;
		...
		if (itemHit == 88)/****** import ******/
		{
		 ...
		 if (!SFReplyRecord.good) goto over;
		 if(filename == NULL) goto over;
	...
	charerror:
		  ...
		  else
		  {
		   ...
		   if (!checkforerrors()) plus(characterl.name,characterl.level);
		  }
		  goto bigupdate;
          #endif /* !CARBON */
 		}
		...
		if (itemHit == 34)/*********** done button *************/
		 {
		  ploticon3(129,buttonrect);
		  if (charnum > -1) sound(26260);
		  for(t=0;t<= charnum;t++) plus(c[t].name,c[t].level);
			 ...
			 for(t=0;t<c[newcharpick].numitems;t++)
			  {
			   ...
			   if ((item.type == 25) || (item.type == 23) || (item.type < 0)) danger = TRUE;
			  }
		...
		if ((itemHit == 35) && (charnum < 5) && (filepick))/**** add to party ****/
		{
		 ...
		 ploticon3(129,buttonrect);
		  ...
		  c[charnum] = characterl;
 		  ...
 		  if ((c[charnum].stamina < 1) || (c[charnum].condition[25])) killparty++;
 		  goto shortupdate;
		 }
		 ploticon3(130,buttonrect);
       }
