...
void spelleffect(short t,short mode)/*** mode 0 = normal, 1 = quickshow ****/
{
 ...
 if (!spellinfo.spelllook2) start = 12032;
  else start = (spellinfo.spelllook2*8)+11992;
 if (!incombat)
 {
  bodyrect.top = 50*t +9;
  ...
  if ((itemused) && (initems)) SetPort(GetWindowPort(itemswindow));
   else SetPort(GetWindowPort(screen));
		   ...
		   if ((body > 9) || ((body > -1) && (body <= charnum)))
			{
				if (body > 9)
					...
					if ((size == 1) || (size == 3)) bodyrect.top -=32;
					if (size > 1) bodyrect.left -=32;
