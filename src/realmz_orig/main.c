...
if(myrmagic_race!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_caste!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_blank100!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_mapstats!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_basetile!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_basescale!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_layout!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagicSolids!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_codename!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_racebasemove!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_codeseg1!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_codeseg2!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_collidecheck!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_notes!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_site!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_bufftrash2!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_definespells!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_todoque!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_spellarea!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_fumque!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_gPlayer!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_battle!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_tempfield!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_monster!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_holdover!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_themap!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_tiny!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_wealthstore!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_thief!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_storage!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_storageitems!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_dotime!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_moneypool!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_que!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_randlevel!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_treasure!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_enc2!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_enc!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_spellinfo!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_spelldata!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_monpos!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_pos!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_field!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_character1!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_characterr!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_c!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_item!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_allweapons!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_allarmor!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_allhelms!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_allmagic!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_allsupply!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_theshop!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_door!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_extenddoor!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_infodoor!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_infodoor!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_stackdoor!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_list!=MYRMAGIC) AcamErreur("Data corrupted");
if(myrmagic_myString!=MYRMAGIC) AcamErreur("Data corrupted");
if(mode>0)
	{
...
{
 NumToString(number,myString);
 if (number > 0) DrawString(myString);
}
/************************ stringnozero ********************/
...
void ToolBoxInit(void)
{
	...
	if (divine) temp = MyrOpenResFile((Ptr)"\p:Divine Right Module");
	if ((development) || (divine)) divineref = MyrOpenResFile((Ptr)"\p:Divine Development X-tras");
	...
	{	 
	 ...
	 if ((itemHit == 1) || (auto256))
	 {
	  ...
	  if (currentDepth < 8)
	  {
	 	 if (SetDepth(testdevice, 8, 4, ((**(**testdevice).gdPMap)).pixelSize)) exit(0);
	  }
	 }
	 else exit(0);
	}
...
keepmoving:
	...
	if (gTheEvent.modifiers&optionKey) nofade = TRUE;/** option disables all fading routines **/
	 	
	if (BitAnd(gTheEvent.modifiers,shiftKey)) tagger = TRUE;/** shift enambles code tracking **/
	...
	if ((fp = MyrFopen(":Data Files:Data ID","rb")) == NULL) scratch2(1);
	...
	if ((fp = MyrFopen(":Scenarios:City of Bywater:Data NI","rb")) == NULL) scratch2(14);
	...
	if ((!nofade) && (!nologo)) fadeinout(50, fadein);/******** fade to black ******/
	...
	t = GetNextEvent (everyEvent,&gTheEvent);
				...
				DoCorrectBugMADRepeat();
			#endif
	if (gTheEvent.modifiers&alphaLock) warn(21);
	/************************************************************************/
	/************************************************************************/
...
if ((fp = MyrFopen(":Data Files:Data AD","rb")) == NULL) scratch(197);
fread(&spellarea,sizeof spellarea,1,fp);
fclose (fp);

if ((fp = MyrFopen(":Data Files:Data S","rb")) == NULL) scratch2(4);/********** load in standard spells **********/
...
openrace();
 ...
 if ((fp = MyrFopen(":Data Files:Combat Data BD","rb")) == NULL) scratch(412);/********* load in combat pixmap map stats ***/
...
void MainLoop(void)
{
  ...
  for(;;)
  {
   ...
   if (TickCount() > oldtick + 20)
   {
	   ...
	   flamestage2++;
	   if (flamestage2 > 7) flamestage2 = 0;
	   ...
	   if (!reducesound)
	   {
		   ...
		   if (t < 5) sound(615);
		   if (t > 95) sound(645);
		   if (twixt(t,10,15)) sound(642);
		   if (twixt(t,30,45)) sound(10090);
	   }
   }
#endif
   HandleEvent();
	 ...
	 if (characterl.name[0] < 'A')/*** correct chracter file if it is corrupt ***/
		{
		 ...
		 for(t=0;t<30;t++) characterl.name[t] = characterl.name[t+1];
...
changed:
		...
		CtoPstr(str);
		if (MyrBitTstLong(&serial,7)) ParamText(myString,str,theString,(StringPtr)"");
		 else ParamText(myString,Name_String,theString,(StringPtr)"");
	...
	out:
		...
		if (!look)
		{
		 ...
		 else
		 {
		  DisposeDialog(about);
		  about = NIL;
		  if (!incombat) updatemain(FALSE,-1);
		 }
		FlushEvents(everyEvent,0);
		return;
}
...
if (currentscenario > topfantasoftsceanrio)//*** v7.1  3rd party scenario
{
 ...
 if ((fp = fopen(filename,"rb")) == NULL) scratch(700);
...
if (strlen(codeseg2) < temp) temp = strlen(codeseg2);
...
for(t=0;t<temp;t++)
{
 ...
 for(tt=1;tt<=registrationname[0];tt++) registrationname[tt] = tolower(registrationname[tt]);/**** convert to all lower case ******/
     ...
     for(tt=1;tt<=codeseg2[0];tt++)/***** do bit flip for codeseg2 ****/
	 {
	   if (MyrBitTstLong(&regcode,codeseg2[tt]%primer1))
	   {
	    ...
	    else
	    {
		 ...
		 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(700);
		 fseek(fp,5 * sizeof reclevel,SEEK_SET);
		 ...
		 fclose (fp);
     	 ...
     	 for(tt=1;tt<=tempnamestring[0];tt++) tempnamestring[tt] = tolower(tempnamestring[tt]);/**** convert to all lower case ******/
     	 ...
     	 if (tempvalue != regcode)
     	 {
           ...
           else
           {
     	     ...
     	     if (strcmp(tempnamestring,registrationname)) exit(0);/***** user does not match code so offset tempvalue *****/
         	setfree2(0);        	 
...
// Fantasoft v7.0 END
      ...
      StringToNum((StringPtr)gotword,&regcode);
         ...
         for(tt=0;tt<26;tt++) 
		 {
		  ...
		  if (tempword[tt] == 0) goto pushon;
		  if (tempword[tt] == 32) tempword[tt] = 128;
	 ...
	 if ((fp = fopen(filename,"rb")) == NULL) scratch(122);
	 ...
	 templong += 100;
 	 ...
 	 if ((tempvalue < 999) && (tempvalue)) exit(0);
...
short specailabs(short i)
{
	if (i < 0) return(-i);/******** this HAS to be here to account for a bug in the old Think C abs() routine I used for the registraiton codes.  Yuck! ***/
	return(i);
}
/*************** regscen_pc *******************************************/
...
if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(122);
...
if ((UserCode < 999) && (UserCode)) exit(0);
...
pictrect.bottom = pictrect.top + downshift;
 ...
 switch(who)
 {
  ...
  default:
  	...
  	if (c[who].items[itemnumber].equip) showcurse = 1;
  		else showcurse = 0;
  break;
 }
	...
	if (item.iscurse)
	{
	 if (!showcurse) temp = getselection(item.iscurse);
	  else getselection(item.itemid);
