...
void makestring(short id)
{
 ...
 {
 	 getfilename("Data OD");
  	 if ((fp = MyrFopen(filename,"r+b")) == NULL) scratch(10);
 ...
 else
 {
 	 getfilename("Data SD2");
  	 if ((fp = MyrFopen(filename,"r+b")) == NULL) scratch(11);
	 GetDialogItem(gCurrent,id,&itemType,&itemHandle,&itemRect);
	 GetIndString(myString,3,1);
	 GetDialogItemText(itemHandle,myString);
...
void getstring(void)
{
 ...
 if (stringindex < 0)
 {
  GetIndString(myString,3,1);
  getfilename("Data OD");
	 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(12);
	 fseek(fp,abs(stringindex) * 25,SEEK_SET);
 ...
 {
 	 getfilename("Data SD2");
	 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(13);
	 fseek(fp,stringindex * sizeof myString,SEEK_SET);
...
void warp(void)
{
 ...
 killparty = 0;
    ...
    if (iconhand)
    {
     ...
     EraseRect(&itemRect);
     if(monpick.size == 2) InsetRect(&itemRect,0,16);
     if(monpick.size == 1) InsetRect(&itemRect,16,0);
     if(monpick.size == 0) InsetRect(&itemRect,16,16);
...
over:
	 ...
	 updatequest:					  
		...
		if ((questid > -1) && (questid < 101))
		{
			...
			if (itemHit == 11)
			{								
				if (quest[questid]) quest[questid] = FALSE;
				 else quest[questid] = TRUE;
	 ...
	 if (itemHit == 12) goto updatequest;
	...
	if (itemHit == 7)/***** land only ****/
	{
	 sound(607);
		...
		for(t=0;t<90;t++)
		{
			for(tt=0;tt<90;tt++) site[t][tt] = 0;
		}
	 
		if ((op = MyrFopen(":Data Files:CLD","w+b")) == NULL) scratch(14);
		getfilename("Data DD");
		if ((fp =MyrFopen(filename,"rb")) == NULL) scratch(15);
		getfilename("Data LD");
		if ((fp2 = MyrFopen(filename,"rb")) == NULL) scratch(16);
		getfilename("Data RD");
		if ((fp3 =MyrFopen(filename,"rb")) == NULL) scratch(17);
		while (fread(&door,sizeof door,1,fp) == 1)
		{
			...
			CvtTabDoorToPc(door,100);
			if (!fwrite(&door,sizeof door,1,op)) scratch(18);
			CvtTabDoorToPc(door,100);

			CvtFieldToPc(&field);
			if (!fwrite(&field,sizeof field,1,op)) scratch(19);
			CvtFieldToPc(&field);
			
			CvtRandLevelToPc(&randlevel);
			if (!fwrite(&randlevel,sizeof randlevel,1,op)) scratch(20);
			CvtRandLevelToPc(&randlevel);

			/* MYRIAD : Boolean no conversion needed */
			if (!fwrite(&site,sizeof site,1,op)) scratch(20);
		}
		fclose (fp);
		fclose (fp2);
		fclose (fp3);
   ...
   if (itemHit == 15)/**** give item *****/
   {
	 ...
	 for(t=0;t<20;t++) treasure.itemid[t] = itemnumid;
	 booty(1);
	 DisposeDialog(gGeneration);
	 needupdate = TRUE;
 	 return;
   ...
   if (itemHit == 31)/**** check NPC *****/
   {
	...
	if (itemHit == 6)
	{
	 ...
	 sound(607);
		if ((op = MyrFopen(":Data Files:CL","w+b")) == NULL) scratch(21);
		getfilename("Data DD");
		if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(22);
		getfilename("Data LD");
		if ((fp2 = MyrFopen(filename,"rb")) == NULL) scratch(23);
		getfilename("Data RD");
		if ((fp3 = MyrFopen(filename,"rb")) == NULL) scratch(25);
		while (fread(&door,sizeof door,1,fp) == 1)
		{
			CvtTabDoorToPc(door,100);
		 	sound(141);
			...
			CvtTabDoorToPc(door,100);
			if (!fwrite(&door,sizeof door,1,op)) scratch(25);
			CvtTabDoorToPc(door,100);
		
			CvtFieldToPc(&field);
			if (!fwrite(&field,sizeof field,1,op)) scratch(26);
			CvtFieldToPc(&field);
			
			CvtRandLevelToPc(&randlevel);
			if (!fwrite(&randlevel,sizeof randlevel,1,op)) scratch(27);
			CvtRandLevelToPc(&randlevel);
		...
		if ((op = MyrFopen(":Data Files:CD","w+b")) == NULL) scratch(28);
		getfilename("Data DDD");
		if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(29);
		getfilename("Data DL");
		if ((fp2 = MyrFopen(filename,"rb")) == NULL) scratch(30);
		getfilename("Data RDD");
		if ((fp3 = MyrFopen(filename,"rb")) == NULL) scratch(31);
		while (fread(&door,sizeof door,1,fp) == 1)
		{
			CvtTabDoorToPc(door,100);
			sound(141);
			...
			CvtTabDoorToPc(door,100);
			if (!fwrite(&door,sizeof door,1,op)) scratch(32);
			CvtTabDoorToPc(door,100);
			
			CvtFieldToPc(&field);
			if (!fwrite(&field,sizeof field,1,op)) scratch(33);
			CvtFieldToPc(&field);
			
			CvtRandLevelToPc(&randlevel);
			if (!fwrite(&randlevel,sizeof randlevel,1,op)) scratch(34);
			CvtRandLevelToPc(&randlevel);
		}
		fclose (fp);
		fclose (fp2);
	...
	getfilename("Data SD");
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(35);/********* shops ********/
	if ((op = MyrFopen(":Data Files:CS","w+b")) == NULL) scratch(36);
	while (fread(&theshop,sizeof theshop,1,fp) == 1)
		{
		// No conversion needed : i write the data read as is 
		if (!fwrite(&theshop,sizeof theshop,1,op)) scratch(37);
		}
	fclose (fp);
	fclose (op);

	getfilename("Data TD2");	
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(38);/********* thief ********/
	if ((op = MyrFopen(":Data Files:CT","w+b")) == NULL) scratch(39);
	while (fread(&thief,sizeof thief,1,fp) == 1)
	{
		if (!fwrite(&thief,sizeof thief,1,op)) scratch(40);
	}
	fclose (fp);
	fclose (op);
	
	getfilename("Data TD3");	
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(41);/********* time encoutner ********/
	if ((op = MyrFopen(":Data Files:CTD3","w+b")) == NULL) scratch(42);
	while (fread(&dotime,sizeof dotime,1,fp) == 1)
	{
		if (!fwrite(&dotime,sizeof dotime,1,op)) scratch(43);
	}
	fclose (fp);
	fclose (op);

	getfilename("Data ED");	
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(44);/********* simple encounters ********/
	
	if ((op = MyrFopen(":Data Files:CE","w+b")) == NULL) scratch(45);
	while (fread(&enc,sizeof enc,1,fp) == 1)
	{
 	...
 	fwrite(&enc,sizeof enc,1,op);
		for(tt=0;tt<4;tt++)
		{
			if (!fwrite(&buffer[tt], 80,1,op)) scratch(46);
		}
	}
	fclose (fp);
	fclose (op);
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(47);/********* complex encounters ********/
	if ((op = MyrFopen(":Data Files:CE2","w+b")) == NULL) scratch(48);
	
	while(fread(&enc2,sizeof enc2,1,fp))
	{
	for(t=0;t<9;t++) fread(&buffer[t],40,1,fp);
		if (!fwrite(&enc2,sizeof enc2,1,op)) scratch(49);
		for(t=0;t<9;t++)
		{
			if (!fwrite(&buffer[t],40,1,op)) scratch(50);
		}
	}
	fclose (fp);
	fclose (op);
