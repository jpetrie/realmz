...
void swap(void)
{
 ...
 if ((shopavail) || (templeavail)) looktype = 0;

 inswap = TRUE;
 if (!reducesound) sound(3003);
 ...
 if ((shopavail) || (templeavail)) MyrCDiStr(64,(StringPtr)"Money Changing is available.");
  else MyrCDiStr(64,(StringPtr)"Money Changing is not available.");
		  ...
		  if ((itemHit > 16) && (itemHit < 23))
		    {
		     ...
		     wieght = times = 1;
		     if (moneytype == 0) times = 5;
		     if (moneytype == 2) wieght = 15;

		     while (Button())
		     {
		      if (((itemHit == 17) || (itemHit == 19) || (itemHit == 21)) && (c[whichchar].money[moneytype]) >= times) direction = -1;
		       else if ((((itemHit == 18) || (itemHit == 20) || (itemHit == 22))) && (moneypool[moneytype] >= times)) direction = 1;
		        else direction = 0;
		        		        		       
		       if ((c[whichchar].load + (wieght*times) > c[whichchar].loadmax) && (direction == 1)) direction = 0;
		       moneypool[moneytype] -= direction*times;
 		       TextSize(22);
 		       if (direction > 0) sound(10051);
 		           else sound(663);
