...
update:
	...
	for(t=2;t<=13;t++) DialogNum(t,races.minmax[t-2]);
	for(t=14;t<=19;t++) DialogNumNZ(t,races.attbonus[t-14]);
	for(t=36;t<=43;t++) DialogNumNZ(t,races.plusminustohit[t-36]);
	...
	if (races.canregenerate) MyrCDiStr(49,(StringPtr)"Yes");
	 else MyrCDiStr(49,(StringPtr)"No");
	...
	for(t=59;t<=72;t++) DialogNumNZ(t,races.specialability[t-59]);
	for(t=28;t<=35;t++) DialogNumNZ(t,races.drvbonus[t-28]);
    ...
    for(t=0;t<4;t++) MyrCDiStr(52+t,(StringPtr)"");
    for(t=0;t<40;t++)
    {
     if (races.conditions[t])
     {
	  ...
	  if (conditionindex == 4) goto moveon;
	 }
	}
...
out:
    if (background != NIL) SetPortDialogPort(background);
	DisposeDialog(gGeneration);
	if (background != NIL) DrawDialog(background);
}