...
void addinitialitems(void)
{
 ...
 c[0] = characterl;
 for(t=0;t<c[0].numitems;t++) wear(0,t,0);
 characterl = c[0];
...
void NewCharacter(short level)
{
	...
	for(t=0;t<20;t++) characterl.armor[t] = 0;
	for(t=0;t<40;t++) characterl.condition[t] = races.conditions[t];
	for(t=0;t<40;t++)
	{
	 if (caste.conditions[t] == 1) characterl.condition[t] = -1;
	}
	for(t=0;t<30;t++) characterl.items[t].charge = characterl.items[t].id = characterl.items[t].ident = characterl.items[t].equip = 0;
...
waynew:
	...
	for(t=0;t<15;t++) characterl.spec[t] = 0;
	for(t=0;t<12;t++) characterl.special[t] = 0;
	for(t=0;t<8;t++) characterl.save[t] = 50 + races.drvbonus[t] + caste.drvbonus[t];
	for(t=0;t<8;t++) characterl.special[t] = races.plusminustohit[t];
	...
	for(;;)
	{
	 ...
	 if (characterl.st > 25)
	 {
	  ...
	  exit(0);
		...
		if (caste.minimumagegroup > 1) applyage(characterl.race,2,1);/*********** applies initial Young age parameters ***/
		if (caste.minimumagegroup > 2) applyage(characterl.race,3,1);/*********** applies initial Adult age parameters ***/
		if (caste.minimumagegroup > 3) applyage(characterl.race,4,1);/*********** applies initial Prime age parameters ***/
		if (caste.minimumagegroup > 4) applyage(characterl.race,5,1);/*********** applies initial Senior age parameters ***/
	...
	for(t=0;t<7;t++)
	{
	    ...
	    {	    
 		...
 		if (Rand(100) > 80) characterl.special[randrange(0,7)] ++;
	    if (Rand(100) > 90) characterl.special[randrange(0,7)] ++;
	    if (Rand(100) > 95) characterl.special[randrange(0,7)] ++;
		...
		if (characterl.co > 16)
		{
		 ...
		 if (temp > caste.maxstaminabonus) temp = caste.maxstaminabonus;
		}
		...
		characterl.tohit += temp;
 		...
 		characterl.ac = 0;
 		if (characterl.de > 14) characterl.ac -= 2*(14 - characterl.de);
		...
		if (!caste.canusemissile) characterl.missile = 0;
		 
		characterl.movementmax = races.basemove + caste.movebonus; 
			
		for(t=0;t<8;t++)
		{
		 if (characterl.save[t] < -99) characterl.save[t] = -99;
		 if (characterl.save[t] > 120) characterl.save[t] = 120;
		}
		...
		if (caste.spellcasters[0][1])
		{
		 ...
		 if ((level+1) >= caste.spellcasters[0][1]) characterl.spellpointsmax = 4 + characterl.in + (Rand(characterl.wi));/******* scorcerer ***/
		}
		if (caste.spellcasters[1][1])
		{
		 characterl.spellcastertype = 2;
		 if ((level+1) >= caste.spellcasters[1][1]) characterl.spellpointsmax = 4 + characterl.wi + (Rand(characterl.in));/******* preist ***/
		}
		if (caste.spellcasters[2][1])
		{
		 characterl.spellcastertype = 3;
		 if ((level+1) >= caste.spellcasters[2][1]) characterl.spellpointsmax = 10 + Rand(characterl.wi + characterl.in);/******* enchanter ***/
		}
		...
		if (level) for(t=0;t<level;t++) levelup(0,TRUE);
		characterl = c[0];
...
backup:
	    ...
	    if (itemHit == 70)/******** view aging **********/
		{
			  ...
			  for(t=0;t<5;t++)
			  {
			   ...
			   if (twixt(temp,races.agerange[t][0],races.agerange[t][1])) ForeColor(whiteColor);
			   ...
			   TextSize(18);
			   for(tt=0;tt<15;tt++) DialogNumNZ(14+(t*17)+tt,races.agechange[t][tt]);
			  }
	...
	short a;
	 ...
	 if (gTheEvent.modifiers&alphaLock) warn(21);
		...
		if (IsDialogEvent (&gTheEvent))
		{
		 ...
		 if(gTheEvent.what == mouseDown) goto domouse;
		}
		else
		{
...
{
for (t=0;t<7;t++) for(tt=0;tt<12;tt++) characterl.cspells[t][tt] = 0;
Spellselect();
gCurrent = gGeneration;
ShowStats(0);
}
...
if (question(2)) savecharacter(-1);
else minus(characterl.name,0);
