...
inshop = TRUE;
    ...
    SetCCursor(sword);
    for (t=0;t<=numchannel;t++) quiet(t);
...
if (!mode) 
  {
    ...
    if (!shopavail)
    {
     ...
     BackColor(whiteColor);
 	 	 ...
 	 	 if (characterl.numitems-9 > 0) SetControlMaximum(charitemsvert,characterl.numitems-9);
 	 	  else SetControlMaximum(charitemsvert,0);
 	 	 if (!mode) shopselection = 0;
...
{
 if ((!iteminfo) || (!ident)) return;
 
 			 if (item.iscurse) loaditem(item.iscurse);/********* so details dont give away cursed items ****/// Fantasoft v7.0
 		   ...
 		   if (item.vssmall > 0)
 		   {
 		     ...
 		     if (item.heat+item.cold+item.electric+item.vsundead+item.vsdd+item.vsevil) MyrDrawCString(" + Special  ");
 		      else MyrDrawCString("  ");
 		   }
 		    ...
 		    if (item.damage > 0)
 		     ...
 		     else if (item.vssmall) MyrDrawCString("  ");
...
short canuse(short itemid, short raceid, short casteid)
{
 ...
 {
  if (raceid != item.specificrace) cantuseflag = TRUE;
 }
 
 if (item.specificcaste)
 {
  if (casteid != item.specificcaste) cantuseflag2 = TRUE;
 }
...
if (!cantuseflag)
{
 for(tt=0;tt<58;tt++)
 {
  if (MyrBitTstLong(&item.itemcat[tt/32],tt%32)) goto pushon;/********** locate item catagory ********/
 }
	...
	for(tt=0;tt<9;tt++)/******** look for race restriction ************/
 	{
  	 ...
  	 {
  	  if (MyrBitTstShort(&races.descriptors,tt)) cantuseflag = TRUE;
  	 }
 	}  
	...
	for(tt=0;tt<7;tt++)/******** look for caste restriction ************/
 	{
  	 ...
  	 {
  	  if (tt == (caste.casteclass-1)) cantuseflag2 = TRUE;
  	 }
 	}  
   
   if (item.casteclassonly)
   {
  	 if (!MyrBitTstShort(&item.casteclassonly,caste.casteclass-1)) cantuseflag2 = TRUE;/******** test to see if there is some type of caste class restriction ***/
   }
  
  if ((!cantuseflag) && (!cantuseflag2)) return(TRUE);
 }
	else
	{
	 if (!MyrBitTstLong(&caste.itemtypes[tt/32],tt%32)) cantuseflag2 = TRUE;
	  else cantuseflag = TRUE;
	}
}
...
reload:
	...
	if (item.nohands) DialogNum(21,item.nohands);
	RGBForeColor(&cyancolor);
	if (item.vssmall) DialogNum(29,item.vssmall);
	showindex = 5;
	...
	if (ident)
	{
	 ...
	 if (item.sp3 < 0)/****** ability to hit monster types ****/
		{
		   ...
		   if (item.sp5 > 0) strcpy(special,(StringPtr)"+");
	 ...
	 if ((item.sp4 > 0) && (item.sp4 < 16)) /****** add to characters special abilitys like pick lock ****/
		...
		RGBForeColor(&greencolor);
		if (item.damage) DialogNum(15,item.damage);
		if (item.st) DialogNum(16,item.st);
		if (item.lu) DialogNum(17,item.lu);
		if (item.ac) DialogNum(19,item.ac);
		if (item.movement) DialogNum(20,item.movement);
		if (item.magres) DialogNum(22,item.magres);
		if (item.spellpoints) DialogNum(23,item.spellpoints);

		RGBForeColor(&cyancolor);
		
		if (item.heat) DialogNum(9,item.heat);
		if (item.cold) DialogNum(10,item.cold);
		if (item.electric) DialogNum(11,item.electric);
		if (item.vsundead) DialogNum(12,item.vsundead);
		if (item.vsdd) DialogNum(13,item.vsdd);
		if (item.vsevil) DialogNum(14,item.vsevil);
		
		GetDialogItem(info,27,&itemType,&itemHandle,&icon);
		if (item.cost < 0) DrawPicture(on,&icon);
		 else DrawPicture(non,&icon);
	}
	...
	{
	 if (cantuseflag) MyrCDiStr(26,(StringPtr)"Racial Conflict");
	 if (cantuseflag2) MyrCDiStr(28,(StringPtr)"Caste Conflict");
	 ForeColor(redColor);
	 MyrCDiStr(25,(StringPtr)"N");
	}
	else MyrCDiStr(25,(StringPtr)"Y");
	...
	if (item.casterestrictions)/******* show nonusable by specific caste class******/
	{
	 ...
	 for(t=0;t<7;t++)
	 {
	  if (MyrBitTstShort(&item.casterestrictions,t))
	  {
	   ...
	   PtoCstr(myString);
	   if (comma) strcat(special,(StringPtr)", ");
	   strcat(special,myString);
	   comma = TRUE;
	  }
	 }
						...
						if (whichWindow  == itemswindow)
							{
							 ...
							 if (point.h < 280)
							 {
								 ...
								 if (itemselectnew < c[charselectnew].numitems)
								 {
									 ...
									 for(t=4;t<27;t++) MyrCDiStr(t,(StringPtr)"");
