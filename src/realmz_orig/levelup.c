...
for(t=0;t<10;t++) if ((caste.attacks[t] <= characterl.level) && (caste.attacks[t])) characterl.normattacks++;
...
/********** check for new conditions for caste due to new level ***************/
	for(t=0;t<40;t++)
	{
	 ...
	 {
	  if (characterl.condition[t] > -1) characterl.condition[t] = -1;
	   else characterl.condition[t] --;
	 }
	}

 /************** check race limitation on max attacks **************/
  
if (characterl.normattacks > (2*races.numofattacks[1])) characterl.normattacks = (2*races.numofattacks[1]);

tohit = caste.tohit[1];
characterl.tohit += caste.tohit[1];
characterl.dodge += caste.dodge[1];
characterl.handtohand += caste.hand2hand[1];
if (caste.missile[1]) characterl.missile += Rand(caste.missile[1]);


/*********** set spellcastertype for level up for higher starters ************/
if (caste.spellcasters[0][1])  characterl.spellcastertype = 1;
else if (caste.spellcasters[1][1]) characterl.spellcastertype = 2;
else if (caste.spellcasters[2][1]) characterl.spellcastertype = 3;


switch(characterl.spellcastertype)/********** only award spell points if they are above neccesary level but not 1st level *********/
{
 case 1: /***** Sorcerer ****/
 		if ((caste.spellcasters[0][1] <= characterl.level) && (characterl.level > 1)) spellpoints = characterl.level + Rand(characterl.in + (characterl.wi/2));
 break;

 case 2:/***** Priest ******/
 		if ((caste.spellcasters[1][1] <= characterl.level) && (characterl.level > 1)) spellpoints = characterl.level + Rand(characterl.wi + (characterl.in/2));
 break;

 case 3:/***** Enchanter ****/
 		if ((caste.spellcasters[2][1] <= characterl.level) && (characterl.level > 1))  spellpoints = characterl.level + Rand(characterl.wi + (characterl.in/2));
 break;
}
...
if (!mode)
{
 ...
 if (characterl.normattacks > 12) characterl.normattacks = 12;
