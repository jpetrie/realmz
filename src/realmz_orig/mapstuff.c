...
void showmap(short mapnumber)
{
 ...
 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(115);
 ...
 if (themap.show < 0)
 {
  ...
  goto out;
 ...
 else if (themap.pictid)
			 {
			  ...
			  pict(themap.pictid,itemRect);
			...
			else
				{
				 ...
				 if (temp * themap.iconsize < 320) temp++;
			...
			for(t=0;t<10;t++)
			{
			 if (themap.icon[t][0])
			 {
			  if (!themap.iconsize) themap.iconsize = 32;
...
if (!indung) loadland(landlevel,TRUE);
 else loadland(dunglevel,TRUE);
