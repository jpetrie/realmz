...
void threed(long id,short gox,short goy,short dir)
{
 ...
 updatewalls(floorx-10,floory-10);
   ...
   if (!skiptime)
   {
    if (checkforsecret(FALSE)) updatewalls(floorx-10,floory-10);
   }
 
 if (bringbacktag) goto bringback;
 
 if (!skiptime) sound(130);
...
for (;;)
	{
	 ...
	 if (StillDown())
	 {
	  delay(10);
	  GetMouse(&point);
	  if (StillDown()) goto doitagainsam;
	 }
	 ...
	 if (gTheEvent.modifiers&alphaLock) warn(21);
		...
		if (gTheEvent.what == autoKey) goto dokey;
		 ...
		 doitagainsam:
					 	 	...
					 	 	if (!hide)
					 	 	{
					 	 	 ...
					 	 	 else if (viewtype == 1)
					 	 	 {
								...
								itemRect = AHEAD_RECT;
								if( PtInRect( point, &itemRect )) cursor = 165;


								itemRect = BACK_RECT;
								if( PtInRect( point, &itemRect )) cursor = 166;
									
									
								itemRect = TURN_LEFT_RECT;
								if( PtInRect( point, &itemRect )) cursor = 167;
											
											
								itemRect = TURN_RIGHT_RECT;
								if( PtInRect( point, &itemRect )) cursor = 168;
					 	 	 ...
					 	 	 else
					 	 	 {
								...
								if ((point.h >= point.v) && (point.h <= 320 + (leftshift/2) - point.v)) cursor = 139;
								
								if ((point.h <= point.v) && (point.v <= 320 + (leftshift/2) - point.h)) cursor = 141;
						
								if ((point.v >= 320 + (downshift/2) - point.h) && (point.v >= point.h)) cursor = 145;
						
								if ((point.h >= 320 + (downshift/2) - point.v) && (point.v <= point.h)) cursor = 143;
			...
			break;
					...
					switch (thePart)
					{
					  ...
					  case  inContent:
			             ...
			             goback:
			              ...
			              if ((theControl == tradebut) && (charnum)) UpdateWindow(FALSE);
			              if (theControl == shopbut) UpdateWindow(FALSE);
			              if (theControl == itemsbut) UpdateWindow(FALSE);
			              if (theControl == swapbut) UpdateWindow(FALSE);
			              
			              if (!indung) return;/***** leave dungeon ***/
				...
				case keyDown:
					...
					dokey:
						...
						case 'p':/***** p   scroll    *****/
							if ((!inspell) && (checkfortype(charselectnew,13,TRUE))) theControl =  viewspellsbut;
						break;
						case 'k':/***** k   create scroll    *****/
							if ((!inspell) && (checkfortype(charselectnew,13,TRUE)))
							{
							 if ((incamp) && (c[charselectnew].stamina > 0)) theControl =  overviewbut;
							}
						break;
						case 'c':/***** c   Camp    *****/
							theControl =  campbut;
						...
						break;
						case 't':/***** t   trade    *****/
							if (charnum) theControl = tradebut;
						break;
						case 'h':/***** h   heal    *****/
							theControl = barbut;
						break;
						case 'r':/***** r   rest    *****/
							if (incamp == TRUE) theControl = rest;
						break;
						case 'e':/***** e   encounter    *****/
							if ((!incamp) && (!shopavail) && (!templeavail)) theControl = shopbut;
						break;
						case 'a':/***** a   area search    *****/
							if (!incamp) theControl = overviewbut;
						break;
						case 'g':/***** g   go shop    *****/
							if ((!incamp) && ((shopavail) || (templeavail))) theControl = shopbut;
						break;
					...
					if (theControl != NIL)  goto goback;

					makemove:
					
					if ((!deltax) && (!deltay)) goto update;
						...
						if ((wall) && (!MyrBitTstShort((void *)(&wall),14)) && (!MyrBitTstShort((void *)(&wall),13)))
						{
						 ...
						 if ((MyrBitTstShort((void *)(&wall),10)) || (MyrBitTstShort((void *)(&wall),3))) goto check1;/**** note / encounter ****/
						 
						 if (!MyrBitTstShort((void *)(&wall),15)) goto check1;
						 ...
						 {
						  check1:
						  if (!skiptime) timeclick(1,TRUE);
						  if (revertgame) return;
						  if (!indung) return;
						  
						  if (BitAnd(wall,6))/**** some kind of door ****/
						  {
						    if ((!MyrBitTstShort(&wall,2)) && (Rand(10) < 4)) sound(134);/**** squeeky door ****/
						  }
					...
					if (MyrBitTstShort(&wall,3))/**** encounter ****/
					  {
					   ...
					   {
					     if (revertgame) return;
					     if (!indung) return;/***** leave dungeon   Need in both places ***/
					     deltax *= -1;
					     deltay *= -1;
					     if (needdungeonupdate) UpdateWindow(FALSE);
...
void updatewalls(long x,long y)
{
 ...
 if (Rand(100) < 10)
 {
  if ((!doreg3()) && (currentscenario > 10)) scratch(452);
 }

 if (viewtype == 1)
 {
 		if (FrontWindow() != gWindow) UpdateWindow(FALSE);
 		 else
 		 {
            CGrafPtr qdp;
 ...
 {
	 SetPort(GetWindowPort(look));
	 if (FrontWindow() != look) SelectWindow(look);
	 ...
	 for(t=y;t<y+20;t++)
	 {
	  ...
	  for(tt=x;tt<x+20;tt++)
	  {
	   box.left +=16;
	   box.right +=16;
	    if ((t> -1) && (t< 90) && (tt >-1) && (tt < 90)) plotwall(field[t][tt]);
	  }
	 }
