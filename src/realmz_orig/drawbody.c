...
void drawbody(short body,short force,short where)/***** 0 = look, 1 = buff ******/
{
	...
	if (body > 9)
	{
		if (monster[body-10].stamina < 1) return;
		if (monster[body-10].condition[1]) helpless = TRUE;
	...
	{
		if (c[body].stamina < 1) return;
		if (c[body].condition[1]) helpless = TRUE;
	...
	if ((body == q[up]) || (force) || (helpless))/***** 0 = look, 1 = buff ******/
	{
	 ...
	 if ((!where) && (iconhand)) PlotCIcon(&bodyrect,iconhand);/***** look *****/
