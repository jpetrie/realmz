...
void centerpict(void)
{
	...
	if (indung)
	{
	 ...
	 else if ((viewtype == 1) && (needdungeonupdate)) UpdateWindow(0);
	  else  updatewalls(floorx-10,floory-10);
	...
	if (partyy > 13) partyy = 12;
	 else if (partyy < 0) partyy = 0;
	if (partyx > 15) partyx = 14;
	 else if (partyx < 0) partyx = 0;
	...
	for(tt=looky;tt<13+looky;tt++)
	{
		...
		for(t=lookx;t<15+lookx;t++)
		{
			...
			if ((!site[t][tt]) && (randlevel.uselos)) fastplot(252,icon,0,1);/**** showblack ****/
			else
			{
			   tempicon = field[t][tt];
			   ...
			   {
				 if (tempicon < -1999) tempicon += 2000;
				  else if (tempicon <  -999) tempicon += 1000;
		monstericon:		
			    ...
			    if (iconhand)
			    {
				 ...
				 {
	   			  if ((**iconhand).iconBMap.bounds.bottom == 64) icon.top -= 32;
	   			  if ((**iconhand).iconBMap.bounds.right == 64) icon.left -= 32;
				 }
			   ...
			   else
			   {
				...
				if (tempicon > 200) goto monstericon;
...
if (randlevel.isdark)
  {
   ...
   else
   {
	...
	{
	 if (partyy > 6 + (2*screensize)) bitrect.bottom = temp;
	 else bitrect.top = (320 + downshift) - temp;
	}
	...
	{
	 if (partyx > (8 + (screensize*3))) bitrect.right = temp;
	 else bitrect.left = (320 + leftshift) - temp;
	}
 }
    iconhand = NIL;
    
    if (charnum > 5) charnum = 5;
    if (charnum < 0) charnum = 0;
        
    	if (innotes) iconhand = GetCIcon(145);
	       else if (inboat)
	       {
			if (boatright) iconhand = GetCIcon(13400+lastpix);
  			 else iconhand = GetCIcon(13300+lastpix);
	       }
    	 	else if (incamp) iconhand = GetCIcon(178);
    	 	 else if (basescale[lastpix]) iconhand = GetCIcon(9500+horseicon-(500*face));
	      	  else iconhand = GetCIcon(175+(11*face));
        ...
        SetPort((GrafPtr)GetWindowPort(screen));
        if (iconhand) PlotCIcon(&icon,iconhand);
        SetPort((GrafPtr)GetWindowPort(look));
