...
short getscroll(void)
{
	...
	if (!checkfortype(charselectnew,13,FALSE)) goto more;
...
more:
		...
		{
			if (!initems) SetPort(GetWindowPort(look));
			 else SetPort(GetWindowPort(itemswindow));
		...
		if (c[charselectnew].scrollcase[itemHit-7].powerlevel)
		{
			...
			{
			 if (question(-5)) c[charselectnew].scrollcase[itemHit-7].powerlevel = 0;
			 goto over;
			}
