...
short attack(short chare,short mon)
{
	...
	if (chare < 7) character = c[chare];
	if (mon > 9) monst = monster[mon-10];
	...
	reply = FALSE;
    for (t=0;t<=charnum;t++) if ((c[t].inbattle) && (!c[t].traiter)) reply = TRUE;
       if (!reply)
       {
        killmon = numenemy;
        return(0);
	...
	if (character.armor[2])
	{
		...
		if (item.sp1 == 121) att += 5*item.damage;/******* double to hit weapon ****/
		
		if (item.sp1 == -10)/***** weapon inflicts condition on hit ******/
		{
		 if (item.sp2 == 1)/***** DRVs.  ******/
		 {
		  if (!savevs(item.sp4,mon)) goto autoadd;
		 }
		 else if (item.sp2 == 2)/***** % chance.  ******/
		 {
		  if (Rand(100) <= item.sp4) goto autoadd;
		 }
...
moveon:
	...
	if (character.condition[2]) att -= abs(character.condition[2]);/*** tangled ***/
	if (character.condition[21]) att +=15;/**** strong ***/
	if (character.condition[6]) att -= 15;/**** slow ***/
	if (character.condition[29]) att -= 10;/**** confused ***/
	if (character.condition[27]) att -= 15;/**** blind ***/
	if (character.condition[4]) att +=5;/**** bless ***/
	if (character.condition[3]) att -=5;/**** curse ***/
	if (character.condition[36]) att -= abs(character.condition[36]);/*** Hinder atk ***/
	if (((monst.type[4]) || (monst.type[5])) && (character.condition[22])) att += 10;/*** +10% for pro evil ***/
   ...
   if (mon > 9)
   {
	...
	if (mon > 9)
	{
		...
		if (monst.condition[29]) att += 10;/*** confused enemy ***/
		if (monst.condition[27]) att += 15;/*** blind enemy ***/
		if (monst.condition[6]) att += 15;/*** slow enemy ***/
		if (monst.condition[4]) att -= 5;/*** blessed enemy ***/
		if (monst.condition[3]) att += 5;/*** cursed enemy ***/
		if (monst.condition[24]) att -= 10;/*** invisible enemy ***/
		if (monst.condition[2]) att += abs(monst.condition[2]);/*** tangled ***/
		if (monst.condition[37]) att += abs(monst.condition[37]);/*** hinter defense ***/
		if (monst.condition[38]) att -= abs(monst.condition[38]);/*** defense bonus ***/
		if (monst.condition[22]) att -= 10;/*** pro evil enemy***/
	...
	else
	{
		...
		if (c[mon].condition[29]) att += 10;/*** confused MAN ***/
		if (c[mon].condition[27]) att += 15;/*** blind MAN ***/
		if (c[mon].condition[6]) att += 15;/*** slow MAN ***/
		if (c[mon].condition[4]) att -= 5;/*** blessed MAN ***/
		if (c[mon].condition[3]) att += 5;/*** cursed MAN ***/
		if (c[mon].condition[24]) att -= 10;/*** invisible MAN ***/
		if (c[mon].condition[2]) att += abs(c[mon].condition[2]);/*** tangled ***/
		if (c[mon].condition[37]) att += abs(c[mon].condition[37]);/*** hinder defense ***/
		if (c[mon].condition[38]) att -= abs(c[mon].condition[38]);/*** defense bonus ***/
	}
...
if (att < 5) att = 5;
	...
	if (Rand(100) <= att) success = TRUE;

	if (mon > 9)
	{
     if (monst.condition[1]) success = TRUE;
	} 
	 else if (c[mon].condition[1]) success = TRUE;
...
donefumble:
 ...
 restorecontrol:
	  if (c[chare].condition[25] == 1) c[chare].condition[25] = poss = 0;
      GetControlBounds(campbut, &r);
	  ploticon3(129,r);
      sound(141);
      combatupdate2(chare);
	  if (doauto[chare]) theControl = autoone[chare];
      checkfordoauto();
	  return(0);
	 }
	}
	...
	if (item.sp1 == 120) success = TRUE;/********* auto hit weapon ************/

	if (success)
	{
	 if ((mon > 9) && (monst.condition[31]) && (Rand(100) < 34)) mon = chare;
	  else if (mon < 9)
		{
		 if ((c[mon].condition[31]) && (Rand(100) < 34)) mon = chare;
		}

		if (character.condition[21]) damage += 3;/****** strong *****/
		if (!character.armor[2]) damage += Rand(character.handtohand);
		else
		{
		 if (mon > 9) /***** hit monster ****/
		 {
		  monster[mon-10].beenattacked = TRUE;
			...
			if ((item.heat) && (!monst.spellimmune[1]))
			{
			 ...
			 if (monst.condition[11]) specdamage /=2;
			 if (savevs(1,chare)) specdamage /=2;
			 specialdam += specdamage;
			 if (specdamage) showresults(mon,-19,chare);
			}
			
			if ((item.cold) && (!monst.spellimmune[2]))
			{
			 specdamage = Rand(item.cold);
			 if (monst.condition[12]) specdamage /=2;
			 if (savevs(2,chare)) specdamage /=2;
			 specialdam += specdamage;
			 if (specdamage) showresults(mon,-20,chare);
			}
			if ((item.electric) && (!monst.spellimmune[3]))
			{
			 specdamage = Rand(item.electric);
			 if (monst.condition[13]) specdamage /=2;
			 if (savevs(3,chare)) specdamage /=2;
			 specialdam += specdamage;
			 if (specdamage) showresults(mon,-21,chare);
			}
			if	((item.vsundead) && (monst.type[1])) damage += Rand(item.vsundead);
			if	((item.vsdd) && ((monst.type[2]) || (monst.type[3]))) damage += Rand(item.vsdd);
			if	((item.vsevil) && ((monst.type[4]) || (monst.type[5]))) damage += Rand(item.vsevil);
		 ...
		 else /**** hit man ****/
		 {
			...
			{
			 specdamage = Rand(item.heat);
			 if (savevs(1,mon)) specdamage /=2;
			 if (c[mon].condition[11]) specdamage /=2;
			 specialdam += specdamage;
			 if (specdamage) showresults(mon,-19,chare);
			}
			if (item.cold)
			{
			 specdamage = Rand(item.cold);
			 if (c[mon].condition[12]) specdamage /=2;
			 if (savevs(2,mon)) specdamage /=2;
			 specialdam += specdamage;
			 if (specdamage) showresults(mon,-20,chare);
			}
			if (item.electric)
			{
			 specdamage = Rand(item.electric);
			 if (c[mon].condition[13]) specdamage /=2;
			 if (savevs(3,mon)) specdamage /=2;
			 specialdam += specdamage;
			 if (specdamage) showresults(mon,-21,chare);
			}
			damage += Rand(item.vssmall);
		...
		if (mon > 9)
		{
			if (addcond)/**** weapon hit adds condition ******/
			 {
			  if (monster[mon-10].condition[whichcond-20] > -1) monster[mon-10].condition[whichcond-20] += amountcond;
			  showresults(mon,-40,chare);
			 }
		
			if (monst.condition[1]) damage = monst.stamina;
			monster[mon-10].beenattacked = TRUE;
		...
		else
		{
			if (addcond)/**** weapon hit adds condition ******/
			 {
			  if (c[mon].condition[whichcond-20] > -1) c[mon].condition[whichcond-20] += amountcond;
			  showresults(mon,-40,chare);
			 }
		
			if (c[mon].condition[1]) damage = c[mon].stamina;
			c[mon].beenattacked = TRUE;
			c[mon].stamina -= (damage + specialdam);
			c[mon].damagetaken += (damage + specialdam);
			if (c[mon].stamina < 1)
			{
			    if (c[mon].stamina > -10) c[mon].bleeding = TRUE;
				killbody(mon,0);
				showresults(mon,0,chare);
				kill = TRUE;/**** return kill ******/
			}
		}
		if (!kill) showresults(mon,0,chare);
	}
	else
	{
...
trynewweapon:
	...
	if (monst.weapon)
	{
		...
		if (item.sp1 == -10)/***** weapon inflicts condition on hit ******/
		{
		 if (item.sp2 == 1)/***** DRVs.  ******/
		 {
		  if (!savevs(item.sp4,chare)) goto autoadd;
		 }
		 else if (item.sp2 == 2)/***** % chance.  ******/
		 {
		  if (Rand(100) <= item.sp4) goto autoadd;
	...
	else item.blunt = 0;/***** erase blunt/sharp if not using a weapon ***/

	att += 50 + (5 * monst.hd) + (5 * monst.damplus) + 20*behind;
	if (monst.condition[29]) att -=10;/*** confused ***/
	if (monst.condition[27]) att -=15;/*** blind ***/
	if (monst.condition[22]) att +=10;/*** pro evil ***/
	if (monst.condition[21]) att +=15;/*** strong ***/
	if (monst.condition[2]) att -= abs(monst.condition[2]);/*** tangle ***/
	if (monst.condition[4]) att +=5;/*** bless ***/
	if (monst.condition[3]) att -=5;/*** curse ***/
	if (monst.condition[6]) att -=15;/*** slow ***/
	if (monst.condition[36]) att -= abs(monst.condition[36]);/*** hinder attack ***/
	...
	if (chare<9)
	{
		...
		if (character.condition[24]) att -=10;/*** invisible ***/
		if (character.condition[6]) att +=15;/*** slow ***/
		if (character.condition[4]) att -=5;/*** bless ***/
		if (character.condition[27]) att +=15;/*** blind ***/
		if (character.condition[29]) att +=10;/*** confused ***/
		if (character.condition[3]) att +=5;/*** curse ***/
		if (character.condition[2]) att += abs(character.condition[2]);/*** tangle ***/
		if (character.condition[37]) att += abs(character.condition[37]);/*** hinder defense ***/
		if (character.condition[38]) att -= abs(character.condition[38]);/*** defense bonus ***/
		att -= Rand(character.lu + character.maglu);/*** luck ***/
		att -= character.ac;
	...
	else
		{
			...
			if (monster[chare -10].condition[6]) att +=15;/*** slow ***/
			if (monster[chare -10].condition[4]) att -=5;/*** bless ***/
			if (monster[chare -10].condition[3]) att +=5;/*** curse ***/
			if (monster[chare -10].condition[27]) att +=15;/*** blind ***/
			if (monster[chare -10].condition[29]) att +=10;/*** confused ***/
			if (monster[chare -10].condition[24]) att -=10;/*** invisible ***/
			if (monster[chare -10].condition[2]) att += abs(monster[chare -10].condition[2]);/*** tangle ***/
			if (monster[chare -10].condition[37]) att += abs(monster[chare -10].condition[37]);/*** hinder defense ***/
			if (monster[chare -10].condition[38]) att -= abs(monster[chare -10].condition[38]);/*** defense bonus ***/
			att -= monster[chare -10].ac;
			att -= 2*abs(monster[chare -10].condition[7]);/*** shield ***/
			if (((monst.type[4]) || (monst.type[5])) && (monster[chare -10].condition[22])) att -= 10;/*** +10% for pro evil ***/
		}
...
if (att < 10) att = 10;
	...
	if (chare < 9)
	 {
	 if ((c[chare].condition[31]) && (Rand(100) < 34) && (!monster[mon-10].dist)) chare = mon;
	 }
	  else if (chare > 9)
		{
 		 if ((monster[chare-10].condition[31]) && (Rand(100) < 34) && (!monster[mon-10].dist)) chare = mon;
		}
		...
		if (success)
		{
			...
			if (monst.condition[21]) damage +=3;/***** strong ****/
			damage += monst.condition[32];/***** attack bonus ****/
			if (!monst.weapon)
			{
			 if (monst.attacks[attacknum][0]) damage += randrange (monst.attacks[attacknum][0],monst.attacks[attacknum][1]);
			  else damage += randrange (monst.attacks[0][0],monst.attacks[0][1]);
			}
			else
			{
			 damage += item.damage;
				...
				if ((item.heat) && (!monst.spellimmune[1]))
				  {
					specdamage = Rand(item.heat);
			   		if (savevs(1,chare)) specdamage /=2;
			   		if (monster[chare-10].condition[11]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-19,mon);
				  }
				if ((item.cold) && (!monst.spellimmune[2]))
				  {
					specdamage = Rand(item.cold);
			   		if (savevs(2,chare)) specdamage /=2;
			   		if (monster[chare-10].condition[12]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-20,mon);
				  }
				if ((item.electric) && (!monst.spellimmune[3]))
				  {
					specdamage = Rand(item.electric);
			   		if (savevs(3,chare)) specdamage /=2;
			   		if (monster[chare-10].condition[13]) specdamage /=2;
			   		if (specdamage) specialdam += specdamage;
			   		if (specdamage) showresults(chare,-21,mon);
				  }
				...
				if (item.heat)
				  {
					specdamage = Rand(item.heat);
			   		if (savevs(1,chare)) specdamage /=2;
			   		if (c[chare].condition[11]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-19,mon);
				  }
				if (item.cold)
				  {
					specdamage = Rand(item.cold);
			   		if (savevs(2,chare)) specdamage /=2;
			   		if (c[chare].condition[12]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-20,mon);
				  }
				if (item.electric)
				  {
					specdamage = Rand(item.electric);
			   		if (savevs(3,chare)) specdamage /=2;
			   		if (c[chare].condition[13]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-21,mon);
				  }
			}
...
doit:
			...
			if (t) /***** special attack *********/
			{
			 ...
			 if (chare < 9)
			 {
			  switch(t)
			  {
			   case 1:/**** fear ****/
			   		...
			   		{
			   		 if ((c[chare].condition[0] += abs(specdamage)) < 30) c[chare].condition[0] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 2:/**** paralyze ****/
			   		...
			   		{
			   		 if ((c[chare].condition[1] += abs(specdamage)) < 30) c[chare].condition[1] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 3:/**** curse ****/
			   		...
			   		{
			   		 if ((c[chare].condition[3] += abs(specdamage)) < 30) c[chare].condition[3] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 4:/**** stupid ****/
			   		...
			   		{
			   		 if ((c[chare].condition[5] += abs(specdamage)) < 30) c[chare].condition[5] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 5:/**** entangle ****/
			   		...
			   		{
			   		 if ((c[chare].condition[6] += abs(specdamage)) < 30) c[chare].condition[6] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 6:/**** poison ****/
			   		...
			   		{
			   		 if ((c[chare].condition[9] += abs(specdamage)) < 30) c[chare].condition[9] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 7:/**** confuse ****/
			   		...
			   		{
			   		 if ((c[chare].condition[29] += abs(specdamage)) < 30) c[chare].condition[29] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 16:/**** disease ****/
			   		...
			   		{
			   		 if ((c[chare].condition[28] += abs(specdamage)) < 30) c[chare].condition[28] += abs(specdamage);
			   		 sound(630);
			   ...
			   case 8:/**** drain spell points ****/
			   		...
			   		{
			   		 temp = monst.hd*3;
			   		 if (temp > c[chare].spellpoints) temp = c[chare].spellpoints;
			   ...
			   case 11:/**** fire damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		...
			   		if (c[chare].condition[11]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-19,mon);/****** special attack ******/
			   break;
			   ...
			   case 12:/**** cold damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		...
			   		if (c[chare].condition[12]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-20,mon);/****** special attack ******/
			   break;
			   ...
			   case 13:/**** shock damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		...
			   		if (c[chare].condition[13]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-21,mon);/****** special attack ******/
			   break;
			   
			   case 14:/**** chemical damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		if (savevs(4,chare)) specdamage /=2;
			   		if (c[chare].condition[14]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-22,mon);/****** special attack ******/
			   break;
			   
			   case 15:/**** mental damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		if (savevs(5,chare)) specdamage /=2;
			   		if (c[chare].condition[15]) specdamage /=2;
			   		specialdam += specdamage;
			   		if (specdamage) showresults(chare,-23,mon);/****** special attack ******/
			   break;
			 ...
			 else if (monster[chare-10].magres > 100) goto whiff;
			  else  /****** affect monster ******/
			  {
			   switch(t)
			   {
			   ...
			   case 8:/**** drain spell points ****/
			   		...
			   		{
			   		 temp = monst.hd*3;
			   		 if (temp > monster[chare-10].spellpoints) temp = monster[chare-10].spellpoints;
			   ...
			   case 10:/**** charm ****/
			   		if (!savevs(0,chare))
			   		{
			   		 if ((!monst.traiter) && (monster[chare-10].traiter)) numenemy--;
			   		 if ((monst.traiter) && (!monster[chare-10].traiter)) numenemy++;
			   		 monster[chare-10].traiter = monst.traiter;
			   		 monster[mon-10].target = -1;
			   		 showresults(chare,-26,mon);/****** special attack ******/

					 if (killmon >= numenemy) killbody(chare,0);/*** charmed last guy, bail out!! ***/
			   		}
			   break;
			   ...
			   case 14:/**** chemical damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		if (savevs(4,chare)) specdamage /=2;
			   		specialdam += specdamage;
			   		if (monster[chare-10].condition[14]) specdamage /=2;
			   		if (specdamage) showresults(chare,-22,mon);/****** special attack ******/
			   break;
			   ...
			   case 15:/**** mental damage ****/
			        specdamage = Rand(monst.attacks[attackloop][1]);
			   		if (savevs(5,chare)) specdamage /=2;
			   		specialdam += specdamage;
			   		if (monster[chare-10].condition[15]) specdamage /=2;
			   		if (specdamage) showresults(chare,-23,mon);/****** special attack ******/
			   break;
		  ...
		  if ((partycondition[2]) && (damage > 1) && (chare < 9))/**** barkskin & other *****/
		  {
		   ...
		   sound(-694);
			...
			if (chare < 9)
			{
				if (addcond)/**** weapon hit adds condition ******/
				 {
				  if (c[chare].condition[whichcond-20] > -1) c[chare].condition[whichcond-20] += amountcond;
				...
				if ((c[chare].stamina < 1) && (c[chare].stamina > -10)) c[chare].bleeding = TRUE;
				...
				if (addcond)/**** weapon hit adds condition ******/
				 {
				  if (monster[chare-10].condition[whichcond-20] > -1) monster[chare-10].condition[whichcond-20] += amountcond;
				  showresults(chare,-40,mon);
				 }
