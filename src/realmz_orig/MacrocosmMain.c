...
void UpdateWindow(short titleonly)
{
	...
	if (titleonly) SetRect( &dest, 40+leftshift/2,12+downshift/2,275+leftshift/2,30+downshift/2);
	 else SetRect( &dest, 0,0,320+leftshift,320+downshift);
	...
	switch( temphead )
	{
		case 1:/*** north/south ****/
			...
			for (t=0;t<14;t++)
			{
			 ...
			 if (MyrBitTstShort(&viewdung[t],14)) gPlayer.hWall[t] = 4;/**** Door ****/
			 if (MyrBitTstShort(&viewdung[t],2)) gPlayer.hWall[t] = 3;/**** Arch ****/
			 if (MyrBitTstShort(&viewdung[t],12)) gPlayer.hWall[t] = 2;/**** Stair ****/
			}						
			...
			for(t=0;t<10;t++)
			{
			 ...
			 if (MyrBitTstShort(&vviewdung[t],13)) gPlayer.vWall[ t ] = 3;/**** verticle door ***/
			 if (BitAnd(vviewdung[t],8200)) gPlayer.vWall[ t ] = 2;/**** arch stair door ***/
			}
		...
		case 2:/**** east/west ****/
			...
			{
			 gPlayer.hWall[ t ] = MyrBitTstShort(&viewdung[t],15);/**** wall ****/
			 if (MyrBitTstShort(&viewdung[t],13)) gPlayer.hWall[t] = 4;/**** horizontal door ***/
			 if (MyrBitTstShort(&viewdung[t],2)) gPlayer.hWall[t] = 3;/**** horizontal arch ***/
			 if (MyrBitTstShort(&viewdung[t],12)) gPlayer.hWall[t] = 2;/**** horizontal stair ***/
			}			
			...
			{
			 gPlayer.vWall[ t ] = MyrBitTstShort(&vviewdung[t],15);/**** wall ****/
			 if (MyrBitTstShort(&vviewdung[t],14)) gPlayer.vWall[t] = 3;/**** verticle door ***/
			 if (BitAnd(vviewdung[t],8200)) gPlayer.vWall[t] = 2;/**** verticle arch/stair ***/
			}
