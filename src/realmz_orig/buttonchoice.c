	...
	if (theControl == movelook)
	{
	 ...
	 if ((fat > 134) && (!cancamp))
	 {
	  ...
	  return(0);
		...
		{
		 GetMouse(&point);
 	 	 if (PtInRect(point,&lookrect)) updatearrow(1);
 	 	  else 
 	 	  {
 	 	   SetCCursor(sword);
 	 	   tagold = 20;
		  ...
		  if (checkdelay(1))
		  {
			...
			{
				deltax = deltay = 0;
				if (point.v < partyy *32) deltay --;
				if (point.v > partyy *32 + 32) deltay ++;
				if (point.h < partyx *32) deltax --;
				if (point.h > partyx *32 + 32) deltax ++;
				if ((!deltax) && (!deltay)) return(0);
			}
			
			nosound = FALSE;
			if (deltax == -1) boatright = -1;
			 else if (deltax == 1) boatright = 0;			

			if (deltax > 0) face = 1;
			 else if (deltax < 0) face = 0;
			
			hit = field[partyx + lookx +deltax][partyy +looky+deltay];
			oldhit = hit;
			
			if ((hit < 0) && (hit > -999)) if (solids[-hit]) solidtile = TRUE;/******** check for solid status ********/
			...
			if (hit > 999)/**** some type of door ******/
			{
			 ...
			 if (!nosound) sound(mapstats[temp].sound);
			  else
			  {
			   sound(82);
			   nosound = FALSE;
			 ...
			 if (revertgame) return(0);
				...
				if (hit < 2999)/***** normal door or detected secret area ****/
				{
				 ...
				 if (hit > 999)
				 {
					if (mapstats[hit-1000].needboad == 1) goto doboat;/**** hit boat ****/
				 }

				 if (hit < 999)
				 {
				  if (mapstats[hit].needboad == 1) goto doboat;/**** hit boat ****/
				 }
				...
				{
				 if ((hit > 200) || (oldhit < 0))  hit = basetile[lastpix];
					...
					else
					{
					 ...
					 {
					  doboat:
					  if (basescale[lastpix]) goto noboat;//*** fantasoft v7.1  NO boats indoors, bypass this code.
					   if (question(15))
					   {
					    inboat = TRUE;
					    moveparty(1);
				...
				noboat://*** fantasoft v7.1  NO boats indoors, bypass this code.
					...
					if ((!mapstats[hit].solid) && (!inboat) && (mapstats[hit].needboad != 2) && (!solidtile)) moveparty(1);
					 else if ((inboat) && (mapstats[hit].needboad == 2)) moveparty(1);
...
next:
			...
			key = 0;
  			if (checkforsecret(FALSE)) newland(partyx+lookx,partyy+looky,0,TRUE,0);
		  }
		}
...
SetPort(GetWindowPort(screen));
						 ...
						 if ((c[t].canheal) && cancast(t,0))
						  {
							...
							if (holdover[tt].stamina > holdover[tt].staminamax) holdover[tt].stamina = holdover[tt].staminamax;
						  }
						}
					}
   ...
   if (theControl == overviewbut)/*************** make scroll ****************/
   {
	...
	{
		if (!c[charselectnew].spellcastertype) warn(15);
		else
		{
			sound(6001);
	 ...
	 else if (!incamp) /****** random choice & search ******/
	 {
	  ...
	  {
	  	delay(1);
	    if (checkforsecret(TRUE)) newland(partyx+lookx,partyy+looky,0,TRUE,0);
	    if (revertgame) return(0);
	    timeclick(1,TRUE);
	    if (revertgame) return(0);
	    tickcheck();
		...
		if (globalmacro[5])/*** activate global temple macro if any ****/
		{
		 ...
		 else /**** check for seamless encounter ****/
		 {
		    ...
		    temp = 0;
			       ...
			       if ((PtInRect(test,&randlevel.randrect[t])) && (randlevel.percent[t] < 0))
			       {
			        for(tt=0;tt<3;tt++) 
			        {
			         if (Rand(100) <= abs(randlevel.randdoorpercent[t][tt])) 
			         {
			          ...
			          if (randlevel.randdoorpercent[t][tt] > 0) randlevel.randdoorpercent[t][tt] = 0;
			         }
			       }
			      }
			    }
			    ...
			    seemless = FALSE;
			    if (reply == -1) goto godooritem;/******** activate door item *****/
			    return(0);
		 }
	}
...
godooritem:
	 ...
	 if (reply < 0) return(reply);/**** goto shop ***/
	 
	 if (!indung) updatemain(FALSE,-1);
	  else
	  {
	   updatemain(TRUE,-1);
	   threed(-1L,0,0,0);
	...
	if (theControl == castspellsbut)
	{
		...
		sound(141);
		if (!cancast(charselectnew,0)) return(0);
		scribing = 1;
		if (!inspell)
		{
		 ploticon3(129,r);
...
castincamp:
		...
		if (castnum > -1)
		{
			if (!skip) updatemain(TRUE,-1);
			updatecharshort(charselectnew,FALSE);
			cleartarget();
			temp = 0;
			if (spellinfo.targettype > 2) temp = charnum;
			if (!spellinfo.targettype) temp = powerlevel-1;

			didnpc = FALSE;
			if (spellinfo.targettype == 5) track[charselectnew] = TRUE;
			 else if (temp < charnum)
			  {
				textbox(3,47,0,0,textrect);
				if (!getchoice(temp,0,TRUE)) return(0);
			  }
			  else
			  {
			   for(t=0;t<= charnum;t++) track[t] = TRUE;
			...
			if (didnpc) for(t=0;t<heldover;t++) holdover[t] = monster[t];
			  
			if (spellinfo.targettype == 11) sound(spellinfo.sound2+600);
			delay(20);
			EraseRect(&textrect);
			inspell = fastspell = memoryspell = FALSE;
			updatecontrols();
		}
		else updatemain(TRUE,-1);
	}
