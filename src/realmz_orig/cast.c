...
void cast(short targetnum,short who)
{
	...
	if (spellinfo.spellclass != 9) ploticon3(11992+spellinfo.spelllook1*8,spell);
	times = 128 + delayspeed;
	
	if ((showcast) && (spellinfo.targettype != 6) && (spellinfo.spellclass != 9) && (targetnum > 1)) times = 30 + delayspeed;
	
	curControlValue = spellinfo.cost*powerlevel;/***** spell energy used *****/
	if (spellinfo.targettype == 9) curControlValue = 0;
	
	if (powerlevel < 1) powerlevel = 1;
	
	if (spellinfo.targettype == 0) curControlValue /= powerlevel;
...
recalc:
		...
		if (who < 9)
		{
		...
		else
			{
				...
				if ((monster[monsterup].size == 1) || (monster[monsterup].size == 3)) orig.top -= 16;
				if (monster[monsterup].size >1) orig.left -= 16;
			}
			...
			else
			{
				if (target[loop][1] == 2)
				{
				 ...
				 if ((temp == 1) || (temp == 3)) dest.top -=16;
				 if (temp > 1) dest.left -=16;
				}
				else
				{
...
pushon:
			...
			if ((spellinfo.targettype != 6) && (!bad)) spelltargets(loop,-1,0,0);
			 else if (spellinfo.targettype == 6)
				{
				 inspell = FALSE;
				 if (q[up] < 10) centerfield(5+(2*screensize),5+screensize);
				}
			target[loop][0] = target[loop][1] = -1;
