...
void resolvespell(void)
{
	...
	if (spellinfo.powerdur1) for(t=0;t<powerlevel;t++) duration += randrange(spellinfo.powerdur1,spellinfo.powerdur2);
	...
	if ((q[up] < 6) && (q[up] > -1) && (!monsterturn))
	{
		if (spellinfo.spellclass == 9)/**** Missile Bonus Damage ****/
		{
		 ...
		 if (caste.getsmissilebonus) damage += randrange(1,(c[charup].level/2));
		}
	}
	...
	{
	 sound(647);
	 for(t=0;t<=charnum;t++) if (track[t])
	 {
	  for(tt=0;tt<=c[t].numitems;tt++) c[t].items[tt].ident = TRUE;
	 }
	}
		...
		if (special == 50) /**** light *****/
		{
		 sound(601);
			if (powerlevel * 30 > partycondition[0])
			{
			 ...
			 if (FrontWindow() == look) centerpict();
			}
			 inspell = 0;
			 {
                 CGrafPtr thePort = GetQDGlobalsThePort();
    			 if ((thePort == GetWindowPort(screen)) && (checkforitem(805,FALSE,-1))) upbutton(TRUE);
    	     }
			 return;
		}
		else
		{
			if (duration > partycondition[special]) partycondition[special] = duration;
			inspell = 0;
			if (special == 4)
			{
			 if (indung)
	...
	{
	 if (!spellinfo.size) spellinfo.size = Rand(5);
	 for(t=0;t<=charnum;t++) if (track[t])
	 {
	   ...
	   *attr +=1;
		  ...
		  switch(spellinfo.size)
		  {
		   ...
		   case 2:/***** Knowledge ****/
		   		if (c[t].in > 15) c[t].magres += caste.magres;
		   break;
		  
		   case 3:/***** Judgment ****/
		   		if (c[t].wi > 15) c[t].magres += caste.magres;
		   break;
		  
		   case 4:/***** Agility ****/
		   		if (c[t].de > 14) c[t].ac +=2;
		   break;

		   case 5:/***** Vitality ****/
		   		if (c[t].co > 18) for(tt=0;tt<8;tt++) c[t].save[tt]+= 5;
		   break;
		   ...
		   case 11:/***** Spell Points ****/
		   		if (c[t].spellpointsmax > 0) c[t].spellpointsmax += Rand(20);
		   break;
		  }
		 }
		 else  warn(79);
	...
	if (special == 56) /******* phase *******/
	{
	 if (canundo)
	 {
		...
		if (temp > 999)
		{
		 if (mapstats[temp-1000].solid) goto phaseintosolid;
		}
...
phaseintosolid:
		...
		if (!spellinfo.size) c[charup].attacks = 0;
	...
	if ((spellinfo.targettype == 9) && (incombat)) flashmessage((StringPtr)"This spell affects all those friendly to the caster.",305,322,60,6001);
	  else if ((spellinfo.targettype == 10) && (incombat)) flashmessage((StringPtr)"This spell affects all enemies of the caster.",305,322,60,6001);
	...
	if ((quickshow) && ((spellinfo.targettype == 3) || (spellinfo.targettype == 4) || (spellinfo.targettype > 8)))
	{
	 if ((!collideflag) && (incombat))
	 {
	  ...
	  if (incombat) spelleffect(temp,TRUE);
	 }
	}
...
doagain:
			...
			if ((t < 9) && (incombat))
			{
			 if ((t != q[up]) && (c[t].spellpointsmax) && (c[t].condition[35]) && (q[up] > 9))
			 {
			  ...
			  if (c[t].spellpoints > c[t].spellpointsmax) c[t].spellpoints = c[t].spellpointsmax;
			 }
			}
			 else if (incombat)
			{
			  monst = monster[t-10];
			 if ((t != q[up]) && (monster[t-10].spellpoints) && (monster[t-10].condition[35]) && (q[up] < 9)) monster[t-10].spellpoints += curControlValue;
			}

			doesresist = resist(t);
		    if ((collideflag) && (collidecheck[collideflag])) goto out;
			...
			if ((!doesresist) || (!incombat))
			{
				if (t<9)  /**** check resistances ****/
				{
					if ((damagetype < 8) && (damagetype))
					{
						...
						if ((damage) && (c[t].condition[10+damagetype])) damage /=2;
				...
				else if ((damagetype < 8) && (damagetype))
					{
						...
						if ((damage) && (monst.condition[10+damagetype])) damage /=2;
	...
	immune:
				...
				{
				 if (special == 28) damage = adjdam = duration;/*** disease ***/
				 
					if (t > 9)
					{
					 if (abs(monster[t-10].condition[special-1] + duration) < 125)
					  {
						 if (monster[t-10].condition[special-1] > -1) monster[t-10].condition[special-1] += duration;
					  }
					}
					else if ((c[t].condition[special-1] + duration) < 100)
					{
						 if (c[t].condition[special-1] > -1) c[t].condition[special-1] += duration;
				    }
				}
				else if (special > 99)
					{
						if (t > 9) monster[t-10].condition[special-101] = 0;
						 else
						 {
				...
				if (spelllist(t,special))
				{
					if ((!adjdam) && (special))
					{
						...
						if (!usequickshow) spelleffect(t,0);
					}
				}
				
				if (special == 49) adjdam = damage;
				
				if ((adjdam) && (!damage) && (special != 60) && (special != 59)) damage = 1;
				...
				if (t < 9)
				{
				 ...
				 {
					sound(600+spellinfo.sound2);
					if (damage < 0) heal(t,abs(damage),FALSE);
					else
					{
						if ((c[t].stamina < 0) && (damage > 0)) continue; /***** dont do damage to unconsious characters *****/
						c[t].stamina -= damage;
						select[t] = -1;
						if (damage > 0) c[t].beenattacked = TRUE;
				...
				else if ((adjdam) && (t > 9))
					{
						...
						if ((q[up] < 6) && (q[up] > -1)) c[charup].damagegiven += damage;
	 					if ((q[up] < 6) && (q[up] > -1) && (spellclass == 9)) c[charup].hitsgiven ++;

						if (monster[t-10].stamina <= 0)
						{
						 if ((q[up] < 6) && (q[up] > -1)) c[charup].kills++;
						 killbody(t,0);
						}
						else
						{
							if (damage) showresults(t,0,q[up]);
							if (!usequickshow) spelleffect(t,0);
							if ((spellclass == 9) && (misslecount++ < spellinfo.fixedtargetnum))
							{
							    if (killmon == numenemy) return;
								goto doagain;
							}
						}
					}
			...
			else
			{
				...
				{
					if ((q[up] < 6)  && (q[up] > -1)) c[charup].imissed++;
			...
			if (incombat)
			{
			  if ((inspell) && (spellinfo.targettype > 8)) delay(5);
			    else delay(0);
			}
		}
	}
