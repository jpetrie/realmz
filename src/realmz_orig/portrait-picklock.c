...
void portrait(short mode)
{
 ...
 if (characterl.gender==1) MyrCDiStr(73,(StringPtr)"Male");
	else MyrCDiStr(73,(StringPtr)"Female");
 ...
 for(t=3;t<9;t++)
 {
  ...
  if (iconhand != NIL)
  {
   if ((**iconhand).iconBMap.bounds.bottom == 32) InsetRect(&buttonrect,6,6);
 ...
 updatelight:
  ...
  if (index == 96) MyrCDiStr(79,(StringPtr)"Misc. Humanoid");
...
short picklock(short who,short type)
{
 ...
 if ((who < 0) || (who > charnum)) return(0);

 tumblers = thief.tumblers;
 if (tumblers > 6) tumblers = 6;
 ...
 for(t=0;t<6;t++) one[t] = 8 + yellow/2;
 ...
 limit = (start + tumblers * 120) + 180;
		...
		while (!Button())
		{
		 ...
		 {
		  one[t] += delta;
		  if (one[t] > 208) one[t] = 208;
		 }
		  else
		  {
		   one[t] -= delta;
		   if (one[t] < 10) one[t] = 10;
		   temprect.left = one[t];
		   EraseRect(&temprect);
		  }
		 ...
		 ForeColor(redColor);
		 r1.right = 8 + one[t];
		 if (r1.right > yellow) r1.right = yellow;
		 PaintRect(&r1);
		 ...
		 if (one[t] > yellow)
		 {
		  ForeColor(yellowColor);
		  r1.right = 8 + one[t];
		  if (r1.right > green) r1.right = green;
		  r1.left = yellow;
		  PaintRect(&r1);
		 }
	...
	delay(50);
	reply = TRUE;
	for(t=0;t<tumblers;t++) if (one[t] < yellow) reply = FALSE;
