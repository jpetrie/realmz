...
short newland(long x,long y,short mode,short modecode,short data)
{
 ...
 if (mode == -1) goto startover;/***** monster macro *****/
 ...
 if (mode == 2)/****** trigger simple encounter *****/
 {
  ...
  for(t=0;t<8;t++) infodoor.code[t] = infodoor.id[t] = 0;
 ...
 if (!indung) doorid = landlevel*10000 + x + y*100;
  else doorid = dunglevel*10000 + x + y*100; 
 ...
 moveon:
           ...
           if (infodoor.percent < 1) return(FALSE); /****** disabled door ****/
           if  (Rand(100) > infodoor.percent) return(FALSE);
         ...
         startcode:
			...
			if ((code < 0) && (code != -14) && (code != -23))
			{
			 ...
			 else if (stackindex == 0) gosub = FALSE;
			...
			case 112://***** POP Stack
				if (stackindex > 0) stackindex--;//*** just decrease the stack index
			break;

             case 76:/***** Increment ú Decrement Quest Value *****/
				...
				if (extracode[3])/**** looks to have an autobranch ***/
				{
				 if (quest[extracode[0]] >= extracode[3])
				 {
				  switch(extracode[2])
					{
              	     case 1://**** X-AP
						 if (gosub) pushonstack(infodoor,t);
             ...
             case 77:/***** Branch on Quest Value *****/
				...
				loadextracode(id);
				if (quest[extracode[0]] >= extracode[1]) reply = TRUE;
					else reply = FALSE;
             ...
             case 78:/***** Branch on Tile Parameters *****/
				...
				if (temp > 999) temp -= 1000;//*** wipe out door marker
				...
				{
					case 1:
						if (holdstatbase.shore) reply = TRUE;
					break;

					case 2:
						if (holdstatbase.needboad) reply = TRUE;
					break;

					case 3:
						if (holdstatbase.ispath) reply = TRUE;
					break;

					case 4:
						if (holdstatbase.los) reply = TRUE;
					break;

					case 5:
						if (holdstatbase.flyfloat) reply = TRUE;
					break;

					case 6:
						if (holdstatbase.forest) reply = TRUE;
					break;

					case 7:/*** specific tile id ****/
						if (temp == extracode[1]) reply = TRUE;
					break;
             ...
             case 122:/***** cause fumble *****/
				 ...
				 {
                  if (extracode[1]) sound(extracode[1]);
                 ...
                 if (incombat)
                 {
                  ...
                  spellinfo.spelllook2 = 5;
					if (nummon < maxmon)
					{
					 ...
					 if (extracode[2] < 0) extracode[2] = Rand(abs(extracode[2]));
					...
					switch(extracode[4])
					{
					 case 0://*** default to old style rules
							 if (doingque) buildmonster(todoque[abs(data)-1].traiter);
							  else if (!battlemacro) buildmonster(monster[macromonster].traiter);
							   else buildmonster(-1);
					 break;
					    ...
					    for(ud=start;ud<stop;ud++)
						  {
							...
							if (PtInRect(point,&goodrect))
							 {
								...
								{
								 if (((mapstats[monster[nummon].underneath[1][1]-1000].solid) && (!monster[nummon].size)) || (mapstats[monster[nummon].underneath[1][1]-1000].solid == 2)) goto yep2;
								}
								else goto yep2;
							    ...
							    if ((size == 1) || (size == 3))
								{
									...
									{
									 if (mapstats[monster[nummon].underneath[1][0]-1000].solid > 1) goto yep2;
									}
									else goto yep2;
								}
							    ...
							    if (size == 3)
								{
									...
									{
									 if (mapstats[monster[nummon].underneath[0][0]-1000].solid > 1) goto yep2;
									}
									else goto yep2;
								}
							    ...
							    if (size > 1)
								{
									...
									{
									 if (mapstats[monster[nummon].underneath[0][1]-1000].solid > 1) goto yep2;
									}
									else goto yep2;
								}
							...
							good3:
							if (extracode[3]) sound(extracode[3]);
						    showresults(nummon+10,-12,nummon+10);
						    spelleffect(nummon+10,0);
						    if (monster[nummon].traiter) numenemy++;
             ...
             case 125:/***** destroy related monsters *****/
                 ...
                 {
                  loadextracode(id);
                  if (!extracode[1]) extracode[1] = 100;
                  temp = 0;
					for(tt=0;tt<maxmon;tt++)
					{
			...
			case 126:/***** Battle combatround macro *****/
 				 ...
 				 if (battle.battlemacro > 0) return(0);/***** not a valid code, stuck in the old days ****/
 				 
 				 loadextracode(id);
 				 
 				 if (extracode[0] == 1)
 				 {
 				  if (Rand(100) > extracode[1]) return(0);
 				 }
 				 else if ((!extracode[0]) && ((combatround-1) != extracode[1])) return(0);
 				 
 				 
 				 if (extracode[2] != 1) battle.battlemacro = 0;//** fantasoft v7.1 /**** single time macro ****/
 				 
 				 
 				 /***** activate standard macro *****/
 				
 				 if (extracode[2] == 2) loaddoor2(randrange(extracode[3],extracode[4])); /**** branch to random macro *****/
 				  else loaddoor2(extracode[3]);
             ...
             case 127:/***** Continue if Monster Present *****/
 				...
 				for(tt=0;tt<nummon;tt++)
 				{
 				 if ((monster[tt].name == id) && (monster[tt].stamina > 0)) reply = TRUE;
 				}
				  ...
				  switch(extracode[0])
					{
					 case 1://****** alter attacks/round
						...
						if (c[tt].attackbonus < 0) c[tt].attackbonus = 0;
					 break;
					 ...
					 case 2://****** alter spells/round
						if (c[tt].maxspellsattacks)
						{
						 ...
						 if (c[tt].maxspellsattacks < 1) c[tt].maxspellsattacks = 1;
						}
					 break;

					 case 3://****** alter movement
						c[tt].movementmax += extracode[1];
						if (c[tt].movementmax < 3) c[tt].movementmax = 3;
					 break;

					 case 4://****** alter damage
						c[tt].damage += extracode[1];
						if (c[tt].damage < 0) c[tt].damage = 0;
					 break;
					 ...
					 case 5://****** alter spellpoints
						if (c[tt].spellpointsmax)
						{
						 ...
						 if (c[tt].spellpointsmax < 0) c[tt].spellpointsmax = 0;
						}
					 break;
					 ...
					 case 6://****** alter hand 2 hand
						if (c[tt].handtohand)
						{
						 ...
						 if (c[tt].handtohand < 1) c[tt].handtohand = 1;
						}
					 break;

					 case 7://****** alter stamina
						c[tt].staminamax += extracode[1];
						if (c[tt].staminamax < 2) c[tt].staminamax = 2;
					 break;

					 case 8://****** alter AR
						c[tt].ac += extracode[1];
						if (c[tt].ac < 0) c[tt].ac = 0;
					 break;

					 case 9://****** alter ToHit
						c[tt].tohit += extracode[1];
						if (c[tt].tohit < 2) c[tt].tohit = 2;
					 break;

					 case 10://****** alter ProJo ToHit
						c[tt].missile += extracode[1];
						if (c[tt].missile < 2) c[tt].missile = 2;
					 break;

					 case 11://****** alter Magic Resistance
						c[tt].magres += extracode[1];
						if (c[tt].magres < 0) c[tt].magres = 0;
					 break;
			 ...
			 case 107:/************ improved selective battle ***********/
			  ...
			  for(tt=0;tt<8;tt++)/**** save old door codes ****/
			  {
               ...
               loadextracode(id);
               if (extracode[2]) sound(extracode[2]);
               if (extracode[3]) textbox(-1,extracode[3],0,0,textrect);
                
               if (!extracode[1]) battlenum = extracode[0];
                else battlenum = randrange(extracode[0],extracode[1]);
                
               branchdoorcode = extracode[4];

               combat(0,1);
               
               if (revertgame) return(0);
               
               
               lastpix = -1;
               if (indung) loadland(dunglevel,TRUE);
                else loadland(landlevel,TRUE);
			  ...
			  for(tt=0;tt<8;tt++)/**** restore old door codes ****/
			  {
               ...
               for(tt = 0;tt<=charnum;tt++) c[t].inbattle = TRUE;
               updatemain(FALSE,0);
               
               if (coward) goto startover;
			 break;
             ...
             case 106:/***** Set Darkland Status *****/
				 	...
				 	if ((extracode[1]) && (randlevel.isdark == extracode[0]-1)) return(0);/*** discontinue codes ***/
             ...
             case 97:/****** Allow Full Map ********/
             	...
             	if (!multiview) warn(96);
             	multiview = TRUE;
             	
             	if (viewtype == 1) UpdateWindow(TRUE);
             	             	
             break;
            
             case 96:/****** Require 3D Map ********/
             
             	if (multiview) warn(97);
             ...
             case 92:/****** Alter Size of Random Rect ********/
                 ...
                 if (indung) saveland(dunglevel);
                  else saveland(landlevel);
				 ...
				 if (!indung) loadland(landlevel,0);
				  else loadland(dunglevel,0);
             ...
             case 90:/***** Take away Victory ****/
             	...
             	switch(extracode[1])
             	{
             	 case 1:/*** take from picked ****/
             	 	for(tt=0;tt<=charnum;tt++)
             	 	{
             	 	 if (track[t]) c[tt].exp -= extracode[0];
             	 	}
             	 break;
            ...
            if (heldover < 20)
            {
				...
				for(temp=0;temp<holdover[heldover].hd;temp++) holdover[heldover].stamina += Rand(8);
				...
				for(temp=0;temp<6;temp++) holdover[heldover].save[temp] += 7*howhard;
...
checkforduplicates:
             ...
             case 87:/************ Branch On Allies In Party ***********/
	            ...
	            tempholdovername = extracode[0];//**** fantasoft v7.1
             	...
             	for(temp=0;temp<heldover;temp++) if (holdover[temp].name == tempholdovername) reply = TRUE;//**** fantasoft v7.1
             ...
             case 86:/************ Branch on Misc. ***********/
                ...
                switch(extracode[0])
                {
		                  case 0:/****** class present *********/
							for(tt=0;tt<=charnum;tt++)
							{
							 if (extracode[1] < 0)
							 {
							  if ((c[tt].caste == abs(extracode[1])) && (track[tt])) reply = TRUE;
							 
							 }
							 else if (c[tt].caste == extracode[1]) reply = TRUE;
							}
		                  break;
		             	  ...
		             	  case 1:/**** race present ****/
							for(tt=0;tt<=charnum;tt++)
							{
							 if (extracode[1] < 0)
							 {
							  if ((c[tt].race == abs(extracode[1])) && (track[tt])) reply = TRUE;
							 
							 }
							 else if (c[tt].race == extracode[1]) reply = TRUE;
							}
		             	  break;
		             	  ...
		             	  case 2:/**** gender present ****/
							for(tt=0;tt<=charnum;tt++)
							{
							 if (extracode[1] < 0)
							 {
							  if ((c[tt].gender == abs(extracode[1])) && (track[tt])) reply = TRUE;
							 
							 }
							 else if (c[tt].gender == extracode[1]) reply = TRUE;
							}
		             	  break;

		             	  case 3:/**** inboat ****/
							if (inboat) reply = TRUE;
		             	  break;

		             	  case 4:/**** incamp ****/
							if (incamp) reply = TRUE;
		             	  break;
		             	  ...
		             	  case 5:/**** caste class present ****/
							for(tt=0;tt<=charnum;tt++)
							{
							 ...
							 if (extracode[1] < 0)
							 {
							  if ((caste.casteclass == abs(extracode[1])) && (track[tt]))  reply = TRUE;
							 }
							 else if (caste.casteclass == extracode[1]) reply = TRUE;
							}
		             	  break;
		             	  ...
		             	  case 6:/**** race class present ****/
							for(tt=0;tt<=charnum;tt++)
							{
							 ...
							 if (extracode[1] < 0)
							 {
							  if ((MyrBitTstShort(&races.descriptors,abs(extracode[1])-1)) && (track[tt])) reply = TRUE;
							 }
							 else if (MyrBitTstShort(&races.descriptors,extracode[1]-1)) reply = TRUE;
							}
		             	  break;
				...
				if (reply == TRUE) id = extracode[3];
				  else if (extracode[4]) id = extracode[4];
             ...
             case 85:/************ Branch To Random Door ***********/
	            ...
	            if (extracode[3]) sound(extracode[3]);
                if (extracode[4]) textbox(-1,extracode[4],0,0,textrect);
             ...
             case 81: /******* Branch on PC Condition *******/
                 ...
                 switch(extracode[1])
                 {
                  case 0:/**** check whole party ****/
                  		for(tt=0;tt<=charnum;tt++) if (!c[tt].condition[extracode[0]]) reply = FALSE;
                  break;
                  ...
                  case -1:
						for(tt=0;tt<= charnum;tt++)
						{
						 if (track[tt])
						 {
						  if (!c[tt].condition[extracode[0]]) reply = FALSE;
						 }
						}              
                  break;
                  
                  default:
						if (!c[extracode[1]].condition[extracode[0]]) reply = FALSE;
                  break;
                 }
             ...
             case 2:/***** battle ****/
			  ...
			  for(tt=0;tt<8;tt++)/**** save old door codes ****/
			  {
               ...
               if (!extracode[1]) battlenum = extracode[0];
                else battlenum = randrange(abs(extracode[0]),abs(extracode[1]));
                
                
               if (extracode[0] < 0) battlenum = -(abs(battlenum));//** fantasoft v7.1
               //** above line accounts for - battle id in a Range of IDs and allows surprise
                
               if (extracode[4] == 10) revivepartyflag = TRUE;
                else revivepartyflag = FALSE;
               ...
               if (revivepartyflag)//Fantasoft v7.1
               {
				...
				if (!indung) loadland(landlevel,1);
	    			else loadland(dunglevel,1);
...
battledone:
             ...
             case 4:/******************* simple encounter *********************/
				...
				if ((fp = MyrFopen(":Data Files:CE","rb")) == NULL) scratch(124);
	        ...
	        type1comeback:
	           ...
	           if (enc.choiceresult[0] == -4) reply = 4;/********* auto goto result 4 ****/
	           else
	           {
	           ...
	           if (!reply) /***** cancel ******/
	            {
	              ...
	              if (!mode) door[doornum] = infodoor = holddoor;/**** dont do random door ***/
	              if ((!indung) && (!seemless)) moveparty(-1);
                  return(TRUE);
	            }
	            else
	             {
             ...
             type2new:
				...
				if ((fp = MyrFopen(":Data Files:CE2","rb")) == NULL) scratch(125);
	           ...
	           if (reply == -1) return(-1);/*** activate door item ******/
	           ...
	           if (!reply) /***** cancel ******/
	            {
	            ...
	            else
	             {
	              ...
	              if ((reply == 4) && (enctry == 1) && (enc2.maxtimes > 1)) reply = 3;/**** change to timeout ***/
	              for(tt=0;tt<8;tt++)
	               {
			...
			loopback:
             ...
             case 7: /********** Action Data Data With X-AP Data ********/
                 ...
                 {                  
                  case -1:/********* Replace Simple Result Code ************/
                  	  loaddoor2(extracode[2]);
	                  if ((fp = MyrFopen(":Data Files:CE","r+b")) == NULL) scratch(853);
                  ...
                  case -2:/********* Replace Complex Result Code ************/
                  	  loaddoor2(extracode[2]);
	                  if ((fp = MyrFopen(":Data Files:CE2","r+b")) == NULL) scratch(125);
                  ...
                  default:
					...
					for(tt=0;tt<8;tt++)
					{
	                 ...
	                 if (indung) loadland(dunglevel,0);
	                  else loadland(landlevel,0);
	               break;
	              }
             ...
             case 10:/***** give treasure ****/
                 getfilename("Data TD");
		         if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(126);
             ...
             case 12:/***** new land icon ******/
                 ...
                 if (indung) field[extracode[2]][extracode[1]] = extracode[3];
                  else field[extracode[1]][extracode[2]] = extracode[3];
             ...
             case 13: /******* enable / disable door *****/
                 ...
                 if (extracode[3] < 0) indung = TRUE;
                  else if (extracode[3] > 0) indung = FALSE;
                  
                 loadland(templong,0);
                 if (extracode[1]) door[extracode[1]].percent = extracode[2];
             ...
             case 15:/***** give damage/heal to picked characters ****/
                 ...
                 for(tt=0;tt<=charnum;tt++) if (track[tt])
                 {
                  if (extracode[3]) sound(extracode[3]);
                  heal(tt,extracode[0] * randrange(extracode[1],extracode[2]),1);
                 }
                 if (extracode[4]) textbox(-1,abs(extracode[4]),0,0,textrect);
             break;
             ...
             case 17:/***** cast spell on picked ****/
              	 ...
              	 if (extracode[3]) spellinfo.cannot = 3;/**** force affect ****/
             ...
             case 18:/***** cast spell on party ****/
              	 loadextracode(id);
              	 for(tt=0;tt<maxloop;tt++) track[tt] = 0;
              	 for(tt=0;tt<=charnum;tt++) track[tt] = 1;
              	 loadspell2(extracode[0]);
              	 powerlevel = extracode[1];
              	 spellinfo.saveadjust += extracode[2];
              	 if (extracode[3]) spellinfo.cannot = 3;/**** force affect ****/
            ...
            teleport:
                 ...
                 if (indung) saveland(dunglevel);
              	 ...
              	 if (indung)
              	 {
              	  if (extracode[0] > -1) dunglevel = extracode[0];
              	  if (extracode[1] > -1) x = floorx = extracode[1];
              	  if (extracode[2] > -1) y = floory = extracode[2];
              	  infodoor.landid = dunglevel;
              	  loadland(dunglevel,0);
              	 }
              	 else
              	 {
              	  if (extracode[0] > -1) landlevel = extracode[0];
              	  if (extracode[1] > -1) x = lookx = extracode[1];
              	  if (extracode[2] > -1) y = looky = extracode[2];
              	  partyx = partyy = 0;
              	  infodoor.landid = landlevel;
              ...
              if (code == 45) goto endteleport;/***** teleport only ***/
              
              	 infodoor.landid = landlevel;
              	 seconddoor = TRUE;
              	 if (abs(field[x][y]) > 999) goto recheck;
              	 ...
              	 if (checkforcharge(extracode[0]) >= extracode[2])
              	  {
	 	             ...
	 	             case 1:
              	     	 if (gosub) pushonstack(infodoor,t);
	 	             	 id = extracode[3];
	 	             	 if (id != -1) goto type1new;
	 	             break;
	 	             	
	 	             case 2:
              	     	 if (gosub) pushonstack(infodoor,t);
	 	             	 id = extracode[3];
	 	             	 if (id != -1) goto type2new;
	 	             break;
	 	           }
              	 }
              	 else
             ...
             case 68:/***** Alter Party Fatigue ****/
              	 loadextracode(id);
              	    switch(extracode[0])
              	    {
              	     case 1:
	                     fat = 135;
	                     if (FrontWindow() == look) updatefat(TRUE,0,TRUE);
	 	             break;
             ...
             case 23:/***** alter rand rect info (Land) ****/
				 ...
				 if (extracode[3] > -1) randlevel.battlerange[temp1][0] = extracode[3];
				 if (extracode[4] > -1) randlevel.battlerange[temp1][1] = extracode[4];
				 saveland(templong);
        ...
        case -23:/***** alter rand rect info (dungeon) ****/
				 ...
				 if (picture) 
				 {
					 ...
					 if ((itemRect.bottom < (320 + downshift)) || (itemRect.right < (320 + leftshift)))
					 {
					  ...
					  itemRect.right += itemRect.left;
				  	 ...
				  	 EraseRect(&lookrect);
					 if (picture) DrawPicture(picture,&itemRect);
				  }
             break;
             ...
             case 66: /******* Disable/Enable Camping *******/
             	 ...
             	 if (temp != id)
             	 {
             	  if (!id) flashmessage((StringPtr)"You may now camp again.",50,50,0,6001);
             	   else flashmessage((StringPtr)"You may not camp at the present time.",50,50,0,6001);
             	 }
             break;
             ...
             case 29: /******* Give/Display Map *******/
             	 ...
             	 if (id > -1) flashmessage((StringPtr)"You gain a map, to view the map use Maps/Notes in the Menu.",30,355,0,30005);
             	  else showmap(abs(id));
             break;
                                  
             case 30: /******* Set Picked on Check Vs. Special Ability or Attribute *******/
                 loadextracode(id);
                 if (extracode[2] == 1) for(temp=0;temp<=charnum;temp++) track[temp] = TRUE;
                	 if (extracode[2] == 2) for(temp=0;temp<=charnum;temp++) if (c[temp].stamina > 0) track[temp] = TRUE;
             	 for(loop=0;loop<=charnum;loop++) if (track[loop])
             	 {
             	  ...
             	  if (extracode[3])/************** check vs. attribute ***********/
             	  {
             	   temp = (Rand(25) - extracode[1]);
	                  ...
	                  {
	                    case 0:/*** brawn ****/
	                  		if (temp < c[t].st) track[loop] = TRUE;
	                    break;
	                  
	                    case 1:/*** intelligence ****/
	                  		if (temp < c[t].in) track[loop] = TRUE;
	                    break;
	                  
	                    case 2:/*** wisdom ****/
	                  		if (temp < c[t].wi) track[loop] = TRUE;
	                    break;
	                  
	                    case 3:/*** dexterity ****/
	                  		if (temp < c[t].de) track[loop] = TRUE;
	                    break;
	                  
	                    case 4:/*** consitition ****/
	                  		if (temp < c[t].co) track[loop] = TRUE;
	                    break;
	                    ...
	                    case 6:/*** luck ****/
	                  		if (temp < c[t].lu) track[loop] = TRUE;
	                    break;
	                  }
					if (extracode[0] < 0)/*** reverse pick *****/
					{
					 if (track[loop]) track[loop] = 0;
						else track[loop] = TRUE;
					}
           	  	  }
             	  else/************** check vs. special ability ***********/
             	  {
             	   if (Rand(100) > c[t].spec[abs(extracode[0])] + extracode[1]) track[loop] = FALSE;
             	    else track[loop] = TRUE;
             	  }
             	 }
				  ...
				  if (extracode[0] < 0)/**** set on fail if desired *****/
				  {
	             	 for(loop=0;loop<=charnum;loop++)
	             	 {
	             	  if (track[loop]) track[loop] = FALSE;
	             	   else track[loop] = TRUE;
	             	 }
             	 }
             break;
             ...
             case 31: /******* Branch on Check Vs. Ability *******/
                 ...
                 loadextracode(id);
				 for(temp = 0;temp <= charnum;temp++) if (track[temp]) track[0] = temp;
                 if (extracode[2])
                 {
                  temp = (Rand(25) - extracode[1]);
                  switch(extracode[0])
                  {
                    case 0:/*** brawn ****/
                  		if (temp < c[track[0]].st) goto goodcheck;
                  		 else goto badcheck;
                    break;
                  
                    case 1:/*** intelligence ****/
                  		if (temp < c[track[0]].in) goto goodcheck;
                  		 else goto badcheck;
                    break;
                  
                    case 2:/*** wisdom ****/
                  		if (temp < c[track[0]].wi) goto goodcheck;
                  		 else goto badcheck;
                    break;
                  
                    case 3:/*** dexterity ****/
                  		if (temp < c[track[0]].de) goto goodcheck;
                  		 else goto badcheck;
                    break;
                  
                    case 4:/*** consitition ****/
                  		if (temp < c[track[0]].co) goto goodcheck;
                  		 else goto badcheck;
                    break;
                    ...
                    case 6:/*** luck ****/
                  		if (temp < c[track[0]].lu) goto goodcheck;
                  		 else goto badcheck;
                    break;
                  }
                 }
             ...
             case 33:  /**************** take gold *************/
             	 ...
             	 if (extracode[0] > 0) reply = takegold(extracode[0],0);
             	  else reply = takegold(abs(extracode[0]),1);
             	 
             	 if ((!reply) && (extracode[1] == -1)) t = 6;
             ...
             case 36: /********** Store / Give Equipment **************/
             	...
             	if ((id) && (!storeditems))/********** take parties equipment *************/
             	{
             	 ...
             	 for(tt=0;tt<=charnum;tt++)
             	 {
             	  ...
             	  for(loop=0;loop<30;loop++) dropitem(tt,c[tt].items[0].id,0,0,TRUE);
             	  for(loop=0;loop<30;loop++) c[tt].items[loop] = blank100;
             	  c[tt].numitems = 0;
             	 }
             	}
             	else if ((!id) && (storeditems))/********** give parties equipment back *************/
             	{
             	 ...
             	 for(tt=0;tt<=charnum;tt++)
             	 {
             	  ...
             	  for(loop=0;loop<30;loop++)
             	  {
             	   ...
             	   {
             	    if (c[tt].items[loop].id) storageitems[temp4++] = c[tt].items[loop].id;
             	    c[tt].numitems++;
             	    ...
             	    if (c[tt].items[loop].equip) wear(tt,loop,0);
             ...
             case 38:  /********* branch on possesion II *****/
              ...
              branch:
                ...
                if (extracode[1] == 2) goto forcebranch;/******** force branch ********/
          ...
          jumptoextradoor:
             ...
             case 41:  /**************** eliminate simple enc option *************/
             	loadextracode(id);
				if ((fp = MyrFopen(":Data Files:CE","r+b")) == NULL) scratch(127);
             ...
             case 42:  /**************** Branch on % Chance *************/
            	...
            	{
            	 if (extracode[1] == -2) t = 99;/**** dropout & erase ****/
            	 if (extracode[1] == 2) goto keepcodes;/**** dropout & keep codes ****/
            	 if (extracode[1] == 1) goto forcebranch;
            	}
             ...
             case 44:
             	for(tt=0;tt<7;tt++) enc2.code[id-1][tt] = enc2.id[id-1][tt] = 0;
             	enc2.code[id-1][7] = 24;
             break;
             ...
             case 43:  /**************** give condition *************/
            	loadextracode(id);
            	 	...
            	 	for(tt=0;tt<=charnum;tt++)
            	 	{
            	 	 if (c[tt].condition[extracode[1]] > 0)  c[tt].condition[extracode[1]] = 0;
            	 	   dosound = FALSE;
		            	switch(extracode[0])
		            	{
		            	 case 0:/****** party ****/
		            	 ...
		            	 case 1:/****** picked ****/
            	 	 	 	if (track[tt]) dosound = TRUE;
		            	 break;
		            	
		            	 case 2:/****** alive ****/
            	 	 	 	if (c[tt].stamina > -10) dosound = TRUE;
		            	 break;
		            	}
             ...
             case 46:/********** branch on quest ***********/
                ...
                {
                 if (gosub) pushonstack(infodoor,t);//***** Fantasoft v7.1 BEGIN
                 goto forcebranch;
                }
             ...
             case 72:/********** branch on range of quest ***********/
				...
				for(tt=extracode[0];tt<=extracode[1];tt++) if (!quest[tt]) smallreply = FALSE;
			...
			loopback2:
             ...
             case 74:/********** Take / Give Spell Points To Picked Characters ***********/
             	 ...
             	 for(tt=0;tt<= charnum;tt++)
				 {
				  ...
				  {
					  needupdate = TRUE;
					  if (extracode[3]) sound(extracode[1]);
					  ...
					  for(loop = 0;loop < temp;loop++)
					  {
					   ...
					   if (extracode[0] < 0) temp2 *= -1;

					   c[tt].spellpoints += temp2;
					   
					   if (c[tt].spellpoints < 0) c[tt].spellpoints = 0;
					   if (c[tt].spellpoints > c[tt].spellpointsmax) c[tt].spellpoints = c[tt].spellpointsmax;
				  }
				 }
				 if (needupdate) updatemain(0,0);

                 if (extracode[4]) textbox(-1,extracode[4],0,0,textrect); /**** text message ***/
             break;
             ...
             case 75:/********** Branch on Spell Points ***********/
					...
					for(tt=0;tt<=charnum;tt++)
					 {
					  if (track[tt])
					  {
						if (c[tt].spellpoints >= extracode[1]) smallreply = TRUE;
					  }
					 }
				...
				else if (extracode[2] == 1) goto keepcodes;
             ...
             case 47:/************ setquest ***********/
                ...
                if (id > 0) quest[id] = TRUE;
                 else quest[abs(id)] = FALSE;
             break;
             ...
             case 48:/************ selective combat ***********/
			   ...
			   if (extracode[1]) battlenum = randrange(extracode[0],extracode[1]);
			    else battlenum = extracode[0];
               ...
               reply = FALSE;
               for(tt=0;tt<=charnum;tt++) if (c[tt].inbattle) reply = TRUE;
               ...
               if (reply)
               {
                ...
                {
                 getfilename("Data TD");
		         if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(128);
            	   ...
            	   if (((extracode[4]) && (c[tt].stamina > 0)) || (!extracode[4]))
             		{
	             	 ...
	             	 {
	             	  case 0: /**** Race ****/
						if (c[tt].race == extracode[2]) track[tt] = TRUE;
						
	             	  break;
	             	  
	             	  case 1: /**** Gender ****/
	             	  	   if (c[tt].gender == extracode[1]) track[tt] = TRUE;
	             	  break;

	             	  case 2: /**** Caste ****/
	             	  	   if (c[tt].caste == extracode[2]) track[tt] = TRUE;
	             	  break;

	             	  case 3: /**** Race Class ****/
	             	  	   loadprofile(c[tt].race,0);
	             	  	   
	             	  	   if (MyrBitTstShort(&races.descriptors,extracode[2]-1)) track[tt] = TRUE;
	             	  break;

	             	  case 4: /**** Caste Class ****/
	             	  	   loadprofile(0,c[tt].caste);
	             	  	   if (caste.casteclass == extracode[2]) track[tt] = TRUE;
	             	  break;
	             	 }
             	 }
               } 
             ...
             case 51:/************ alter shop ***********/
             	 ...
             	 for(tt=0;tt<1000;tt++)
             	 {
             	  if (theshop.id[tt] == extracode[2])
             	  {
             	   ...
             	   if (theshop.num[tt] < 0) theshop.num[tt] = 0;
             	  }
             	 }
             	 saveshop(1,(Ptr)"");
             break;
             ...
             case 52:/************ pick on pos/move/item/%/current selected ***********/
             	 loadextracode(id);
             	 if (extracode[2] != 2) for(tt=0;tt<6;tt++) track[tt] = 0;
            	 for(tt=0;tt<6;tt++) track[tt] = 0;
            	 ...
            	 if ((((extracode[2] == 1) && (c[tt].stamina > 0)) || (!extracode[2])) || ((extracode[2] == 2) && (track[tt])))
             	  {
	             	 ...
	             	 {
	             	  case 0: /**** movement ****/
	             	  	 if (c[tt].movementmax < extracode[1]) track[tt] = TRUE;
	             	  break;
	             	  
	             	  case 1: /**** position ****/
	             	  	   if (tt+1 < extracode[1]) track[tt] = TRUE;
	             	  break;
	             	  
	             	  case 2: /**** possess item id ****/
	             	  	 if (checkforitem(extracode[1],FALSE,tt)) track[tt] = TRUE;
	             	  break;
	             	  
	             	  case 3: /**** % ****/
	             	  	 if (Rand(100) <= extracode[1]) track[tt] = TRUE;
	             	  break;
	             	  
	             	  case 4: /**** DRVs. Attribute, ie, Brawn, Agility ****/
	             	  	 if (!savevsattr(extracode[1],tt)) track[tt] = TRUE;
	             	  break;
	             	  
	             	  case 5: /**** DRVs. Spell Type, ie, Chemical, Electrical ****/
	             	  	 if (!savevs(extracode[1],tt)) track[tt] = TRUE;
	             	  break;
	             	  ...
	             	  case 7: /**** item is worn ****/
	             	  	 if (checkforitem(extracode[1],FALSE,tt))
	             	  	 {
	             	  	  for(loop=0;loop<=c[tt].numitems;loop++)
	             	  	  {
	             	  	   if ((c[tt].items[loop].id == extracode[1]) && (c[tt].items[loop].equip)) track[tt] = TRUE;
	             	  	  }
	             	  	 }
	             	  break;
             ...
             case 53:/************ pick on caste ***********/
            	 ...
            	 if ((((extracode[2] == 1) && (c[tt].stamina > 0)) || (!extracode[2])) || ((extracode[2] == 2) && (track[tt])))
             		{
						if (c[tt].caste == extracode[0]) track[tt] = TRUE;
						
						if (extracode[1] == 1)/*** fighter types ***/
						{
						 if ((c[tt].caste == 1) || (c[tt].caste == 3) || (c[tt].caste == 4)) track[tt] = TRUE;
						}
						
						if (extracode[1] == 2)/*** magical types ***/
						{
						 if ((c[tt].caste == 3) || (c[tt].caste == 6) || (c[tt].caste == 7) || (c[tt].caste == 8)) track[tt] = TRUE;
						}
						
						if (extracode[1] == 3)/*** thief/monk ***/
						{
						 if ((c[tt].caste == 2) || (c[tt].caste == 5)) track[tt] = TRUE;
						}
             	   }
               } 
             break;
             ...
             case 54:/*************** alter time encounter ****************/
				...
				if ((fp = MyrFopen(":Data Files:CTD3","r+b")) == NULL) scratch(129);
				fseek(fp,extracode[0] * sizeof dotime,SEEK_SET);
		  		fread(&dotime,sizeof dotime,1,fp);
		  		CvtTimeEncounterToPc(&dotime);
		  		if (extracode[1] > -1) dotime.percent = extracode[1];
		  		if (extracode[2] > -1) dotime.increment = extracode[2];
		  		if (extracode[3]) dotime.day = tyme.tm_yday;
		  		if (extracode[4] > -1) dotime.day += extracode[4];
             ...
             case 55:/*************** Branch on Picked ****************/
				...
				switch(extracode[0])
				{
				 case 0:/***** Success if ANY picked *****/
				 	for (t=0;t<=charnum;t++) if (track[t]) reply = TRUE;
				 break;
				
				 case 1:/***** Success if Specific Picked *****/
				 	if (track[0]) reply = TRUE;
				 break;
				 ...
				 default:/***** Success if X Number Picked *****/
				 	for (t=0;t<=charnum;t++) if (track[t]) reply++;
				 	if (reply == abs(extracode[0])) reply = TRUE;
				 	 else reply = FALSE;
				 break;
             ...
             case 56:/*************** Branch on Battle Outcome ****************/
			  ...
			  for(tt=0;tt<8;tt++)/**** restore old door codes ****/
			  {
	           ...
	           else if (branchdoorcode == -1)
	           {
	            ...
	            sound(26260);
	            warn(124);
	            for(temp=0;temp<=charnum;temp++) c[temp].exp -= 2000 * c[temp].level;
	            deltax = lastdeltax;
	            deltay = lastdeltay;
	            moveparty(-1);
	            return(TRUE);
			 ...
			 case 63:/*************** Alter Game Time ****************/
             	...
             	{
             	 case 1:/************ set clock **********/
             	 	if (extracode[1] != -1) tyme.tm_yday = extracode[1];
             	 	if (extracode[2] != -1) tyme.tm_hour = extracode[2];
             	 	if (extracode[3] != -1) tyme.tm_min = extracode[3];
             	 break;
			 ...
			 case 65:/*************** Award Random Item(s) ****************/
             	 ...
             	 if (extracode[0] < 0) temp4 = Rand(abs(extracode[0]));
             	  else temp4 = extracode[0];
             	  
             	 for(loop=0;loop<20;loop++) treasure.itemid[loop] = 0;
             ...
             case 60:/*************** Alter party money **************/
             	...
             	for(tt=0;tt<=charnum;tt++)
             	{
             	 if ((!extracode[1]) || (extracode[1] && track[tt]))
             	 {
             	  ...
             	  if (extracode[0] == 3) c[tt].load -= 14*c[tt].money[2];
             ...
             case 62:/*************** Display Scrolling Text **************/
				...
				if (indung)
				{
				if (viewtype == 1)  UpdateWindow(FALSE);
				}
				updatemain(FALSE,-1);
             break;
             ...
             case 61:/*************** shift party level, X: or Y: **************/
             	loadextracode(id);
              	 ...
              	 if (indung)
              	 {
              	  ...
              	  if (extracode[3])
              	  {
              	   if (Rand(100) < 50)  temp = randrange(1,extracode[1]);
              	    else  temp = -(randrange(1,extracode[1]));
              	    floorx += temp;
              	  }
              	   else floorx += extracode[1];
              	  
              	  if (extracode[3])
              	  {
              	   if (Rand(100) < 50) temp = randrange(1,extracode[2]);
              	    else temp = -(randrange(1,extracode[2]));
              	    floory += temp;
              	  }
              	  else floory += extracode[2];
            ...
            if (mode == -1) return(0);/**** monster macro, dont do anything fancy, just return to killbody ***/;
            
			if ((!mode) && (!encountflag)) door[doornum].percent = infodoor.percent = -1;/**** eliminate door ****/

           keepcodes:
           
            if ((encountflag == 2) && (enctry-- > 1)) goto type2;
            if ((encountflag == 1) && (enctry-- > 1)) goto type1;
