...
loaditemnotype(tempid);
  ...
  if (theControl == shopitemsvert)
  {
   ...
   if ((!tempid) && (cr != -1)) item.iconid = 0;
  }
   else
   {
    icon.left = 10;
    if (!tempid) item.iconid = 0;
   }
  ...
  if (item.iconid)
  {
   if (shopequip)
   {
    if ((item.type == 25) || (item.type == 23) || (item.type < 0)) ploticon3(131,icon);
     else ploticon3(176,icon);
   }
   iconhand = NIL;
   ...
   if (item.iscurse)
   {
    ...
    if ((!shopequip) && (!showcurse)) GetIndString(myString,temp+lg,item.iscurse-temp+1);// Fantasoft 7.1
   }
    
   if (!lg) TextFace(outline);
   
   if (initems) SetPort(GetWindowPort(itemswindow));
...
{
 EraseRect(&checkrect);
 if (t != cr) PlotCIcon(&checkrect,portrait);
}

if (t == cl) ploticon3(145,checkrect);
...
{
 EraseRect(&checkrect);
 if (t != cl) PlotCIcon(&checkrect,portrait);
}

if (t == cr) ploticon3(145,checkrect);

if (portrait) DisposeCIcon(portrait);
...
{ 
	...
	if ((**iconhand).iconBMap.bounds.bottom ==  44) InsetRect(&where,-6,-6);
	...
	if (who > -1)
	{
		...
		else if (thePort != GetWindowPort(screen)) noshow = TRUE;
		...
		if ((c[who].condition[9]) || (c[who].condition[28]) || (c[who].condition[2])) poison = TRUE;
		if ((c[who].condition[25] < 0) || (doauto[who]) || (c[who].traiter)) charm = TRUE;
		if ((c[who].condition[0]) || (c[who].condition[2]) || (c[who].condition[6])) movement = TRUE;
		if ((c[who].condition[1]) || (c[who].condition[26]) || (c[who].condition[27])) physical = TRUE;
		if ((c[who].condition[5]) || (c[who].condition[29]) || (c[who].condition[39])) magic = TRUE;
		if ((c[who].condition[36]) || (c[who].condition[37]) || (c[who].condition[3])) attdef = TRUE;
