...
void bodycount(void)
{
 ...
 for(t=0;t<maxloop;t++) select[t] = 0;
 ...
 for(t=0;t<nummon;t++)
   {
    ...
    if (monster[t].cansum < 0) monster[t].traiter = 0;
    if ((monster[t].stamina > 0) && (!monster[t].traiter) && (monster[t].cansum)) select[count++] = t+1;
   }
 ...
 for(t=0;t<32;t++) if (select[t] < 0) maxtochose++;
 if (maxtochose > 18) maxtochose = 18; 
...
EndUpdate(GetDialogWindow(bodyc));
  ...
  if (((147-chosen+maxtochose) >  146) && (147-chosen+maxtochose < 155)) compassnew = GetCCursor(147-chosen+maxtochose);
   else compassnew = GetCCursor(147);
		...
		for(t=0;t<32;t++)
		{
		 if (select[t] < 0)
		 {
		  ...
		  if (chosen > -1)
		  {
			  ...
			  for(tt=0;tt<4;tt++) if (holdover[chosen].condition[tt] > 0) holdover[chosen].condition[tt] = 0;
			  ...
			  for(tt=0;tt<6;tt++) monster[abs(select[t])-1].items[tt]=0;
			  for(tt=0;tt<3;tt++) monster[abs(select[t])-1].money[tt]=0;
		  }
		 }
		}
 		for(t=0;t<maxmon;t++) monster[t].stamina = 0;
...
void monsterupdate(short who,Rect where)
{
     ...
     if (!monster[who].size) InsetRect(&where,19,19);
     if (monster[who].size == 2) InsetRect(&where,3,19);
     if (monster[who].size == 1) InsetRect(&where,19,3);
     if (monster[who].size == 3) InsetRect(&where,3,3);
...
void updatenpcmenu(void)
{
 ...
 for(t=1;t<=20;t++)
 {
  ...
  if (t >= heldover) holdover[t].name = 0;// Fantasoft v7.1   Clean out any junk that does not belong.
 }
 ...
 for(t=1;t<=heldover;t++)
 {
  ...
  CtoPstr(monstername);
  SetMenuItemText(gNPC,t,(StringPtr)monstername);
  if (holdover[t-1].iconid < 512) SetItemIcon(gNPC,t,holdover[t-1].iconid);
