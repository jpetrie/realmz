...
short spelllist(short who,short special)
{
	...
	if (who <= charnum)
	{
	 if ((c[who].condition[30]) && (Rand(100) < 34)) who = q[up];
	}

	if (who < 0) return(0);
	if (twixt(who,charnum+1,9)) return(0);

	
	if ((special == 59) || (special == 60)) /***** power surge/drain ******/
	{
	 if (special == 60) damage = -(damage);
	 if (who < 9)
	 {
	  c[who].spellpoints += damage;
	  damage = 0;
	  if (c[who].spellpoints < 0) c[who].spellpoints = 0;
	  if (c[who].spellpoints > c[who].spellpointsmax) c[who].spellpoints = c[who].spellpointsmax;
	  updatecharshort(who,FALSE);
	 }
	 else
	 {
	  monster[who-10].spellpoints += damage;
	  if (monster[who-10].spellpoints < 0) monster[who-10].spellpoints = 0;
	  damage = 0;
	 }
	 return(0);
	}
...
donefumble:
	...
	if (special == 63)/**** detect magic ****/
	{
		if (who > 9)
		{
			for(t=0;t<6;t++)
			{
				...
				if (temp) loaditem(temp);
				if (item.ismagical) monster[who-10].items[t] = -(temp);
			}
		}
	}
	...
	if ((special == 49) || (special == 27))/********** death magic/Flesh to Stone **********/
	{
	 if (who < 9)
	 {
	   ...
	   if (special == 27) c[who].condition[26] = -1;
	 }
	  else damage = adjdam = 10 + monster[who-10].stamina;
	}
	...
	if (special == 61) /***** dispel Magic **  leave broke apart ***/
	{
		if (who < 9)
		{
		 for(t=0;t<40;t++)
		 {
		  if (c[who].condition[t] > 0) c[who].condition[t] = 0;
		  if (c[who].traiter)
		  {
		   c[who].traiter = 0;
		   killparty--;
		...
		else
		{
		 for(t=0;t<40;t++)
		 {
		  if (monster[who-10].condition[t] > 0) monster[who-10].condition[t] = 0;
		 }
		}
	}
	...
	if (special == 62)/************** remove curse *****************/
	{
		...
		for(t=0;t<c[who].numitems;t++)
		{
			...
			if ((item.iscurse) && (c[who].items[t].equip)) removeitem(who,t,FALSE,TRUE);
		}
	}
...
onemoretime:
	 ...
	 if ((fp = MyrFopen(filename,"rb")) == NULL)
	 {
	   ...
	   fclose(fp);/***** close monster file ******/
		...
		for(t=0;t<40;t++) if (monpick.condition[t] > -1) monster[who-10].condition[t] = monpick.condition[t];
		monster[who-10].dx += (Rand(3) - 2);
		monster[who-10].stamina = monster[who-10].bonus;
		for(temploop=0;temploop<monster[who-10].hd;temploop++) monster[who-10].stamina += Rand(8);
		...
		if ((monster[who-10].dx) < 1) monster[who-10].dx = 1;
		
		monster[who-10].magres += (3*howhard);
		for(loop2=0;loop2<6;loop2++) monster[who-10].save[loop2] += 7*howhard;
		...
		if (monster[who-10].stamina < 1) monster[who-10].stamina = 1;
	...
	if (special == 58) /***** summon monster *****/
	{
		while (done == FALSE)
		{
			for(loop1 = nummon; loop1 < maxmon; loop1 ++) if (monster[loop1].stamina <= 0) sumnum = 1;
			
			/****** make sure you dont summont 2 to the same spot or overlap ***/
			if (field[target[loop][0]][target[loop][1]] <= maxloop) sumnum = 0;
			if ((field[target[loop][0]][target[loop][1]-1] <= maxloop) && ((monpick.size == 1) || (monpick.size == 3))) sumnum = 0;
			if ((field[target[loop][0]-1][target[loop][1]] <= maxloop) && (monpick.size >1)) sumnum = 0;
			if ((field[target[loop][0]-1][target[loop][1]-1] <= maxloop) && (monpick.size == 3)) sumnum = 0;
			...
			if (sumnum)
			{
				...
				for(loop1 = nummon; loop1  <maxmon; loop1 ++)
				{
					if (monster[loop1].stamina < 1)
					{
						...
						for(loop2=0;loop2<monster[loop1].hd;loop2++) monster[loop1].stamina += Rand(8);
						...
						if ((monster[loop1].dx) < 1) monster[loop1].dx = 1;
						...
						if ((monster[loop1].magres < 99) && (monster[loop1].magres > 9)) monster[loop1].magres += (3*howhard);// Fantasoft v7.0
						for(loop2=0;loop2<6;loop2++) monster[loop1].save[loop2] += 7*howhard;
						...
						if (monster[loop1].stamina < 1) monster[loop1].stamina = 1;
	...
	{
		if (q[up] < 9) too = c[charup].traiter;
		 else too = monster[monsterup].traiter;
		...
		else /*** charm to evil ***/
		{
			...
			{
				if (!monster[who-10].traiter) numenemy++;
	...
	if (special == 2)
	{
		...
		else monster[who-10].movement = 0;
	}
	...
	if ((special == 7) || (special == 3))/***** slow * entangle ***/
	{
		if (who < 9)
		{
		 ...
		 else  monster[who-10].movement = monster[who-10].movement/2;
	}
	...
	if (special == 10) /******* poison *****/
	{
		if (who < 9)
		{
		 if (c[who].condition[25] < 0) c[who].condition[9] = 0;
		 updatechar(who,3);
		}
		else if ((monster[who-10].spellimmune[5]) || (monster[who-10].condition[25])) monster[who-10].condition[9] = 0;
	}
	...
	if ((special == 24) || (special == 91)) /******* haste/age *****/
	{
	 ...
	 if (who <= charnum)
	 {
	  ...
	  if (special == 24) tempage = powerlevel * 30;
	   else tempage = duration * 30;
	  ...
	  if (temp)
	  {
	   if (FrontWindow() == look) in();
	   showageupdate(who,c[who].currentagegroup,0);
	   if (FrontWindow() == look) updatemain(0,-1);
	    else if (FrontWindow() == GetDialogWindow(charstat)) cleanage = TRUE;
	  }
	 }
    }
	...
	if (special == 92) /******* youth effect *****/
	{
	 ...
	 if (who < 9)
	 {
	  ...
	  if (c[who].age < 3650) c[who].age = 3650;
	  
	   temp = Rand(3);
	   c[who].staminamax -= temp;
	   if (c[who].staminamax < 1) c[who].staminamax = 1;
	   
	   c[who].stamina -= temp;
	   if (c[who].stamina < 1) c[who].stamina = 1;
