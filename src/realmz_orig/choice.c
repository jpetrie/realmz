...
short choice(short key)
{
	...
	if ((theControl != shopitemsvert) && (theControl != charitemsvert))
	{
		...
		{
		 ploticon3(132,r);
		 if (shopavail) saveshop(1,(Ptr)""); /***** update current game shop ***************/
			...
			if ((moneypool[0]) || (moneypool[1]) || (moneypool[2] ))
			{
			  ...
			  if (bankavailable)
			  {
			   ...
			   else moneypool[0] = moneypool[1] = moneypool[2]= 0;
			  DisposeWindow(gshop);
			  gshop = NIL;
			  inshop = FALSE;
			  charselectnew = cl;
		...
		if (shopavail)
		{
			if ((theControl == goshopinshop) && (cr != -1)) theControl = weapons;
		...
		if (shop) SetControlMaximum(shopitemsvert,191);
	...
	else
	{
		if (((point.h > (389+ (leftshift/2))) && (point.h < (423+ (leftshift/2)))) || ((point.h >9) && (point.h < 43)))
		{
			...
			if ((point.v > 15)	&& (point.v <48)) gshopitem=1;
			if ((point.v > 55) && (point.v <88)) gshopitem=2;
			if ((point.v > 95) && (point.v <128)) gshopitem=3;
			if ((point.v > 135) && (point.v <168)) gshopitem=4;
			if ((point.v > 175) && (point.v <208)) gshopitem=5;
			if ((point.v > 215) && (point.v <248)) gshopitem=6;
			if ((point.v > 255) && (point.v <288)) gshopitem=7;
			if ((point.v > 295) && (point.v <328)) gshopitem=8;
			if ((point.v > 335) && (point.v <368)) gshopitem=9;

			index = curControlValue + gshopitem;
			if ((gshopitem > characterl.numitems) &&  (ischar)) gshopitem = 0;
			if ((gshopitem > characterr.numitems) && (!ischar) && (shop == FALSE)) gshopitem = 0;
			if ((!theshop.num[shopselection+gshopitem+curControlValue-1]) && (!ischar) && (cr == -1)) gshopitem = 0;
			
			
  if (screensize)
	{
	 if (point.h < 300) quickinfo(cl,index-1,c[cl].items[index-1].id,1);
		else if (cr > -1) quickinfo(cr,index-1,c[cr].items[index-1].id,1);
		 else quickinfo(-1,-1,theshop.id[shopselection+gshopitem+curControlValue-1],1);
	}
		...
		if ((gshopitem) && (!ischar))
		 {
			if (gTheEvent.modifiers&cmdKey)
			{
			 if (cr != -1)
			 {
			  if (c[cr].items[index-1].equip) removeitem(cr,index-1,TRUE,FALSE);
			  else wear(cr,index-1,TRUE);
			  if (cr != -1) characterr = c[cr];
			  theControl = rightarrow;
			  change(FALSE,TRUE);
			  select[cr] = TRUE;
			...
			{
			 if (gTheEvent.modifiers&optionKey) moveicon();
			  else if (!BitAnd(gTheEvent.modifiers,controlKey))
				{
				  if (!screensize) warn(60);//*** give slight delay before warning to give chance for just update item info check
					else
					{
					 oldtime = TickCount();
					 while (StillDown())
