...
void partyloss(short mode)
{
 ...
 if (revivepartyflag) goto revive;//Fantasoft v7.1
...
revive:
 ...
 if (!mode)
 {
		for(t=0;t<20;t++)
		{
		 ...
		 if (tempwindow != GetDialogWindow(background)) DisposeWindow(tempwindow);
		}
		...
		for(t=0;t<60;t++) que[t].duration = que[t].phase = 0;
	    ...
	    DisableItem(gFile,2);
  		DrawMenuBar();
	    SetCCursor(sword);
	    if ((!reducesound) && (!mode)) sound(26260);
	    ...
	    fat = 4;
	    cleartarget();
		for(t=1;t<20;t++) DisableItem(gScenario,t+3);
