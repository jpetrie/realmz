...
{
 #if development
 	if (divineref) return(1);
		else return(0);
 #else
 	if ((MyrBitTstLong(&serial,6+divine)) || (!serial)) return(FALSE);
  	 else if ((!MyrBitTstLong(&serial,6+divine)) && (serial)) return(TRUE);
  	 exit(0);
     return(0);
 #endif
}
...
void drawspell(void)
{
 if ((spellinfo.targettype == 5) || (!spellrange)) return;
...
{       
 ...
 if (character.load < 0) character.load = 0;
    
 		character.loadmax = (character.st + character.magst) * (character.st + character.magst) * 20;
 		if (character.loadmax < 500) character.loadmax = 500;
		top = character.load;
		bottom = character.loadmax;
		temp = 1+(((bottom - top) / bottom) * movement);
	    if (character.condition[2]) temp -= character.condition[2];/**** tangled ***/
	    if (character.condition[6]) temp /= 2;/**** slow ****/
	    if (character.condition[23]) temp *= 2;/******* haste *****/
		if (temp < 2 ) temp = 2;
...
{ 
 ...
 if (!mode)
 {
	if (screen != NIL) DisposeWindow(screen);
...
void getexp(short level,short casteid,short who)
{
 ...
 if (who == -1) characterl.exp = value;
  else c[who].exp += value;
}
/************* cleartarget *******************/
void cleartarget(void)
{
 short t;
 pick = 0;
 for(t=0;t<7;t++) target[t][0] = target[t][1] = -1;
...
void pict(short id,Rect frame)
{
 ...
 if (picturehandle) DrawPicture(picturehandle,&frame);
}
/************************ string ********************/
...
void loadextracode(short id)
{
 ...
 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(118);
...
{
 short reply = FALSE;
 if ((tween > left) && (tween < right)) return (TRUE);
 return(reply);
}
/************************* twixt ******************/
...
{
 short reply = FALSE;
 if ((tween >= left) && (tween <= right)) reply = TRUE;
 return (reply);
}
/**************************** randrange ************************/
...
void updateprep(void)
{
 ...
 SetRect(&itemRect,leftsize,0,rightsize,50);
  ...
  for(t=0;t<= charnum;t++)
  {
   if (c[t].spellpointsmax) temp = 192;
     else temp = 189;
   pict(temp,itemRect);
   updatepictbox(t,0,0);
   itemRect.top +=50;
...
/**********************/
    ...
    if (damage > caste.strength[1]) damage = caste.strength[1];/********** check for max damage bonus ************/
}
/******************* getcharrange *****************/
 ...
 for(t=0;t<=charnum;t++)
  {
    ...
    if (c[t].stamina >0)
    {
     ...
     range[t] = currange;
     if (currange < range[6]) range[6] = currange;
    }
  }
}
/********************* GetDialogNum **************************/
 ...
 else
 {
	 if (indung)
	 {
	  MyrNumToString(floorx,myString);
 	  if (xydisplayflag) MyrDrawCString("?");
 	   else MyrDrawCString((Ptr)myString);
	  MoveTo(406+leftshift,314+downshift);
...
void scratch(short location)
{
 ...
 if ((!nofade) && (!nologo)) fadeinout(25, fadein);/******** fade to black ******/

 strcpy(loc,(StringPtr)"");
 
 if (!MyrBitTstLong(&serial,6+divine)) strcat(loc,(StringPtr)"G");
 if (quest[127] == 66) strcat(loc,(StringPtr)"S");
...
// PtoCstr(hold);
 ...
 if (lowHD) MyrParamText((Ptr)loc,(Ptr)"Note: You are low on hard drive space and this is likely the cause.  Free up some more space and try again.",(Ptr)"",(Ptr)"");
  else ParamText((StringPtr)loc,(StringPtr)"",(StringPtr)"",(StringPtr)"");
...
void savecharacter(short who)
{
 ...
 if (who < 0)
 {
  ...
  if ((doreg()) || (tempchar.level < 4))
  {
	  ...
	  if ((fp = MyrFopen(charfilename,"w+b")) == NULL) scratch(2);
...
{	
	...
	if (theControl == charitemsvert) rect.left = 10;
	...
	switch (theCode)
	{
		 ...
		 rect.bottom = 386;
		  for (tt=0;tt<9;tt++)
		  {
 		  ...
 		  if (theControl == charitemsvert) 
 		  {
 		   ...
 		   if (tempid) showcharge2(charge,lg);
		...
		case kControlDownButtonPart:
		  ...
		  { 
		   curControlValue++;
		   scroll = TRUE;
		   if ((theControl == charitemsvert) && (characterl.numitems <10)) scroll = FALSE;
		   if ((theControl == shopitemsvert) && (characterr.numitems <9) && (cr != -1)) scroll = FALSE;
		  
		   if (scroll == TRUE) for (t=0;t<4;t++) ScrollRect (&rect,0,-10,0L);
 		   delay(2);// Slows it down on fast machines.
 		   icon.top = 336;
		...
		case kControlUpButtonPart:
		  ...
		  { curControlValue--;
		  for (t=0;t<4;t++) ScrollRect (&rect,0,10,0L);
		  delay(2);// Slows it down on fast machines.
		  icon.top = 16;
...
void updateshop(void)
{
	...
	if (moneypool[0] < 1) moneypool[0] = 0;
	if (moneypool[1] < 1) moneypool[1] = 0;
	if (moneypool[2] < 1) moneypool[2] = 0;
...
void shortupdate(short mode)
{
 needupdate = FALSE;
 updateprep();
 for(t=0;t<=charnum;t++) updatechar(t,mode);
}
/**************************** selectupdate ****************/
void selectupdate(void)
{
 for(t=0;t<=charnum;t++)
 {
  if (select[t])
  {
   if (select[t] > 0) updatecharshort(t,0);
    else updatecharshort(t,1);
   select[t] = 0;
  }
 }
}
...
void center(short mon)
{
 ...
 {
  mon -=10;
  if ((!twixt(monpos[mon][0],1,8)) || (!twixt(monpos[mon][1],1,8))) centerfield(monpos[mon][0],monpos[mon][1]);
 }
  else if ((mon > -1) && (mon <= charnum))
	{
	 if ((!twixt(pos[mon][0],1,8)) || (!twixt(pos[mon][1],1,8))) centerfield(pos[mon][0],pos[mon][1]);
	} 
}
/************************ savevs ***********************/
short savevs(short which,short who)
{
 ...
 if (inspell)
 {
	 if (spellinfo.cannot > 1) return (FALSE);
	 spelladjust = (powerlevel*spellinfo.saveadjust) + spellinfo.savebonus;
 }
  
 if ((who > -1) && (who <= charnum))
 {
  if ((which == 0) && (partycondition[7])) spelladjust += 50;
  if (temp <= c[who].save[which]+spelladjust) return (TRUE);
 }
 else if (who > 9)
 {
  if (which == 7)/**** special ****/
  {
   for(t=0;t<6;t++) special += monster[who-10].save[t];
   special /= 6;
     if (temp <= special + spelladjust) return (TRUE);
  }
  else
  {
   if ((temp <= monster[who-10].save[which] + spelladjust) || (monster[who-10].spellimmune[which])) return (TRUE);
   if ((monster[who-10].type[1]) && ((which == 5) || (which == 4) || (which == 0))) return(TRUE);
  }
 }
 return(FALSE);
}
...
short savevsattr(short which,short who)
{
 ...
 if ((who < 0) || (who > charnum)) return(TRUE);
 ...
 if (Rand(100) < adjust) return(TRUE);
 return(FALSE);
}
/***************************** adddelscen ********************************/
void adddelscen(short mode)
{
 ...
 Str255 teststring;
 	...
 	if (mode) MyrParamText((Ptr)"Enter the name of the scenario you wish to install and click OK.",(Ptr)"",(Ptr)"",(Ptr)"");
 		else MyrParamText((Ptr)"Enter the name of the scenario you wish to uninstall and click OK.",(Ptr)"",(Ptr)"",(Ptr)"");
...
void downbutton(short newport)
{
 GrafPtr oldport; 
 GetPort(&oldport);
 if (newport) SetPort(GetWindowPort(screen));
		 ...
		 if (theControl == autoone[t])
		 {
		  ...
		  //buttonrect = *&(**(autoone[t])).contrlRect;
			  ...
			  else
			  {
			   ...
			   if ((incombat) && (t == q[up])) theControl = campbut;/********* lets rock!!!!!! YAHHHHHHHHHHH!!!!  ************/
			   for (tt=0;tt<=charnum;tt++) if (doauto[tt]) count++;
			  }
		 }
	}
	if (count > charnum) warn(107);/**** let them know how to get out of ALL AUTO ****/
}
/****************************** doreg **********************************/
short doreg(void)
{
...
madechoice:
		   ...
		   if (!mode) copyresource(tempport, portraitrefnum, 'cicn', choice+256, fromid);
		  	else copyresource(temptact, tacticalrefnum, 'cicn', choice+8999, fromid);
		  	
nochoice:
		  
		  if (choice > 0)
		  {
		   if (!mode) return(choice+257);
		    else return(choice+8999);
		  }
		   else return(0);
 return(0);
}
...
void bandaid(void)
{
 ...
 for(t=0;t<=charnum;t++)
 {
  ...
  if (temp > c[t].spec[13]) c[t].spec[13] = temp;
  ...
  if (c[t].version > -3) showmessage2 = TRUE;/******* look for old style PC and bail out ******/
 }
  
 
 if (showmessage1) flashmessage((StringPtr)"Some of these characters have invalid items.  Those items have been deleted. (Click)",50,40,0,5000);
 if (showmessage2)
 {
...
{
	short reply = 0;
	if (c[who].verify1 != (c[who].level + c[who].staminamax + c[who].attackbonus)) reply = TRUE;
	if (c[who].verify2 != (c[who].tohit + c[who].spellpointsmax+c[who].st + c[who].in)) reply = TRUE;
	if (c[who].verify3 != (c[who].ac + c[who].staminamax + c[who].de + c[who].staminamax+c[who].magres)) reply = TRUE;
	return (reply);
}
/**************************** updatemapmenu *****************/
void updatemapmenu(void)
{
 ...
 FILE *fp = NULL;
	...
	for(t=1;t<21;t++)
	{
	  if (map[t-1]) GetIndString(myString,-102,t);
	   else GetIndString(myString,-101,t);
	   
	  SetMenuItemText(gScenario,t+3,myString);
	  if (map[t-1]) EnableItem(gScenario,t+3);
	   else DisableItem(gScenario,t+3);
	} 
 
     getfilename("Data NI");
	 if ((fp = MyrFopen(filename,"rb")) == NULL) scratch(104);
	 fread(&allsupply,sizeof allsupply,1,fp);
	 CvtTabItemAttrToPc(&allsupply,200);
	 fclose (fp);
	 	 
	 if (quickshow) CheckItem(prefer,7,1);
	  else CheckItem(prefer,7,0);
}
/****************************** Startlevel **********************************/
...
void sound(short id)
{
	...
	if (sndhandle != NIL)
	{ 
		if (id > 0) SndPlay(cool[channel],(SndListHandle)sndhandle[channel],TRUE);
		else SndPlay(cool[channel],(SndListHandle)sndhandle[channel],FALSE);
	}
}
/********************* updatemain **************************/
...
{ 
 ...
 if(screen)  // Myriad, current port can be NULL
	 ...
	 if ((!who) || (needupdate)) shortupdate(0);
	  else
	  {
	   out();
	   selectupdate();
 ...
 for (;;)
	{	
		...
		if (IsDialogEvent (&gTheEvent))
		{
		 ...
		 if(gTheEvent.what == keyDown) goto dokey;
		}
		else
		{
			...
			switch (gTheEvent.what)
	            ...
	            goback2:
	              ...
	              {
	               case -2:
	                 if (shopavail) theControl = shopbut;
	                  else theControl = tradebut;
	                 goto goback2;
	               break;
...
{ 
	...
	if (currentscenario < 20) fp = MyrFopen(":Data Files:Data Race","rb");//******** open standard Race file
...
{ 
	...
	if (currentscenario < 20) fp = MyrFopen(":Data Files:Data Caste","rb");//******** open standard caste file
...
short usercheck2(void)
{
  ...
  else strcpy(scenname,(StringPtr)"");
 ...
 if (!strlen(scenname)) return(TRUE);/*** allow them to warp in tutorial ****/
 if (!strcmp(codename,scenname)) return(TRUE);/********** Valid person to warp ****/
#endif
 return(FALSE);
}
/******************* getdivineuser ***************/
	   ...
	   if (selectscenario((Ptr)myString,0))
		{
			...
			if (temp != -1) strcat(myString,theString);
			else strcat(myString,(StringPtr)"Unknown");
			CtoPstr((Ptr)myString);
	...
	if ((fp = MyrFopen(filename,"rb")) == NULL)
	 {
	  ...
	  for (t=0;t<1024;t++) solids[t] = 0;
	  fwrite(&solids,sizeof solids,1,fp);
	 }
	  else fread(&solids,sizeof solids,1,fp);
	 fclose (fp);
...
void updateshopwings(int cl, int cr)
{
 ...
 if (cl == -2) character[0] = characterl;
	else character[0] = c[cl];
 if (cr == -2) character[1] = characterr;
	else if (cr != -1) character[1] = c[cr];
