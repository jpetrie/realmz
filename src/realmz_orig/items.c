...
short items(void)
{
	...
	if (!incombat) music(6);/**** items music ***/

	for(loop=1;loop<8;loop++) track[loop]=0;/**** for encounter use ******/
...
backup:
	...
	if (screensize) quickinfo(charselectnew,itemselectnew+incr,c[charselectnew].items[itemselectnew+incr].id,0);
	...
	if (c[charselectnew].stamina < 1)
	{
	 ...
	 if (c[charselectnew].stamina < -9) ploticon3(2015,charbox);	
	}
	...
	for(t=0;t<=charnum;t++)
	 {
	    c[t].condition[1] = 0;/******** remove HELPLESS condition *****/
	 	if (t == 3) OffsetRect(&buttonrect,24,-72);
		...
		if (t == charselectnew) ploticon3(145,buttonrect);
	...
	if (c[charselectnew].gender == 1) MyrCDiStr(27,(StringPtr)"Male");
	else MyrCDiStr(27,(StringPtr)"Female");
...
checkcontrols:
					...
					if (item.xcharge)
					{
					 ...
					 ploticon3(133,r);
						for(count=first;count<30;count++)
						{
							...
							{
								sound(663);
								if (c[charselectnew].items[count].equip) equip = TRUE;
								if (c[charselectnew].items[count].ident) ident = TRUE;
				...
				if ((theControl == use) && (c[charselectnew].numitems))
				{
				 memoryspell = FALSE;
					...
					if (c[charselectnew].items[itemselectnew+incr].charge > 0) usecharge = TRUE;
				...
				{					
		            ...
		            if ((item.sp2 > 1100) && ((usecharge) || (item.charge < 0))) loadspell2(item.sp2);
		            
					if (spellinfo.spellclass == 9)
					{
					 if (!incombat) goto tryagain;
						if (checkforenemy(1))
						{
							SetPort(GetWindowPort(itemswindow));
							warn(39);
...
bringback1:
					...
					if (item.sound) sound(item.sound + 600);
					...
					if ((item.sp2 > 1100) && ((usecharge) || (item.charge < 0)))
					{
					 loadspell2(item.sp2);
						...
						{
campcast:
							if ((spellinfo.targettype > 2) || (spellinfo.targettype == 7)) temp = charnum;
...
pushon:
							...
							if (select[charselectnew] < 0) goto backup;
							goto tryagain;
						}
					}
...
bustout:
				...
				oneup:
					...
					do
					{
						...
						{
							incr--;
							for(t=0;t<4;t++) ScrollRect (&box,0,10,0L);
							delay(2);// Fantasoft 7.1  Slows it down a bit on fast machines.
							theControl = tempdummy;
							updateitems(0,0);
							quickinfo(charselectnew,itemselectnew+incr,c[charselectnew].items[itemselectnew+incr].id,0);
				 ...
				 if (theControl != describe)
				 {
					...
					else
					{
					 for(t=0;t<=charnum;t++)
					 {
					  ...
					  {
					   c[t].spellpoints -=25;
					   for(tt=0;tt<c[charselectnew].numitems;tt++) c[charselectnew].items[tt].ident = TRUE;
					   theControl = tempdummy;
					   updateitems(0,10);
					   select[t] = TRUE;
					   sound(683);
					...
					if (theControl != describe) goto backup;
...
dropitemsfast:
				...
				if (theControl == checkcontrol)
				{
				 change:
					sound(141);
					incr = 0;
					if (c[charselectnew].load < 0) c[charselectnew].load = 0;
					...
					checkrect.left = 353+leftshift;
				 	...
				 	checkrect.bottom = checkrect.top + 22;
						...
						for(t=0;t<=charnum;t++)
						 {
						 	if (t == 3) OffsetRect(&checkrect,24,-71);
				...
				forcepick:
					...
					if (doit)
					{
						if (c[charselectnew].items[itemselectnew+incr].equip)
						{
							...
							if (wear(charselectnew,itemselectnew+incr,TRUE))
							{
								theControl = tempdummy;
...
getout:
	if (encountflag) track[charselectnew] = 1; /***** for encounter use ****/
	DisposeDialog(charstat);
	DisposeWindow(itemswindow);
	itemswindow = NIL;
	out();
