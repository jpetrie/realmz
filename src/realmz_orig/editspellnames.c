...
OSErr SetIndString(StringPtr theStr,short resID,short strIndex)
{
   ...
   if (!theRes || !(*theRes)) return resNotFound;
    ...
    for(;;)   
     {   
         ...
         if ((itemHit == 24) || (itemHit == 25))
        {
		  ...
		  sound(141);
          if (itemHit == 24) classindex --;
           else classindex ++;
           
           if (classindex < 1) classindex = 3;
           if (classindex > 3) classindex = 1;
...
void editracecaste(void)
{
 ...
 DialogRef gGeneration;
	...
	update:
	  ...
	  if (!mode) MyrCDiStr(63,(StringPtr)"Current Race Names.");
	   else MyrCDiStr(63,(StringPtr)"Current Caste Names.");
