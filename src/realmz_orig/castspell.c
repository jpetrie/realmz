...
selectagain:
	...
	if (!cancast(charselectnew,1))
	{
		...
		if (charselectnew > charnum + 1)
		{
			...
			{
				warn(13);
				if (describe) DisposeDialog(describe);
				return(1100);
			}
		}
		goto selectagain;
	...
	if (!reducesound) sound(20002);
	gCurrent = spellwindow;
	SetPortDialogPort(spellwindow);
	BackPixPat(base);
	TextFont(defaultfont);
	
	if (!screensize) MoveWindow(GetDialogWindow(spellwindow),GlobalLeft+350+leftshift,GlobalTop,FALSE);
	 else MoveWindow(GetDialogWindow(spellwindow),GlobalLeft+469,GlobalTop,FALSE);
...
wayback:
	...
	if (defaultspell)
	{
		...
		if (castnum > -1)
		{
			...
			if (spellinfo.cost < 0) nopower = TRUE;
	...
	{
	 if (!describe) describe = GetNewDialog(169+(1000*screensize),0L,(WindowPtr)-1L);
 ...
 if (DialogSelect (&gTheEvent,&dummy,&itemHit))
 {
  GetDialogItem(spellwindow,itemHit,&itemType,&itemHandle,&buttonrect);
		...
		if (((itemHit == 39) || (itemHit == 40)) && (!incombat) && (charnum - killparty > 0))
		{
			...
			while(loop == TRUE)
			{
				if (itemHit == 39)
				{
					...
					if (cancast(charselectnew,1)) loop = FALSE;
				}
				else
				{
					charselectnew++;
		...
		bigclick:
				...
				if (itemHit - 23 == castlevel) goto back;
				GetDialogItem(spellwindow,castlevel + 23,&itemType,&itemHandle,&buttonrect);
				upbutton(FALSE);
...
powertest:	 
				...
				if ((castnum == -1) || (itemHit - 29 == powerlevel)) goto back;
...
maxpower2:
					...
					if (castnum > -1) DialogNum(42,abs(spellinfo.cost*powerlevel*scribing));
					if (nopower)
					{
						itemHit = castnum + 1;
		...
		smallclick:	
				...
				if (((incombat) && (spellinfo.incombat)) || ((!incombat) && (spellinfo.incamp)) || (scribing == 2) || (encountflag))
				 {
				  ...
				  if (oldred) RGBForeColor(&lightgrey);
				   else ForeColor(yellowColor);
				  oldred = FALSE;
				  if (castnum+1 != itemHit) MyrPascalDiStr(castnum+1,myString);
			...
			advanceit:
					 itemHit += smallclickdirection;
					  if ((itemHit > 0) && (itemHit < 13)) goto smallclick;
					  else
					  {
					   if (smallclickdirection == -1) goto back;
				...
				upbutton(FALSE);
				if (abs(spellinfo.cost*powerlevel*scribing)	> c[charselectnew].spellpoints) goto maxout;
			}
			else goto advanceit;
			...
			dokey:
				    ...
				    if (whichset)/***** set special spells *****/
				    {
				     ...
				     skipdefault = TRUE;
						...
						{
						 if (keylevel[tier] < 0) keylevel[tier] = 0;
						  else keylevel[tier] = 6;
						}

						if (((keylevel[tier] < 1) || (keylevel[tier] > 7)) && (tier == 2))
						{
						 if (keylevel[tier] < 1) keylevel[tier] = 1;
						  else keylevel[tier] = 7;
						}

						if ((tier == 1)  && ((keylevel[tier] < 0) || (keylevel[tier] > 11)))
						{
						 if (keylevel[tier] < 0) keylevel[tier] = 0;
						  else keylevel[tier] = 11;
						}
...
out:
	...
	if (encountflag)
	{
		for(loop=0;loop<8;loop++) track[loop]=0;
...
void updatetier(short mode)
{
 ...
 if (tier == 1) OffsetRect(&tierrect,87,0);
